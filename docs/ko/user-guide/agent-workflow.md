...
모든 작업에 적용할 메서드 순서를 외우지 않습니다. 최신 권한 상태의 `next_action`에서
시작하고 `owner_method`가 있으면 그 경로를 따릅니다. 현재 Volicord 결과가 없거나,
Task를 알 수 없거나, 권한 상태를 새로고쳐야 할 때 먼저 현재 상태를 확인합니다.
안심하기 위해 단계 사이마다 status를 반복 조회하지 않습니다.

반환된 전달 지점마다 다음을 수행합니다.
...
4. Volicord가 보여 주는 현재 적용 범위와 호환되는 쓰기 또는 민감 동작 경계 안에서만
   행동합니다.
5. 반환된 전달 지점이 요구할 때 행동 뒤의 의미 있는 실행과 증거를 기록합니다.
6. 가장 중요한 차단 사유, 확인한 것, 빠진 것, 다음 행위자, 다음 안전한 행동 하나를
   보고합니다.
7. 증거, 최종 수락, 잔여 위험, 세션 Stop, Task 완료를 서로 구분합니다.

반환된 전달 지점은 확인, 쓰기, 기록, 사용자 질문, 대기, 조정, 닫기 중 하나를
가리킬 수 있습니다. 기록된 사실이 바뀌면 다음 응답이 다른 경로를 선택할 수 있습니다.
...
| 판단 요청 | 초점이 맞춰진 질문 하나, 선택지, 결과, 좁은 추천이 있다면 그 추천을 보여 줍니다. | 답하거나, 거절하거나, 미루거나, 범위를 줄이거나, 증거를 더 요구합니다. |
...
| 여러 파일 또는 장기 작업 | 범위, 현재 Change Unit, 증거, 사용자 소유 판단을 보이게 유지합니다. 대화에서 순서를 다시 만들지 않고 영속된 다음 행동에서 재개합니다. |
...
Record와 Detective는 통합 프로필입니다. 호스트 설정, 관찰, 진단 경로를 선택하며
Task 위험 등급이 아닙니다.

각 Task에는 요청한 통제 수준, 유효 통제 수준, 담당 경로가 제공하는 이유가 따로
있습니다. 현재 Task에서는 유효 수준과 프로젝트 소유 정책을 권한 기준으로
취급합니다. Record 프로필에서 더 낮은 수준을, Detective 프로필에서 더 높은 수준을,
에이전트 자신의 요청에서 정책 면제를 추론하지 않습니다. 작업 중 범위, 민감성,
외부 효과, 다른 상향 사실이 드러나면 오래된 대화 맥락에서 계속하거나 수준을 낮추지
말고 반환된 다음 행동을 따릅니다.
...
최초 릴리스의 CLI inbox 전달 경계는
[Agent Connection 참조](../reference/agent-connection.md#first-release-surface)에 있고,
...
준비를 가리키는 다음 행동이 반환되면 호환되는 현재 쓰기 티켓을 발급받거나 재사용한
뒤 다음을 보여 줍니다.
...
채널을 거쳐야 합니다. 보고된 신뢰도를 보존합니다. 의심되는 변경은 확인이 필요하며
에이전트가 임의로 확인 상태로 올려서는 안 됩니다. 불확실한 모든 관찰을 차단 사유로
바꾸지 말고 담당 경로가 제공한 닫기 상태와 다음 행동을 보고합니다.
...
사용자가 닫기가 막혔는지 묻거나 현재 닫기 사실을 권한 상태에서 새로고쳐야 하면 읽기
전용 닫기 상태 점검을 사용합니다. 현재 응답이 이미 최신 닫기 전달 지점을 제공한다면
완료가 가까워졌다는 고정 절차 때문에 별도 점검을 넣지 않습니다. Task 상태는
지원되는 닫기 경로로만 바꿉니다. 산문, 테스트만 있는 상태, 넓은 수락 문구, 생성된
보기, 오래된 상태만으로 닫지 않습니다. 최종 수락과 잔여 위험 수락은 빠진 필수
증거를 대신하지 않습니다.

호스트 Stop은 세션을 끝내며 Task를 닫지 않습니다. 다음 행위자가 사용자이거나 다른
닫기 상태 차단 사유가 남아 있어도 세션은 끝날 수 있습니다. 이때 차단 사유를
공개하고 완료를 주장하지 않습니다. 권한 상태를 새로고칠 수 없으면 마지막 결과를
만들어 내지 말고 Volicord 상태를 검증하지 못했다고 공개합니다.
