...
추적이 필요한 작업에는 아래 순서를 사용합니다.

1. 요청을 보이는 목표, 현재 적용 범위, 범위 밖 항목, 다음 안전한 행동으로
   정리합니다.
2. 사용자에게 묻기 전에 사용할 수 있는 파일, 문서, 테스트, Volicord 상태를
   확인합니다.
3. 다음 안전한 행동을 바꾸는 사용자 소유 판단만 묻습니다.
4. 제품 파일 쓰기나 민감 동작 전에 범위를 새로고침합니다.
5. 행동 뒤에는 의미 있는 실행과 증거를 기록합니다.
6. 가장 중요한 차단 사유, 확인한 것, 빠진 것, 다음 안전한 행동 하나를 보고합니다.
7. 닫기 전에는 증거, 최종 수락, 잔여 위험, 남은 차단 사유를 분리합니다.

조언이나 작은 변경에는 이 순서를 가볍게 적용합니다. 작업이 모호하거나, 여러 파일에
걸치거나, 공개 인터페이스를 바꾸거나, 보안·개인정보 위험을 만들거나, 사용자 소유
판단에 달려 있으면 절차의 무게를 높입니다.
...
준비 경로로 쓰기 티켓을 요청한 뒤 다음을 보여 줍니다.
...
지원되는 실행 또는 관찰 경로로 주장별 증거를 기록합니다. 증거 첨부는 그 기록의
...
사용자가 닫기가 막혔는지만 물으면 읽기 전용 닫기 상태 점검을 사용합니다. 작업 상태는
