...
| 탐지 프로필(`detective`) | 선택한 호스트, 플랫폼, 저장소가 담당 문서가 정의한 모든 훅 및 감시기 설정 전제 조건을 충족할 때 | 미기록 변경 신호를 포함해 설정된 호스트 훅 및 감시기 관찰 경로 |

이 경로를 설정하거나 활성화하는 것만으로 현재 기능 지원이 성립하지는 않습니다.
`states.host_feature_support`를 확인해야 하며, 현재 기능 지원을 주장할 수 있는 상태는
`verified`뿐입니다.
...
Volicord가 관리하지 않는 호스트에는 먼저 허용되는 `HOST` 값의 Agent Connection을 만들고
활성화합니다. 그다음 외부 호스트의 자체 설정에서 아래 프로세스를 시작하게 합니다.

```text
volicord mcp --stdio --connection <connection_id> [--project <project_id>]
```

외부 설정은 사용자 관리 상태로 남고, 그 결과로 시작한 프로세스는 MCP 시작 검증을 여전히
통과해야 합니다. Volicord는 임의의 호스트가 설정을 불러오거나 승인했다고 주장하지
않습니다. 정확한 프로세스와 프로젝트 선택 동작은
