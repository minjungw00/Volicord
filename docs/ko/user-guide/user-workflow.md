...
Core가 User 소유 recovery를 지정하면 MCP는 그 정확한 action key를 보존하지만 Agent action
form을 제공하거나 시도한 Agent action으로 해결할 수 있다고 주장하지 않습니다. User
Channel만이 계속 권한 경로입니다.
...
workflow 호출이 Core에 도달하기 전에 거부되면 유용한 보고는 메서드가 커밋되지
않았다는 사실, checkpoint 또는 UserAction 생성 여부, Product Repository 파일 변경
여부, 확인할 수 있는 현재 Task 단계와 상태 버전, 정확한 재시도 행동도 알려 줍니다.
인자 오류만으로 Task 복구가 필요하다고 판단하지 않습니다.
