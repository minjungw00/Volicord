...
Reference 문서는 나중에 이런 기록과 경로에 Task, Change Unit, Decision Packet, Approval, Write Authorization, Evidence Manifest, Verification, Manual QA, Acceptance, Residual Risk, Projection, Reconcile 같은 정확한 이름을 붙입니다. 제품 가치를 이해하는 데는 먼저 이 이름들을 외울 필요가 없습니다.
