...
엄격한 schema를 복사하지 않고도 좋은 Decision Packet prompt를 알아보고 작성할 수 있도록 실용 예시를 사용합니다.

읽고 나면 초점을 좁힌 사용자 판단을 요청하고, 현실적인 선택지를 비교하고, 경로를 추천하고, 불확실성을 이름 붙이고, 사용자가 미룰 때 어떤 일이 생기는지 설명하고, 필요할 때 관련 risk나 evidence를 연결할 수 있어야 합니다.
...
Agent가 혼자 결정하면 안 되는 product, UX, architecture, security, QA, verification, acceptance, residual-risk, scope/autonomy 판단 때문에 작업이 막혔을 때 읽습니다.
...
Decision Packet은 빈 허가서가 아니라 판단을 돕는 자료처럼 보여야 합니다. 실제 사용자 소유 선택을 이름 붙이고, 선택지와 장단점을 보여주고, 경로를 추천하고, 불확실성을 말하고, 미루면 어떤 일이 생기는지 설명하며, 관련 evidence나 residual risk를 연결합니다.
...
Why now: Login flow의 final UI wiring, copy test, Manual QA 전에 failure-feedback pattern 하나가 필요합니다.
Options:
...
Deferral consequence: API error mapping과 state plumbing은 계속할 수 있지만 final UI behavior, copy, screenshot, Manual QA는 기다려야 합니다.
Related risk or evidence: account-enumeration copy risk, accessibility evidence, screenshot 또는 browser-smoke refs, implementation 뒤 Manual QA refs.
```

이 예시가 좋은 이유는 사용자에게 "login change를 승인할까요?"가 아니라 UX 선택을 묻기 때문입니다. 또한 사용자가 결정하는 동안 무엇을 계속할 수 있는지도 말합니다.

정확한 Decision Packet behavior는 [Decision Packet](../reference/kernel.md#decision-packet)과 [Decision Gate](../reference/kernel.md#decision-gate)가 담당합니다. Manual QA behavior는 [QA Gate](../reference/kernel.md#qa-gate)가 담당합니다.
...
정확한 sensitive-action Approval과 사용자 소유 architecture judgment 경계는 [Approval](../reference/kernel.md#approval), [Decision Packet](../reference/kernel.md#decision-packet), [Sensitive Categories](../reference/mcp-api-and-schemas.md#sensitive-categories)가 담당합니다.
...
Related risk or evidence: privacy exposure, artifact redaction notes, log sample evidence, retention/audit refs, debugging value가 줄어드는 경우의 residual risk.
```
...
Why now: Responsive login flow의 required Manual QA가 passed가 아니어서 close가 막혔습니다.
Options:
- 지금 Manual QA를 수행합니다.
- 이번 close에 대한 Manual QA waiver를 기록합니다. Close-relevant residual risk가 남아 있다면 residual-risk acceptance도 별도 owner path로 route하거나 기록해야 합니다.
- Task를 열어 두고 close 전에 QA를 schedule합니다.
Recommendation: user-facing login workflow라면 Manual QA를 수행합니다. Environment가 unavailable이고 change가 low risk 또는 time-bound일 때만 waive합니다.
Uncertainty: small-screen layout, keyboard flow, screen-reader interpretation, visual polish를 사람이 아직 inspect하지 않았습니다.
Deferral consequence: implementation은 complete 상태로 남을 수 있지만, Manual QA가 passed되거나 valid QA waiver와 필요한 residual-risk acceptance path가 기록될 때까지 close는 blocked 상태여야 합니다.
Related risk or evidence: existing test logs, available screenshot, skipped viewport list, Manual QA requirement, residual-risk follow-up.
```

이 예시가 좋은 이유는 skipped inspection을 이름 붙이기 때문입니다. QA waiver는 QA가 통과했다는 증거가 아니며, 필요한 residual-risk acceptance path가 함께 기록되지 않는 한 그 자체로 residual risk를 수락하지 않습니다.
...
Detached verification이 required 또는 expected이지만, 사용자가 그것 없이 진행하고 싶어 할 때 사용합니다.
...
Options:
- Fresh bundle 또는 fresh worktree에서 detached verification을 실행합니다.
- Independent verification을 사용할 수 있을 때까지 Task를 열어 둡니다.
- Verification을 waive하고, 남은 risk가 visible이고 accepted일 때만 risk-accepted path로 닫습니다.
Recommendation: billing/export behavior라면 detached verification을 실행합니다. Change blast-radius가 낮고 existing self-check evidence가 강할 때만 waive합니다.
...
Related risk or evidence: self-check run refs, missing Eval ref, affected export formats, residual-risk refs, follow-up verification plan.
```

이 예시가 좋은 이유는 assurance wording을 정직하게 유지하기 때문입니다. Verification waiver는 risk-accepted close path를 풀 수는 있지만 detached verification을 만들지는 않습니다.

정확한 verification과 close behavior는 [Verification Gate](../reference/kernel.md#verification-gate), [Verification Independence Profiles](../reference/kernel.md#verification-independence-profiles), [Residual Risk](../reference/kernel.md#residual-risk), [`close_task`](../reference/kernel.md#close_task)가 담당합니다.

## Residual risk acceptance

Implementation과 evidence 뒤에도 알려진 close-relevant risk가 남아 있고, 사용자가 이번 close에서 그 risk를 받아들일지 결정해야 할 때 사용합니다.
...
Related risk or evidence: passing UTF-8 export tests, missing legacy-encoding test coverage, known limitation note, follow-up ref, visible Residual Risk refs.
```

이 예시가 좋은 이유는 acceptance 전에 남은 limitation을 보이게 하기 때문입니다. 사용자는 결과만 받아들이는 것이 아니라, 이름 붙은 남은 위험을 이번 close에서 받아들일 수 있는지도 결정합니다.

정확한 residual-risk behavior는 [Residual Risk](../reference/kernel.md#residual-risk), [Acceptance Gate](../reference/kernel.md#acceptance-gate), [`harness.record_user_decision`](../reference/mcp-api-and-schemas.md#harnessrecord_user_decision), [`close_task`](../reference/kernel.md#close_task)가 담당합니다.
...
| 필요 | Owner |
|---|---|
| Decision Packet 의미와 gate aggregation | [Decision Packet](../reference/kernel.md#decision-packet), [Decision Gate](../reference/kernel.md#decision-gate) |
...
| Manual QA와 QA waiver impact | [QA Gate](../reference/kernel.md#qa-gate) |
| Final acceptance와 residual-risk visibility | [Acceptance Gate](../reference/kernel.md#acceptance-gate), [Residual Risk](../reference/kernel.md#residual-risk) |
| Close blockers와 close reasons | [`close_task`](../reference/kernel.md#close_task) |

## 좋은 답변 패턴

Decision Packet에 답할 때는 ordinary language로 option을 고르고, 중요하게 생각하는 boundary를 덧붙입니다.

```text
Inline failed-login feedback을 선택합니다. Message는 generic하게 유지하고, modal은 추가하지 말고, account recovery는 이 Task 범위 밖으로 둡니다.
```

이런 답변은 named choice를 해결하면서 다른 모든 권한을 부여하는 것처럼 보이지 않기 때문에 유용합니다. Agent에게는 여전히 Write Authorization, evidence, QA, verification, acceptance, residual-risk acceptance, close를 위한 일반 owner path가 필요합니다.
