...
Prompt나 status에서 "approved" 또는 "승인"이라는 말을 쓸 때는 실제 권한이나 기록되는 판단을 이름 붙입니다. Sensitive-action Approval, scope confirmation, Decision Packet resolution, residual-risk acceptance, final acceptance, Write Authorization status를 구분해서 말하고, "승인"을 모든 허가와 판단을 뭉뚱그리는 catch-all label로 쓰면 안 됩니다.
