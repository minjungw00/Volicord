...
"approved", "승인", "go ahead", "진행해" 같은 말은 underlying choice가 제품 장단점, architecture 방향, QA waiver, verification risk, final acceptance, residual-risk acceptance라면 충분하지 않습니다. Prompt는 decision route, 사용자가 결정하는 것, 결정하지 않는 것, evidence 또는 risk refs, close나 write 영향을 이름 붙여야 합니다.
