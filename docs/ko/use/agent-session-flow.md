...
| 판단 | 계속하기 전에 사용자가 판단해야 하는 것. 각 대기 항목을 유형별로 나눕니다. 민감 동작 permission은 그 경로가 pending일 때만 여기에 포함합니다. | 판단 요청, Decision Packet, minimum MVP-1 민감 동작 permission을 위한 Approval 형태 Decision Packet, later Approval profile이 active일 때만 Approval ref, 작업 수락 Decision Packet, 잔여 위험. |
...
| 마무리 또는 닫기 | 닫기를 시도하거나 작업 수락을 요청하기 전에 남은 것. | active일 때 Approval 형태 Decision Packet 또는 Approval, 작업 수락 judgment / Decision Packet, Residual Risk, 닫기 막힘. |
...
- 요구사항 요약(`Discovery Brief` 내부 라벨): 목표, 사용자 가치, 범위, 비목표, 수용 기준, 저장소/문서/테스트/현재 하네스 상태/수락된 결정/현재 작업 아티팩트에서 이미 확인한 확인 가능한 사실, 빠진 정보, 사용자만 할 수 있는 판단, 제품/UX 판단 후보, 기술 구조 판단 후보, 보안/개인정보 판단 후보, QA와 검증 기대 수준, 열린 가정, 남은 불확실성, 안전한 다음 작업 후보 또는 작업 분할 제안을 담은 간결한 요약.
...
요구사항 구체화 밖에서는 다음 안전한 행동을 바꾸는 질문만 합니다. 구체화 중에는 목표, 사용자 가치, 범위, 비목표, 수용 기준, 제품/UX 동작, 기술 구조, 보안/개인정보 자세, QA 또는 검증 기대 수준, 안전한 다음 작업 후보, 작업 분할, 사용자 소유 판단, 숨은 가정을 구체화하는 목적이 분명한 질문을 묻습니다. 긴 질문지를 한꺼번에 던지지 말고 판단 영역별로 묶어 묻고, 불확실성을 명시합니다. 참고 질문은 사용자를 계속 방해하지 말고 따로 남겨 둡니다. 긴 양식보다 추천이 딸린 가장 큰 막히는 질문 하나가 낫습니다.
...
한 번에 하나의 막히는 질문을 묻는다는 말이 요구사항 구체화도 한 번이면 끝난다는 뜻은 아닙니다. 요청이 넓거나 설계 판단이 크면 목표, 사용자 가치, 범위, 비목표, 수용 기준, 영향받는 제품 영역, 사용자 화면이나 흐름, 모듈, 인터페이스, 민감 카테고리(sensitive categories), 사용자가 소유하는 제품 또는 중요한 기술 장단점 판단, 보안/개인정보 선택, 검증 또는 수동 QA 기대 수준, 알려진 제품·구현·검증·QA·후속 위험이 안전한 다음 작업을 제안할 수 있을 만큼 잡힐 때까지 짧은 확인을 여러 차례 이어갈 수 있습니다. 구체화 과정에서는 먼저 확인 가능한 사실을 살피고, 다음 막히는 질문을 묻고, 참고 질문을 남겨 두며, 다음 안전한 행동으로 이동합니다. 긴 양식으로 한꺼번에 묻지 않습니다.
