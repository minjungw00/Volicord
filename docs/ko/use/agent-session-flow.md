...
아주 작은 변경에서는 사용자에게 보이는 예산을 더 줄일 수 있습니다. Trivial scope, changed path 또는 file 변경 없음 결과, self-check 정도면 됩니다. 이 작은 표시가 권한 우회는 아닙니다. 내부적으로는 `direct` 아래의 tiny profile도 active scope, 제품 파일 쓰기에 적용되는 compatible `prepare_write`, 사용자 소유 판단, sensitive-action Approval, security와 privacy boundary, 잔여 위험 표시, close rule을 지켜야 합니다.
