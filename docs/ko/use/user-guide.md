...
에이전트가 승인을 요청한다면 prompt는 실제 path를 이름 붙여야 합니다. 사용자는 sensitive action을 approve하는 것일 수도 있고, scope를 확인하는 것일 수도 있으며, Decision Packet을 resolve하거나, residual risk를 accept하거나, final result를 accept하거나, Write Authorization status를 확인하는 것일 수도 있습니다. "승인"은 포괄 label이나 포괄 위임으로 쓰면 안 됩니다.
