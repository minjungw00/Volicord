...
기본 `guarded` 또는 `managed` init은 모든 필수 호스트 lifecycle hook phase를 설치하고
검증할 수 있어야 합니다. 선택한 Codex 또는 Claude Code 어댑터가 모든 필수 phase에
대해 신뢰할 수 있는 프로젝트 로컬 hook 스키마나 경로를 알지 못하면, init은
`AGENTS.md`나 `.volicord/policy.json`을 집행으로 취급하지 않고 실패합니다.
`--allow-degraded`는 degraded 설정 파일을 명시적으로 원하고 필수 hook phase가 누락된
것으로 보고됨을 이해할 때만 사용합니다.
