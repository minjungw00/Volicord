...
- 담당해야 하는 정보: 범위, API 메서드, 스키마, 오류 동작, 저장소, 런타임 경계,
  ...
  설명자로 이어지는 정확한 의미 기반 계약 ID를 사용합니다. 각 JSON 또는 YAML
  인스턴스는 정확한 설명자 형태 하나를 선택하며, 메서드 경로는 요청과 응답
  스키마를 합치지 않고 각 계약 선택지를 제공합니다. 독자를 위한 형태 표기는
  `schema` 펜스를 사용합니다. 반복되는 요청 및 응답 필드, 필수 여부, Null 허용
  여부, 형식, CLI 문법 목록은 그 담당 원본에서 생성합니다. 메서드 성공 형태는
  공유 또는 메서드별 거절 형태와 구분합니다. 작성자는 생성 영역 주변의 의미와
  정책 산문을 담당합니다. 참조 색인은 사람이 읽는 참조 탐색을 담당합니다.
