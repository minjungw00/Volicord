...
- 담당해야 하는 정보: 튜토리얼 수준의 전제 조건, 릴리스 바이너리 설치 흐름, 개발용
  소스 빌드 경로, 온보딩 수준의 안내형 설정 프롬프트와 `action_required` 안내,
...
- 주요 목표: 준비 상태를 확인한 실행 파일에서 관리 에이전트 호스트 설정 경로
  하나와 문서화된 설정 점검 지점까지 이어지는 가장 짧은 실제 경로를 제공합니다.
...
- 더 깊은 문서로 연결하는 방식: 실행 파일 준비는 [설치](../user-guide/installation.md),
  더 폭넓은 호스트 운영은 [에이전트 호스트 설정](../user-guide/agent-host-setup.md), 멈춘
...
- 주요 목표: 관리되는 Codex record 설정 경로를 준비하고, 검증하고, 운영하고,
  제거하는 방법을 설명합니다.
...
- 담당해야 하는 정보: 권한을 의식한 작업 순서, 가이드 수준에서 Volicord API를
  사용할 때, 증거 관행, 사용자 판단 요청 흐름, 쓰기 티켓 경계, 에이전트를 위한
  닫기 준비 상태 기대입니다.
...
- 담당해야 하는 정보: 범위, API 메서드, 스키마, 오류 동작, 저장소, 런타임 경계,
  ...
  품질 의미의 정확한 계약입니다. 계약을 담는 문서는 기계 판독 가능한 담당
  설명자로 이어지는 정확한 의미 기반 계약 ID를 사용합니다. `DocIndex`는 각 유지
  문서를 생성기와 검증기가 사용하는 정규화된 결합 집합 하나로 해석합니다. 일반
  메서드 문서는 담당 요청과 응답 쌍 하나를 해석하고, 여러 메서드를 담당하거나
  일반 규칙과 다른 문서는 완전한 담당 쌍을 명시적으로 선언합니다. 각 JSON 또는
  YAML 인스턴스는 스키마를 합치지 않고 정확한 설명자 형태 하나를 선택합니다.
  독자를 위한 형태 표기는 `schema` 펜스를 사용합니다. 반복되는 요청 및 응답 필드,
  필수 여부, Null 허용 여부, 형식, CLI 문법 목록은 그 담당 원본에서 생성합니다.
  메서드 성공 형태는 공유 또는 메서드별 거절 형태와 구분합니다. 작성자는 생성
  영역 주변의 의미와 정책 산문을 담당합니다. 참조 색인은 사람이 읽는 참조 탐색을
  담당합니다.
...
- 담당해야 하는 정보: 오래 유지될 크레이트와 모듈 역할, 요청 흐름, 아키텍처 설계
  참조, 소스 학습 경로, 설계 패턴, 저장소와 트랜잭션 아키텍처, 테스트 전략, 구현
  변경 작업 흐름입니다. 현재 아키텍처 설계 참조는
  [`architecture-guide/design/`](../architecture-guide/design/README.md)에 있으며
  [문서 정책](documentation-policy.md#architecture-design-references)이 정의한 현재 상태 절
  스키마를 사용합니다.
- 담당하지 않아야 하는 정보: 제품 동작 계약, API 스키마 의미, 저장소 DDL 계약,
  저장 효과 계약, 보안 보장, 사용자 판단 권한 의미, 줄 번호에 기대는 도우미 목록,
  결정 연대기, 검토 이력, 마이그레이션 설명, 이전과 이후 비교, 거부한 대안, 릴리스
  권고, 이전 경로 보존입니다.
- 허용되는 그림: 안정적인 코드 구조를 가르치는 아키텍처 지도, 모듈 책임 그림,
  생명주기 그림, 요청 순서, 구현 설계 그림입니다.
...
문서 계열: [제품 및 유지보수 헌장](product-maintenance-charter.md),
[문서 정책](documentation-policy.md), [번역 정책](translation-policy.md),
...
- 주요 목표: 작성자, 번역자, 검토자, 에이전트, 구현자가 Volicord의 제품 원칙,
  서비스 계획, 코드 지침, 문서 집합, 테스트, 브랜드 언어, 번역 정책을 정확하고,
  범위에 맞고, 찾을 수 있고, 의미 기준으로 한영 일치하도록 유지하게 돕습니다.
- 의도한 독자: 문서 유지보수자, 번역자, 검토자, 에이전트, 문서와 가까운 작업을
  하는 구현자입니다.
- 담당해야 하는 정보: 제품 및 유지보수 헌장 원칙, 문서 거버넌스, 메타데이터
  사용, 용어 통제, 번역 기대, 그림 범주와 캡션 규칙, 검증 점검, 브랜드 표현,
  코드 지침 경계, 테스트 철학, 문서 헌장, 저장소 작업 규칙과의 상호작용입니다.
