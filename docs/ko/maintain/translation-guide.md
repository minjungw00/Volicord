...
영어 단어가 코드 값과 일반 형용사로 모두 쓰일 수 있으면 먼저 맥락을 판단합니다. `complete`는 `intent=complete`처럼 enum 값이나 식별자일 때만 backtick으로 보존합니다. 일반 산문에서 전체, 전체 평가, 전체 평가 순서를 뜻한다면 자연스러운 한국어로 옮깁니다. 닫기 준비 상태 맥락에서 전체 순서를 말할 때는 전체 닫기 준비 상태 평가 순서를 씁니다.
...
| full evaluation order | 전체 평가 순서, 닫기 준비 상태 맥락에서는 전체 닫기 준비 상태 평가 순서 | `complete` 평가 순서, complete 평가 순서, `complete` 닫기 준비 상태 순서라고 쓰지 않습니다. |
