...
관리 명령의 허용, 결속 검증, 영수증 검증, 릴리스 증거를 구분합니다. 명령이
`host_kind=codex`를 받아들일 수 있고, 설정 경로가 전제 조건을 충족할 수 있으며,
정확한 결속이 성공 영수증을 만들 수 있습니다. 각 사실은 이름 붙은 범위 안에만
머뭅니다. 설정의 존재, 픽스처 통과, 터미널 쪽 MCP 점검은 엄격한 현재 영수증을
대신할 수 없고, 영수증은 정확한 릴리스 아티팩트 증거를 대신할 수 없습니다. 환경과
설정의 적용 가능성은 [시스템 요구사항](../reference/system-requirements.md), 정규 결속과
영수증 의미는
[Agent Connection](../reference/agent-connection.md#host-verification-receipt), 최종 아티팩트
증거와 릴리스 주장은 [호스트 릴리스 증거](../reference/host-release-evidence.md)로 보냅니다.
...
| 정규 Agent Connection 결속과 검증 영수증 | [Agent Connection](../reference/agent-connection.md#host-verification-receipt) |
