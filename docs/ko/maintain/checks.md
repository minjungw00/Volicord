...
Cooperative, detective, preventive, isolated, guard, freeze, careful-mode, sandbox, permission, blocking, tamper-proof, isolation 표현을 봅니다.

Claim이 문서화된 보장 수준과 맞고 preventive 또는 isolated behavior에는 담당 문서와 증명 경로가 있으면 통과입니다. Cooperative나 detective behavior를 증명된 담당 문서 경로 없이 OS 권한, 임의 도구 샌드박스, 변조 방지 저장소, 보편적 도구 실행 전 차단, 보안 격리처럼 설명하면 실패입니다.
