...
| 첫 사용자 작업 루프 | MVP-1 사용자 작업 루프입니다. 평소 말 intake, `harness.status.next_actions`가 있는 status, 집중된 사용자 판단 요청/기록, Core-owned evidence summary, close result/blocker, 작업 수락과 잔여 위험 표시, 명시적으로 수용된 accepted-risk close, compact Core-derived view를 더합니다. | Full Evidence Manifest, detached Eval, full Manual QA matrix, Assurance hardening, operations/export/recover, dashboard, hosted UI, broad connector, automation, detailed report. |
...
| MVP-1 범위 | MVP-1은 사용자에게 보이는 범위, 판단, Core-owned evidence summary, 닫기 막힘, 다음 행동, required work acceptance, 잔여 위험 표시, explicit residual-risk acceptance를 통한 accepted-risk close를 포함하고 이후 프로필은 제외합니다. | [MVP-1 사용자 작업 루프](mvp-user-work-loop.md#mvp-1에-포함되는-것). |
