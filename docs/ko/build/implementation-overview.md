...
- 이전에 open이었던 simplified judgment naming, `request_user_judgment`, MVP-1 next-action output을 `harness.status.next_actions`에 두고 별도 `harness.next`를 later/compatibility material로 남기는 결정, MVP-1 storage minimum, local access error taxonomy, compact status card scope, small direct change evidence requirement, 작업 수락/잔여 위험 minimal records는 이 문서 기준에서 해소되었지만 아직 코드 작성용으로 수락된 것은 아닙니다.
