...
이 단계에서는 원천 기록이 존재할 때 [문서 Projection 참조](../reference/document-projection.md#template-tiers)와 [Template 참조](../reference/templates/README.md)가 소유하는 MVP-required pre-verification renderer를 만듭니다. Eval renderer는 MVP-required로 남지만 Eval 원천 기록이 생기는 MVP-4에서 실행 가능한 렌더링 경로가 완료됩니다.
