...
Docs-maintenance check는 읽기 전용 리뷰 지침이며 runtime conformance나 implementation readiness가 아닙니다. 실행하기 쉬운 drift category와 owner-first resolution은 문서 작성 가이드를 사용하고, docs-maintenance profile reporting boundary는 운영과 Conformance를 사용합니다.
...
문서와 이후 Harness 시스템의 일관성을 유지하기 위한 경로입니다. Maintain 문서는 런타임 동작이 아니라 문서 유지보수를 관리합니다. 영어/한국어 의미 일치 기대사항, owner-boundary review, TODO hygiene, broken-reference check, 중복된 전체 계약을 짧은 요약과 owner link로 바꾸는 방법을 정의합니다.
