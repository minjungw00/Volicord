...
- 사용자 가이드: [사용자 가이드 색인](user-guide/README.md)은 설치, 첫 설정, 작업 흐름 실무, 사용자 판단, 에이전트 호스트 운영, 다중 저장소 설정, 상태, 닫기 해석, 문제 해결을 다룹니다.
...
- 에이전트: [에이전트 가이드](user-guide/agent-workflow.md)를 사용합니다.
...
문서 집합을 유지하거나, 저장소 지침을 검토하거나, 대응 문서를 번역하거나, 담당
경로를 확인할 때 아래 문서를 사용합니다.

- 유지보수 헌장: [제품 및 유지보수 헌장](maintain/product-maintenance-charter.md)
- 문서 규칙: [문서 정책](maintain/documentation-policy.md), [문서 헌장](maintain/document-charters.md), [그림 정책](maintain/diagram-policy.md)
- 번역과 문구: [번역 정책](maintain/translation-policy.md), [브랜드 지침](maintain/brand-guidelines.md)
- 검증: [검증](maintain/validation.md)
- 기계 판독 경로와 용어: [doc-index.yaml](../doc-index.yaml), [용어 지도](../terminology-map.yaml)
