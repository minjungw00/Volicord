...
이 저장소는 문서 재설계 / 피드백 반영 및 문서 검토 단계입니다. 이 페이지는 Harness server/runtime 구현, 생성된 운영 파일, 실행 가능한 fixture 파일, runtime data를 승인하지 않습니다. 첫 runtime batch 계획은 maintainer가 구현 handoff에서 문서를 명시적으로 승인하기 전까지 시작할 수 없습니다. 첫 구현/증명 경로는 Kernel Smoke 먼저, Agency-Hardened MVP 나중, post-MVP automation은 owner 문서가 승격하기 전까지 roadmap에 둡니다.
