...
완전한 guarded 또는 managed 설정은 별도의 `volicord init` 모드입니다. 이 모드는
[관리 CLI](../reference/admin-cli.md)가 정의한 대로 필수 호스트 lifecycle hook에 대한
검증된 지원이나 명시적 degraded opt-in이 필요합니다.
