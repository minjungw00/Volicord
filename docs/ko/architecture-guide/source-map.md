...
| `crates/volicord-core/src/write_ticket/` | 정규 Write Ticket 담당 경계입니다. `planning.rs`는 검증된 비영속 발급 plan을 담당합니다. `semantic.rs`는 planned와 stored 형태의 identity를 구분하면서 두 형태가 공유하는 불변 의미를 제공합니다. `read_model.rs`는 typed ticket, Task, workflow policy, 현재 UserAction resolution, 증거 fact를 취득합니다. `selection.rs`는 순수한 candidate 우선순위와 동률 해소를 담당합니다. `current_validity.rs`는 순수한 effective status, invalidation, authority, approval 평가를 담당합니다. `summary.rs`는 평가된 ticket과 제공된 증거를 adapter-neutral summary로만 변환합니다. `service.rs`는 영속 summary 읽기를 좁게 조율합니다. `policy.rs`는 prepare-write와 attempt-scope 판단을 담당하고, `admission.rs`는 typed 승인 scope 또는 의미 승인 오류를 반환합니다. 공개 응답 구성은 호출 메서드에 남습니다. |
