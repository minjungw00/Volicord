...
Store가 도출한 현재 유효 shaping 권한 graph, 분리된 변경 불가능한 이력, 정확한 stale 폐기
또는 재권한, 명시적 checkpoint succession과 application carry-forward, 결정 owner별 적용,
advisor finalization, Task 전체 shaping 진행은
...
| `crates/volicord-types/src/schema.rs` | 공유 요청, 응답, 저장 레코드 형태와 폐쇄형 상태별 `WorkflowTransitionSubmissionContract`, 한정된 typed form witness, 정규 Advisor observe-only effect contract. |
...
| `crates/volicord-types/src/managed_guidance.rs` | 생성된 managed host guidance, Core workflow contract, MCP action form, MCP semantic schema를 위한 폐쇄형 semantic fact와 서로 다른 canonical digest입니다. 현재 transition과 variant admission, 정확한 메서드-variant form과 고정 권한 인자, 타입 소유 schema 의미, Store가 도출한 현재 권한, 행동할 수 없는 변경 불가능한 이력, 정확한 stale 폐기 또는 새 identity 재권한, implementation 보존 거부, 현재 checkpoint/UserAction 보존, 명시적 호환 application carry-forward, 결정 담당, User Channel resolution, tagged rejection/recovery 표면화, close-review 경계를 포함합니다. 네 digest는 모두 프로젝트 integration revision에 참여합니다. |
...
| `crates/volicord-types/src/integration_revision.rs` | 서로 다른 Store-manifest, managed-guidance, workflow-contract, action-form-contract, MCP-semantic-schema, scalar-contract digest 입력을 포함하는 typed Connection/프로젝트 integration revision basis와 파생. |
...
| `crates/volicord-store/src/core_pipeline/shaping.rs` | Store가 도출한 현재 유효 shaping 권한 graph, 분리된 변경 불가능한 application 및 재권한 이력 읽기, 원자적인 shaping checkpoint/gap/link/UserAction 및 정확한 stale 권한 mutation, 영속 invariant 검증. |
...
| 경로 | 책임 |
|---|---|
| `crates/volicord-core/src/pipeline.rs` | 분리된 읽기 전용 경로 및 승인 context 기반 `CoreService` 구성, typed Core/Store Runtime Home 권한 검사, 공통 사전 점검, replay, plan 선택, 응답, commit 조율, 정확한 typed no-commit transition-submission mode를 담당합니다. 이 mode는 읽기 전용 현재 Store를 대상으로 메서드 admission, 정규화, policy, semantic validation, mutation planning, effect 검증, 실제 결과 검증을 재사용하고 transaction과 모든 영속 또는 repository 효과 전에 멈춥니다. 또한 Store 오류 세부사항 projection과 플랫폼 소유 Product Repository 관찰 실패의 중립 typed 운영 projection을 담당합니다. |
| `crates/volicord-core/src/operation_plan.rs` | Task와 선택적 Change Unit 식별자, 순서가 정해진 Store mutation, event payload로 구성된 메서드 중립 commit 연산 입력. 공개 메서드 결과 또는 응답 타입을 포함하지 않습니다. |
| `crates/volicord-core/src/method_execution.rs` | 메서드 공통 요청 준비, mutation policy 선택, replay decoding, 저장 scalar 변환, generic method-plan 오류 carrier를 담당합니다. Aggregate policy나 source별 오류 매핑은 담당하지 않습니다. |
| `crates/volicord-core/src/method_rejection.rs` | 공유 validation 및 rejection 응답 구성과 메서드 중립 dry-run summary를 담당합니다. |
| `crates/volicord-core/src/error_boundary/` | Store, UserAction, 닫기 준비 상태, artifact policy, Product Repository path의 source 오류 모델을 공개 메서드 계획 오류로 바꾸는 집중 경계 매핑. |
| `crates/volicord-core/src/product_path.rs` | 호출자가 제공한 경로의 공유 어휘 parsing과 플랫폼 소유 활성 Product Repository 관찰을 조율합니다. Store가 반환한 경로는 typed 값으로 도착하며 이 모듈을 거치지 않습니다. |
...
| `crates/volicord-core/src/workflow_projection.rs` | 현재 유효 graph에서 엄격한 정규화 `WorkflowSnapshot`을 구성하고 workflow kind, next actor, typed blocker, required ref, 별도 close readiness, 결정적 고정 좌표 `WorkflowTransitionCatalog`를 유일하게 선택하는 순수 `WorkflowMachine`. |
| `crates/volicord-core/src/methods/tests/workflow_state_space.rs` | Catalog 고유성, required member 무결성, transition 소유 admission identity, nonterminal liveness, 정확한 typed recovery membership을 검사하는 결정적 순수 reachable-state 탐색. 실제 form-to-Core no-commit 실행 가능성 coverage는 Store 기반 MCP journey test가 담당합니다. |
...
| `crates/volicord-core/src/write_ticket/` | 정규 Write Ticket 담당 경계입니다. `planning.rs`는 집중된 `PrepareWriteInput` fact를 평가하고 모든 active 재사용 후보를 stale mutation fact와 구분해 분류한 뒤 공통 fact, mutation fact, 발급·재사용·ticket 없음 가운데 하나인 폐쇄형 `PrepareWriteTicketPlan` 분기를 반환합니다. 발급 분기는 검증된 `PlannedWriteTicket`으로 구체화되고 `MaterializedPrepareWriteTicket`은 발급됨·재사용됨·없음의 identity와 효과를 폐쇄형으로 유지합니다. Planned ticket과 stored ticket은 불변 `WriteTicketPathScope` 하나를 노출하고, ticket 없음만 별도 판단 경로를 소유합니다. `semantic.rs`는 planned와 stored 형태가 공유하는 불변 의미만 제공합니다. `read_model.rs`는 typed stored ticket, Task, workflow policy, 관찰 시각, UserAction resolution, 증거 fact를 취득합니다. 원시 UserAction 권한 값은 `approval.rs`에 직접 전달하고 `WriteTicketCurrentFacts`에 보존하지 않습니다. `approval.rs`는 현재 민감 승인 집합을 비공개로 구성하고 typed 승인 근거 또는 `WriteTicketApprovalAssessment`를 반환합니다. `current_validity.rs`는 terminal stored 상태를 사전 평가하고 비승인 현재 fact와 이 평가를 사용해 active candidate를 reusable 또는 invalidated 상태로 평가하며 stored identity를 필수로 유지합니다. `selection.rs`는 과거/표시용 stored 평가 우선순위와 권한 효력이 있는 호환성 cardinality를 서로 다른 순수 policy로 담당합니다. 후자는 `None`, `One`, 또는 정렬된 진단 identity를 가진 불변 조건 보존 `Ambiguous`를 반환합니다. `summary.rs`는 planned projection과 stored projection을 구분합니다. `service.rs`는 현재 fact를 읽기 전에 terminal record와 active record를 나누고, 원시 승인 권한은 `approval.rs`에만 전달하며, 표시할 stored 평가를 선택한 뒤 그 선택의 증거를 읽습니다. `policy.rs`는 typed prepare-write 및 attempt-scope 판단을 담당합니다. `admission.rs`는 `ReusableStoredWriteTicket`과 일치하는 정확한 attempt 호환성 증명을 결합해 `AdmissibleStoredWriteTicket` 또는 admission 오류를 반환합니다. 공개 요청 metadata와 응답 구성은 호출 메서드에 남습니다. |
| `crates/volicord-core/src/methods/` | 공개 메서드 진입점과 요청별 조율을 담당합니다. `record_shaping.rs`는 내부 shaping 검증 helper만 공유하면서 서로 분리된 `record_shaping_checkpoint`와 `finalize_advice` 진입점 및 plan wrapper를 제공합니다. Checkpoint plan은 명시적 succession, 정확한 stale 폐기 또는 새 요청 재인가, live-decision 보존을 담당하고, finalization plan은 advisor 결정 적용과 닫기 근거 생성을 담당합니다. `update_scope.rs`는 정확한 scope 소유 결정만 적용하고 implementation 단계 권한 무효화를 mutation 전에 거부합니다. `advance_task.rs`는 Task 전체 현재 권한을 검증한 뒤 advance 소유 결정 적용과 명시적 phase 전환을 원자적으로 처리합니다. 프로덕션 모듈은 공유 책임의 명시적 담당 모듈을 import하고, `methods/mod.rs`는 모듈 wiring만 제공하며 메서드별 plan wrapper는 해당 메서드 모듈에 둡니다. |
...
| `crates/volicord-core/src/recording/plan.rs` | Record Run 정책 서비스 조율, typed `RecordRunMutationPlan` 조립, 최종 `RecordRunEffect`와 `RecordRunResultFacts` projection. 최종 Store plan 변환 전까지 도메인 mutation을 구분합니다. |
| `crates/volicord-core/src/recording/state.rs` | 투영된 연산 후 `StateSummary` 의미 fact의 Store 기반 취득. |
| `crates/volicord-core/src/recording/tests/` | 책임별 Record Run context, 캡처 권한, 증거, artifact, typed mutation plan, 의미 오류, 결과 fact coverage. |
...
| `crates/volicord-core/src/close_readiness/evidence.rs` | 집중 증거 사실 및 순수 정책 담당 모듈을 통한 닫기 증거와 아티팩트 가용성 평가. |
| `crates/volicord-core/src/close_readiness/acceptance.rs` | 대기 중인 닫기 권한, 취소, 민감 작업 승인, 최종 수락, 잔여 위험 수락 평가. |
| `crates/volicord-core/src/close_readiness/policy.rs` | Store와 독립적인 유효 control 해석 및 typed 준비 상태 평가를 닫기 상태로 만드는 순수한 순서 결합. |
| `crates/volicord-core/src/close_readiness/blockers.rs` | 정규 typed 닫기 차단 사유 구성, Write Ticket 차단 사유 projection, 여러 차단 사유에 걸친 action 정규화. |
...
| 경로 | 책임 |
|---|---|
| `crates/volicord-mcp-wire/src/methods.rs` | 정확한 MCP 인자, 구조화 결과, workflow action form, 현재 권한 인자 맥락, retry contract, 한정된 workflow-contract 실패 action·stage 진단, 운영 오류, 간결한 mutation, UserAction projection, 직렬화, 생성 요청/결과 schema 소유권. |
| `crates/volicord-mcp-wire/src/action_form.rs` | 폐쇄형 transition submission-contract branch에서 MCP 요청 경로로 가는 정규 projection, 정확한 제출 variant selector, 필수·선택 Agent 경로와 고정 경로 소유권, descriptor 무결성 검사. |
...
| `crates/volicord-mcp/src/adapter.rs` | 유지되는 연산 전 routing identity, 유효하지 않은 인자를 위한 한도 있는 읽기 전용 현재 권한 bootstrap, mutation Core 진입 전 정확한 메서드·semantic variant·action-form-ref admission, 활성 mutation-context 상관관계, context에 결합된 Core 호출 API를 담당합니다. Catalog를 게시할 때는 모든 완전한 witness를 정확한 메서드 adapter로 투영하고 읽기 전용 현재 권한에서 검증된 invocation을 파생한 뒤 대응하는 Core no-commit planner를 호출합니다. 또한 Core 밖의 managed in-chat begin/probe/get integration-verification를 조율합니다. |
| `crates/volicord-mcp/src/constants.rs` | 사용자 수준 verification 요청과 현재 managed workflow의 메서드 그룹·variant catalog admission, 정확한 메서드-variant form과 고정 권한 인자, 타입 소유 schema와 retry contract, 정직한 pre-Core 거부 보고, Store 유래 현재 권한, 변경 불가능한 이력 제외, 정확한 stale 복구, 새 UserAction identity, 명시적 shaping과 advance, implementation 보존 거부, rejection/recovery 표면화, close review, stop 규칙, unavailable 경계, 선택적 active diagnostics를 설명하는 MCP initialize instruction. |
