...
| `crates/volicord-mcp` | 정규 관리 launch 계약, 시작 검증, 도구 목록, `tools/call` 디코딩과 디스패치, 관리형 표준 입출력 프레이밍, Core 호출, 프로토콜 레지스트리의 서버 선호 리비전 사용을 위한 MCP 어댑터 라이브러리. |
...
| MCP 프로토콜 프로필 경계 | `volicord-mcp-protocol`은 폐쇄형 리비전 타입 집합, 검토된 프로덕션 프로필, 메시지·도구·스키마 기능 선언, 명시적 순회 순서, 서버 선호 리비전을 담당합니다. MCP 어댑터는 이 경계를 사용하며 호스트 호환성 관찰은 서버 선호값을 담당하지 않습니다. | [소스 지도](source-map.md), [테스트 전략](testing-strategy.md), [MCP 전송](../reference/mcp-transport.md). |
