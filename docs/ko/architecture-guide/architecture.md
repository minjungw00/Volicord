...
| Codex 릴리스 검증 | 공유 시나리오는 서로 독립적인 Linux, macOS, native Windows, WSL2 셀에서 실행됩니다. 각 셀은 정확한 Codex 아티팩트와 capability 집합에 결속되며 실행할 수 없는 플랫폼은 다른 셀에서 추론하지 않고 미실행으로 남습니다. 운영 crate는 테스트 패키지 아티팩트를 런타임 신뢰 입력으로 사용하지 않습니다. | [호스트 릴리스 증거](../reference/host-release-evidence.md), [테스트 전략](testing-strategy.md), [검증](../maintain/validation.md). |
