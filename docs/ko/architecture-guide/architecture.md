...
| Init setup transaction | `volicord-cli`는 Runtime Home, Store, Codex 구성, repository 관리 파일, activation을 아우르는 읽기 전용 typed plan 하나를 만듭니다. Prepare 단계는 snapshot을 검증하고 같은 directory에 파일을 staging하며 `volicord-store`는 복구 entry를 준비하고 checkpoint합니다. Setup 상태는 소유한 publication과 관찰한 동시 승자를 구분합니다. 실패하면 한도가 있는 역순 rollback을 수행하며, Runtime Home 제거에는 소유 guard가 정확한 publication ID, Runtime Home identity, manifest, 경로, schema, installation identity, managed-host 소비 부재를 다시 검증해야 합니다. 이는 여러 파일시스템 전체의 전역 원자성이 아닙니다. | [관리 CLI](../reference/admin-cli.md), [런타임 경계](../reference/runtime-boundaries.md), [Agent Connection](../reference/agent-connection.md), [실패 모델](../reference/failure-model.md). |
