...
| 프로젝트 Store facade와 읽기 | `CoreProjectStore`는 프로젝트 데이터베이스 facade로 유지됩니다. Handle과 open 진입점은 인프라 담당 모듈에 있고, aggregate 모듈은 자신의 column projection, 읽기 SQL, row 및 JSON decoding, inherent facade 메서드, 집중 테스트를 담당합니다. 이에 따라 프로젝트 상태, replay, Task와 수락 데이터, Change Unit, Write Ticket, Run, 증거, artifact, User Action, continuity, reconciliation 관찰, blocker, event, Agent Session, clock 데이터는 `core_pipeline/` 아래에 명시적인 현재 담당 모듈을 가지며 workflow policy record는 `workflow_records.rs`에 남습니다. | [저장소와 트랜잭션](storage-and-transactions.md), [소스 지도](source-map.md), [저장소](../reference/storage.md), [저장소 레코드](../reference/storage-records.md). |
