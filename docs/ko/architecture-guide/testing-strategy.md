...
| 관리 CLI, 호스트 설정, managed host configuration, 등록, `volicord inbox` 명령 | `crates/volicord-cli/src/`와 `crates/volicord-cli/tests/binary_admin.rs`. | CLI의 함께 있는 단위 테스트와 `binary_admin`. | 부트스트랩, registry, 검사, 스키마 초기화, Agent Connection, 프로젝트 멤버십 동작이 바뀌면 Store 테스트, CLI 문서가 바뀌면 docs-check. |
