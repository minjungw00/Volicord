...
공유 capability 우선 평가기 하나를 사용합니다. 공유 타입은 닫힌 식별자, 런타임 probe
사실, 검토된 호스트·버전·클라이언트 증거 좌표, 정규 Codex 버전 문법, 정확한 증거와 현재
준비 상태를 반영하는 단일 기능 지원 상태 우선순위를 담당합니다. CLI 진단, MCP 전달 자격,
릴리스 검증은 호스트 종류 fallback, 버전 gate, 기능 상태 우선순위를 다시 만들지 않고 이
결과를 소비해야 합니다. 알 수 없는 버전은 `unsupported_by_host`가 아니라
`implemented_unverified`입니다. 적용 가능한 런타임 probe 성공은 `verified`, 실패는 degraded
또는 temporarily unavailable 상태가 될 수 있으며 호스트 기능 부재가 입증된 경우에만
`unsupported_by_host`를 사용합니다.
...
조회하기 전에 지원되지 않음이 입증된 local-web 표면을 거부하고, 전달 lease를 발급할
때도 같은 capability 점검을 반복해야 합니다. 정확한 클라이언트 버전은 증거 메타데이터로
남지만 주된 런타임 자격 gate는 아닙니다.
...
- MCP, CLI 진단, 릴리스 검증은 같은 현재 probe와 증거 사실에 서로 다른 capability 상태를
  부여할 수 없습니다.
...
- 각 명령이 독립적으로 지원 상태를 계산하는 방안은 우선순위, 최신성, 하위 역량 집계가
  달라질 수 있어 거부했습니다.
- 검토된 정확한 버전 매트릭스를 주된 런타임 gate로 쓰는 방안은 실제 capability를 probe하기
  전에 더 새로운 호환 호스트를 미지원으로 잘못 표시하므로 거부했습니다. 검토 버전은 회귀
  fixture와 릴리스 증거 좌표로 남습니다.

## 관련 구현 영역

- [`crates/volicord-types/src/host_feature_support.rs`](../../../../crates/volicord-types/src/host_feature_support.rs):
  닫힌 식별자, 검토된 호스트·버전·클라이언트 증거 사실, 정규 파싱, capability probe 사실,
  단일 기능 상태 우선순위.
...
- [`crates/volicord-mcp/src/adapter.rs`](../../../../crates/volicord-mcp/src/adapter.rs):
  같은 capability 결과를 쓰는 최초 및 발급 시점 local-web 전달 자격 점검.
