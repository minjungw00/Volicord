...
위치가 될 수 없습니다.
정확히 보존되는 모든 자유 형식 이력·현재 포인터 좌표는 조회나 변경 전에 UTF-8 바이트
기준 상한을 적용합니다. 일반 식별 정보와 좌표는 1~1,024바이트이고 `client_name`과
`client_version`은 공유 관리 MCP 클라이언트 식별 정보의 1~256바이트 규칙을 재사용합니다.
Store는 상한을 넘은 값을 trim하거나 복구하지 않습니다. 유효하지 않은 입력은 replay
분류 전에 거부하고, 상한을 넘은 영속 상태는 손상으로 취급하여 닫힌 상태에서 실패합니다.
...
호스트 역량 형태를 완성하며 v7 전이를 만들지 않습니다. 이 제약 없이 v6로 표시된
데이터베이스는 호환되지 않는 형태입니다. 값을 trim하거나 다시 쓰거나 legacy fallback으로
받아들이지 않습니다.
