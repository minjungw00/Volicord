...
이전 관리 규칙은 `.codex hooks`를 접두사로 사용했습니다. 이는 호스트가 실행하는 argv가
아니라 설정 위치를 나타냈기 때문에 현재 Codex 규칙 검증기가 훅을 하나도 실행하기 전에
파일을 거부할 수 있었습니다. 이 수정은 생성되는 관리 호스트 설정만 바꿉니다. 공개 API,
저장 레코드, DDL, 저장 프로필, SemVer 변경은 없습니다. 기존 관리 파일은 일반 소유권 및
fingerprint 점검을 거쳐 교체하고 비관리 파일을 임의로 채택하지 않습니다. Volicord는
안정적인 최소 Codex 버전을 정의하지 않으므로 체크인된 parser fixture만으로는 충분하지
않습니다. 릴리스 검증에서는 적용 가능한 실제 Codex parser로 생성 규칙을 불러오고
검사해야 합니다.
...
Codex parser의 load-time 점검을 다뤄야 합니다.
