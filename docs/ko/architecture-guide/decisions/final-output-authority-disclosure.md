...
이 훅은 검증된 receipt를 별도 호스트 UI 표면에 놓을 수 있지만, 완료 주장 판단은
최종 출력 고지와 다른 책임이며 Record profile에서는 사용할 수 없습니다.
...
5. Detective Stop은 별도 소비자로 남습니다. 같은 최신 검증 사실으로 완료 주장 허용
   여부를 판단하지만 호스트 고유 Stop 결과는 세션 종료를 항상 허용합니다. 이 생명주기
   결과가 기준 최종 출력 표시가 되지는 않습니다. Record profile 고지는 차단하지 않습니다.
