...
| 메타데이터 | 값 |
|---|---|
| 목적 | 현재 Rust 워크스페이스의 아키텍처와 구현 구조 설명으로 안내합니다. |
| 독자 | 구현자, 검토자, 소스 코드 학습자입니다. |
| 담당 범위 | 아키텍처 가이드 계층 탐색과 가이드 수준 구현 학습입니다. 정확한 제품 계약은 참조 문서에 남습니다. |

이 문서는 현재 Rust 구현을 이해하려는 구현자를 위한 소스 코드 학습 진입점입니다.
어디서 시작할지, 다음에 무엇을 읽을지, 정확한 제품 계약이 어디에 있는지를
안내합니다.
