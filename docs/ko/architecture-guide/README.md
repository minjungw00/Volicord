...
책임, 요청 흐름, 저장소와 트랜잭션 경계, 설계 패턴, 테스트 전략, 현재 설계 참조,
구현 변경 작업 흐름으로 안내합니다.
...
| 변경 계획 세우기 | [구현 가이드](change-guide.md) -> [테스트 전략](testing-strategy.md) -> [아키텍처 설계](design/README.md) | 어떤 담당 문서와 소스 영역을 확인할지, 어떤 테스트 계층을 사용할지, 구현 경계가 어떻게 구현되는지 알 수 있습니다. |
| 릴리스 무결성 검증 계획 | [테스트 전략](testing-strategy.md) -> [검증](../maintain/validation.md) | 일반 target, 패키지, checksum, workflow, 선택적 운영 smoke 경계를 확인합니다. |
...
재사용 가능한 UserAction 서비스인
[`crates/volicord-user-action-service/src/`](../../../crates/volicord-user-action-service/src/)를
