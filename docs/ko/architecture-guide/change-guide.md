...
| 외부 정확한 최종 아티팩트 호스트 릴리스 검증 | `cargo test -p volicord-release-validation-tests`, 이어서 [호스트 릴리스 증거](../reference/host-release-evidence.md)의 정확한 `host-release-candidate`, `host-release-gate`, 별도 `host-release-audit` 명령 | 바이너리, 셀 증거, 외부 호스트, 요청한 주장 중 빠진 항목은 조용히 생략하지 말고 사용할 수 없음 또는 실패로 보고합니다. |
