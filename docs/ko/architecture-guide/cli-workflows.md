...
흐름입니다. 구성된 내장 어댑터가 최종 출력 이벤트를 보고하면 CLI는 선택된 프로젝트와
...
세션 종료를 항상 허용하면서 별도 완료 주장 판단에 같은 검증 사실을 사용하며 Record
고지는 차단하지 않습니다. 범용, 사용자 관리, 미지원, 비활성, 저하된 호스트 경로는 고정 호스트
...
호스트 설정은 사용자가 관리합니다. CLI는 활성 Agent Connection에 대한 안내를 보고할 수
있지만, 그 결과로 시작한 프로세스는 MCP 시작 검증을 여전히 통과해야 하며 CLI는 임의의
외부 호스트 설정을 쓰지 않습니다.

## Guard 통합 경계

Guard 통합은 생성 파일, 정책 JSON, 호스트 이벤트 명령, 역량 메타데이터, 사실 기반 감사
입력을 계획합니다. 내장 Record와 Detective 설정 경로는 최종 출력 처리기 부분 집합을
공유하며, Detective만 나머지 생명주기 처리기와 프롬프트 캡처
