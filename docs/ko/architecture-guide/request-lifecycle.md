...
candidate만 authority와 approval 평가로 보냅니다. `selection.rs`는 완결된 stored
평가 중 하나를 선택합니다. `summary.rs`는 planned 입력과 stored 입력을 구분하고
선택한 상태를 adapter-neutral summary로만 변환합니다. 이 전체 stored 읽기가 필요한
메서드는 선택 뒤 증거를 읽는 좁은 `service.rs` coordinator를 사용합니다. 기존
ticket은 담당 문서의 모든 좌표가 계속 유효할 때만 재사용할 수 있습니다.
`record_run` 평가에서는 active candidate에 대해 정확한 Task, Change Unit, Git
workspace, observed-change, policy fingerprint, operation fact를 검사하는 동안 현재
유효성 평가가 `ReusableStoredWriteTicket`임을 증명합니다.
`write_ticket/admission.rs`는 이 reusable 상태와 일치하는 정확한 attempt 호환성
증명을 결합해 `AdmissibleStoredWriteTicket`을 반환합니다. 물리 row를 decode하거나
메서드 응답을 구성하지 않습니다. 기록 plan은 이 승인된 ticket만 Run과 같은
commit 안에서 소비합니다.
