...
이 named report kinds는 기본적으로 projections 또는 state-backed records입니다. Artifact store의 evidence files를 refer할 수 있고 export가 snapshots를 포함할 수 있지만, 그렇다고 Markdown report가 canonical evidence file이 되지는 않습니다.
