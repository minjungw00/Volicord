...
이 문서는 기준 범위 저장소 원천 설계의 `state_version`, 멱등성, 이벤트 의미, 잠금, 마이그레이션 의미를 담당합니다. 참조 문서일 뿐이며 마이그레이션을 실행하거나 런타임 잠금을 만들거나 런타임 상태를 만들지 않습니다.
...
활성 첫 스키마에서는 `tasks.state_version`을 생략해야 합니다. 구현이 레거시 또는 프로토타입 `tasks.state_version` 열을 만나더라도 그 값은 무시되는 메타데이터일 뿐입니다.
...
- 실제 DDL, 마이그레이션 메커니즘, 저장소 프로필, 제약 강화 동작은 담당 문서가 별도로 정의해야 합니다.

증가하는 경우:

- 해당 없음. 기준 범위 마이그레이션 실행은 증가 경우를 정의하지 않습니다.
- 마이그레이션에 대한 공개 API `state_version` 증가는 이 문서에서 정의하지 않습니다.
- 수락된 마이그레이션은 자신의 담당 문서에서 버전과 저장소 프로필 동작을 밝혀야 합니다.
...
- [범위 참조](scope.md)와 영향받는 저장소 담당 문서가 지원되는 마이그레이션 경로를 정의하지 않는 한, 마이그레이션 실행은 기준 범위 밖입니다.
