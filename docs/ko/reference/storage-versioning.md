...
- 승격된 마이그레이션은 수락되기 전에 출발 버전, 대상 버전, 저장소 프로필, 담당 문서, 되돌림 또는 복구 기대치를 선언해야 합니다.
- 승격된 마이그레이션은 `registry.sqlite` 또는 `state.sqlite` 하나를 기준으로 트랜잭션 안에서 실행해야 하며, 중단 상태 복구 규칙을 분명히 가져야 합니다.
