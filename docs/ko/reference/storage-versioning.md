...
변경 불가능한 Agent Connection 통합 instance 좌표를 추가하면 현재 Registry schema
identity가 바뀝니다. 바로 이전 schema의 Runtime Home은 현재가 아닌 저장 계약이므로
명시적인 재초기화 안내와 함께 거부합니다. Store는 column을 제자리에서 추가하거나 이전
행의 값을 합성하거나 바로 이전 형태를 함께 읽지 않습니다.
