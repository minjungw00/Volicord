...
| Connection Projects 멤버십 | 같은 Runtime Home 아래의 `connection_projects` 기록입니다. | `volicord init`, `volicord connection add`, `volicord project use`, 연결 제거 흐름이 선택된 저장소 루트에 따라 멤버십을 추가, 검증, 제거할 수 있습니다. | 멤버십은 Agent Connection의 프로젝트 허용 목록을 제어합니다. 모든 Runtime Home 프로젝트를 등록하지 않으며 프로젝트 등록, 프로젝트 상태, Core 기록을 삭제하지 않습니다. |
