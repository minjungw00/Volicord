...
`harness.stage_artifact`는 로컬 아티팩트 유틸리티이지 Core 변경 권한이 아닙니다. 임시 `StagedArtifactHandle`을 만들 수 있지만, 호환되는 Core/API 경로가 이를 소비하기 전까지 기준 증거를 만들거나, gate를 만족하거나, 지속 `ArtifactRef`를 등록하거나, 닫기 준비 상태에 영향을 주지 않습니다.
...
아티팩트 등록은 활성 `harness.stage_artifact` 유틸리티가 만든 담당 문서의 `StagedArtifactHandle`이나 호환되는 기존 `ArtifactRef`만 사용합니다. 새 아티팩트 바이트는 스테이징을 통해서만 현재 MVP에 들어오고, 기존 바이트는 `existing_artifact` 참조를 통해서만 재사용합니다. 원시 로컬 경로, 원시 로그, 임의 로컬 경로 문자열, 복사된 파일, `captured_artifact` 핸들, 원시 캡처 어댑터 출력, 접점 자체 캡처 주장은 현재 MVP의 등록 권한이 아닙니다.
