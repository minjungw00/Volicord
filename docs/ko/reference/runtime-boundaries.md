...
Volicord 구현은 이 저장소가 유지하는 서버 구현 집합입니다. Volicord 전체가 아니며, Core도 아니고, 실행 중인 프로세스 하나도 아니며, Volicord 상태를 위한 로컬 기준 기록도 아닙니다.

## 담당하는 것 / 담당하지 않는 것

| 이 문서가 담당하는 것 | 이 문서가 담당하지 않는 것 |
|---|---|
| 제품/시스템인 Volicord와 저장소가 유지하는 서버 구현 집합인 Volicord 구현의 구분. | 공개 API 동작, 공개 스키마 형태, 메서드별 효과. |
...
Volicord는 제품, 서버 구현, 실행 파일 역할, 기준 기록 개념을 구분합니다.
...
| Volicord 구현 | 이 저장소가 유지하는 서버 구현 집합. 소스 수준에서는 구현 크레이트, `volicord` 관리 CLI, `volicord-mcp` 로컬 MCP 어댑터, 테스트, 문서, 검증 도구, 저장소 설정을 포함합니다. | 모든 가능한 Volicord 제품 표면, Core 자체, `Volicord Runtime Home`, `Product Repository`, 단일 데몬이나 네트워크 서비스로 보면 안 됩니다. |
...
Volicord는 서버 파일, 제품 파일, 런타임 데이터, 외부 호스트 설정을 구분합니다. Volicord 구현 집합 전체를 위한 단일 필수 파일시스템 루트는 없습니다.
...
| `Volicord Runtime Home` | 저장소/런타임 담당 문서가 정의하는 Volicord 소유 기록, 로컬 런타임 메타데이터, 아티팩트 데이터를 위한 런타임 저장 위치. | `Product Repository`, 기본 서버 설치 저장소, 자동 보안 경계, 기본 격리로 보면 안 됩니다. |
...
Volicord 구현은 이 저장소가 유지하는 서버 구현 집합을 뜻합니다. 코드, 문서, 테스트, 검증 도구, 저장소 설정을 담은 체크아웃에는 Volicord 소스 저장소를 씁니다. 배포된 실행 파일과 필요한 런타임 리소스에는 Volicord 설치를 씁니다.
...
현재 로컬 Rust MCP 어댑터는 Volicord 구현 안의 실행 파일 역할인 `volicord-mcp` stdio 프로세스입니다. MCP 호스트는 이를 자식 프로세스로 시작하고, 프로세스 환경으로 설정을 전달하며, stdin/stdout을 통해 줄 단위 JSON-RPC를 주고받습니다. 기준 프로세스는 TCP, HTTP, Unix-domain socket, 또는 그 밖의 네트워크 리스너를 열지 않습니다.
...
- `Volicord Runtime Home`이 기본적으로 서버 설치 저장 범위라는 주장.
