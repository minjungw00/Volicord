...
MCP 호스트 설정은 외부 MCP 호스트가 소유합니다. [관리 CLI](admin-cli.md)가 그 동작을 정의할 때 Volicord 관리 명령은 지원되는 호스트 설정을 직접 설치할 수 있습니다. 사용자 관리 외부 호스트 설정은 호스트가 소유하는 표면으로 남으며, 이 문서는 위치 경계만 담당합니다.
