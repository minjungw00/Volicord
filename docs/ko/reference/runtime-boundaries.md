...
에이전트, MCP 호출자, CLI 텍스트, 운영자 출력, 제품 파일, Projection Markdown, 템플릿, 상태 카드, 아티팩트 바이트, 대화 기록은 그 자체로 기준 상태를 변경하지 않습니다. 관련 담당 경로가 받아들일 때만 입력 또는 증거 후보가 될 수 있습니다.
