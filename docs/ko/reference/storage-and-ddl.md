...
| `ArtifactRef`, `ValidatorResult`, idempotency와 state conflict behavior | [API Schema Core](api/schema-core.md#artifactref), [API Schema Later](api/schema-later.md#validatorresult-stable-ids), [API Errors](api/errors.md) |
...
- [API Schema Core](api/schema-core.md#artifactref)가 담당하는 registered `ArtifactRef` shape와 active stage value set
...
- API-shaped payload는 [MVP API](api/mvp-api.md)와 [API Schema Core](api/schema-core.md)에 맞게 validate합니다.
...
| `write_authorizations.status` | [Kernel `prepare_write`](kernel.md#prepare_write)와 [`harness.prepare_write`](api/mvp-api.md#harnessprepare_write) |
