...
- Core가 기록한 `blocked` metadata-only notice는 여전히 committed registered artifact record입니다. Artifact ref, hash, size, content type, owner relation, retention class는 metadata-only notice bytes에 적용되며, 금지된 raw bytes를 사용할 수 있게 만들지 않고 audit/display continuity를 보존합니다.
