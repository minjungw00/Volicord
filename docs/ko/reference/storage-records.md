...
값이 있으면 [API 상태 스키마](api/schema-state.md)의 정확한 byte-level 계약을 충족해야
합니다. 선택 JSON은 JSON `null`을, 선택 scalar 저장소는 SQL `NULL`을 사용합니다.
Store는 이 표현들을 trim, 대체, 강제 변환하거나 상호 변환하지 않습니다. 모든 scalar
baseline read는 공유 checked decoder를 사용하며 담당 table, record reference, logical
column을 포함한 typed 영속 데이터 `Corrupt`로 보고합니다. 모든 JSON 담당자는 같은
