...
이 참조 문서는 하네스 Core의 권한 모델을 정의하는 참조 문서입니다. 런타임 상태, 생성된 산출물, 서버 구현 자료가 아닙니다.
...
- Core 개념이 API, 저장소, 보안, 상태 보기, 지원 범위 밖 기능 자료와 닿을 때의 담당 문서 연결.
...
- 렌더링된 상태 보기 본문, 템플릿 문구, 커넥터 사용법, 보안 보장 어휘, 지원 범위 밖 기능 목록.
...
| 기준 범위와 지원 범위 밖 기능은 분리됩니다. | [기준 범위 경계](#core-invariant-baseline-scope-boundary)를 참고합니다. |
...
- 읽기 전용 또는 구체화 전용 Run이 뒤따르는 제품 쓰기를 호환되게 만든다는 의미.
...
별도 QA와 검증 작업 흐름은 기준 범위에서 개념 경계일 뿐 활성 관문이 아닙니다. 담당 문서가 승격하기 전까지 활성 닫기 요구사항처럼 쓰면 안 됩니다.
...
기준 범위 경로:
- 담당 문서가 더 많은 것을 승격하기 전까지 지원 경로는 간결한 잔여 위험 요약, 차단 사유, 증거 참조, `user_judgment` 참조입니다.
- 풍부한 위험 흐름은 승격 전까지 지원 범위 밖입니다.
...
| 지원 범위 밖 기능 | [지원 범위 밖 기능](#core-owner-out-of-scopes)를 참고합니다. |
...
적용 대상:
- 지원 범위 밖 기능.
- 기준 범위 밖의 보증, 면제, QA, 검증, 픽스처 자료.

담당 문서 링크:
- [범위 참조](scope.md).

다른 문서가 정확한 스키마, DDL, 렌더링된 템플릿 문구, 공개 오류 코드, 지원 범위 밖 기능 목록을 필요로 하면 여기서 다시 정의하지 말고 담당 문서로 연결해야 합니다.
