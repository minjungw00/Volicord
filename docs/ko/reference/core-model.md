...
현재 MVP 닫기는 later 보증 자료를 활성 응답 의미로 끌어오면 안 됩니다. `verification_gate`, `qa_gate`, 분리형 검증, `completed_verified`, 자세한 수동 QA 닫기 필드, Full Evidence Manifest 동작, 보증 표시 세부사항은 담당 문서가 명시적으로 켜기 전까지 later 후보 동작입니다.
