...
2. `idempotency_key`와 replay state를 검증합니다.
3. `expected_state_version`을 확인합니다.
4. Active Task를 resolve합니다.
...
1. Request envelope를 검증합니다.
2. Idempotency replay state를 확인합니다.
3. `expected_state_version`을 확인합니다.
4. Run kind를 확인합니다.
5. Run이 product write를 보고하는지 감지합니다.
6. Product write가 있으면 compatible active Write Authorization을 요구합니다.
7. Observed changed paths, commands, tools, secret access를 authorization과 active scope에 맞춰 검증합니다.
8. Compatible하면 authorization을 소비합니다.
9. Run record를 만듭니다.
10. `ArtifactRef`를 등록하거나 연결합니다.
11. Active path가 요구하는 evidence summary 또는 evidence refs를 업데이트합니다.
12. Recorded Run이 영향을 주는 blockers와 gates를 업데이트합니다.
13. Task event를 append합니다.
14. Response를 반환합니다.
