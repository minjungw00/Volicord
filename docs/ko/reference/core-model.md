...
- 티켓 유효성은 현재 `Task`, Change Unit, `scope_revision`, 기준선, workspace
  결속, 승인 근거, 명시적 취소 상태, 정책이 선택한 선택적 유휴 제한 시간에
  의존합니다. 관련 없는 `state_version` 변경은 무효화 조건이 아닙니다.
...
- 민감 범주, 거부되거나 보호된 경로, 비밀값 접근, 외부 전송, 파괴적 동작,
  ...
  높입니다.
- Core는 활성 `Task`를 자동으로 낮추지 않습니다. 완화된 정책은 새 `Task`에만
  적용합니다. 강화된 정책은 활성 `Task`에 재평가 필요를 표시하고 다음 쓰기
  호환 동작 전에 수준을 높입니다.
- 영속 표시는 `tasks.metadata_json`의 닫힌 `policy_control_reevaluation`
  ...
  표시된 통제 수준과 수락 정책을 모두 충족할 때만 이 표시를 지웁니다. 이 표시는 별도의
  정책 권한이나 쓰기 허가가 아닙니다. 정책 커밋은 해당 Task의 모든 활성 티켓을 같은
  트랜잭션에서 `explicit_revoke`로 무효화합니다. Guard와 Core는 충족되지 않은 표시와
  함께 발견한 기존 활성 티켓도 사용할 수 없는 것으로 처리하고 `volicord.prepare_write`를
  요구합니다. Status, Intake resume, UpdateScope, RecordRun, CloseTask는 영속 값, 현재 정책,
...
| 쓰기 호환성 | 제품 파일 쓰기 시도와 유효 `sensitive` 통제 아래의 정확한 비제품 동작은 현재 적용 범위와 열린 쓰기 티켓에 호환되어야 합니다. |
...
- 상태 결속: 유효성 근거는 정확한 `Task`, 현재 적용 Change Unit,
  `scope_revision`, 기준선, workspace 결속, 승인 근거 참조입니다. 발급 때의
  `basis_state_version`은 감사 순서에만 사용합니다.
...
- 명시적 무효화: `Task` 닫기 또는 교체, Change Unit이나 범위 리비전 변경,
  ...
  정책의 선택적 유휴 제한 시간만 구조화된 사유와 함께 티켓을 무효화합니다.
  기본 제한 시간은 없습니다.
