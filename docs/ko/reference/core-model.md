...
MVP-1 `evidence_summary`는 Run, blocker, user judgment, `ArtifactRef`를 참조할 수 있습니다. 하지만 그 자체로 detached verification, 수동 QA, 작업 수락, 잔여 위험 수용, close를 만들지 않습니다.
...
- Work acceptance가 required이면 close basis visibility 이후 compatible work-acceptance `user_judgment`가 기록되어 있습니다.
