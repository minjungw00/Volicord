...
`record_run`은 담당 경로가 승인한 아티팩트 경로를 통해서만 스테이징된 아티팩트를 승격하거나 지속 `ArtifactRef` 값을 연결할 수 있습니다. 원시 비밀값, 토큰, 금지된 민감 로그, 호출자가 임의로 준 경로, 신뢰할 수 없는 바이트는 증거를 완성해 보이게 하려고 저장하면 안 됩니다. 거절, 가림, `omitted`/`blocked` 표시, 승인된 안전 핸들 중 하나로 처리해야 합니다.
