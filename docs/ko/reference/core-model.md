...
- 쓰기 티켓은 상태에 결속된 제품 파일 변경 하나에 Core가 쓰기 권한을 부여한
  사실을 기록합니다. 반복된 `prepare_write`는 호환되고 아직 소비되지 않은 같은
  티켓을 재사용할 수 있지만, 재사용이 경로, 승인, 권한을 넓히지는 않습니다.
...
쓰기 티켓은 제안된 제품 파일 변경 하나에 Core가 쓰기 권한을 부여한 사실을
남기는 영속 Core 기록입니다.
...
- 쓰기 티켓은 Volicord 상태에서 제품 파일 쓰기 시도 하나에 Core가 쓰기 권한을
  부여한 사실을 기록합니다. 쓰기 실행 증명, 증거 기록, 결과 수락, 위험 수락,
