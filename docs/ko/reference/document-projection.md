...
MCP read-only resource staging도 같은 계층을 따릅니다. v0.1 resource는 첫 authority loop를 위한 current project/current task/status output을 노출합니다. v0.2 resource는 status/card 또는 task summary output을 통해 사용자 판단 맥락과 최소 evidence/close summary를 더합니다. Evidence Manifest, reports, bundles, Journey/Spine, design/domain resource는 owner profile이 명시적으로 승격하기 전까지 later-profile 또는 diagnostic read로 남습니다. Projection을 읽는 resource도 여전히 read입니다. Projection job을 만들거나 projection을 authority로 만들면 안 됩니다.
...
이 산출물은 template shape를 재사용할 수 있지만, template 종류가 구현 범위를 키우면 안 됩니다. 작업 수락 필요 여부/상태와 잔여 위험 표시는 관련 있을 때 distinct Core meaning으로 남지만, 별도 필수 projection kind가 아니라 최소 status, decision, close-blocker 요약 안에 나타납니다. Core state와 ref에서 파생된 하나의 간결한 status/next surface와 명확한 사용자 판단 요청 display만으로도 MVP 표시 경로를 충족할 수 있습니다.
...
| Decision Packet | `judgment_category`, `judgment_route`, `display_depth`를 포함한 `state.sqlite.decision_packets`, 관련 `decision_gate` state, judgment event, 관련 approval 또는 reconcile record, artifact ref, 필요할 때 연결된 `state.sqlite.residual_risks` | `TASK` Pending Judgments, Journey Card judgment line, status/next responses, judgment-context resources, decision-packet resources; standalone projection이 켜져 있을 때 optional `DEC` | `request_user_judgment` / `record_user_judgment`, then projector |
...
- Decision Packet: `judgment_category`, `judgment_route`, `display_depth`를 포함한 `state.sqlite.decision_packets`와 관련 ref -> `TASK` Pending Judgments, status/next responses, judgment-context resources, decision-packet resources; standalone projection이 켜져 있을 때 optional `DEC` projection
