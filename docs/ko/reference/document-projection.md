...
MCP read-only resource staging도 같은 계층을 따릅니다. v0.1 resource는 첫 authority loop를 위한 current project/current task/status output을 노출합니다. v0.2 resource는 status/card 또는 task summary output을 통해 사용자 결정 맥락과 최소 evidence/close summary를 더합니다. Evidence Manifest, reports, bundles, Journey/Spine, design/domain resource는 owner profile이 명시적으로 승격하기 전까지 later-profile 또는 diagnostic read로 남습니다. Projection을 읽는 resource도 여전히 read입니다. Projection job을 만들거나 projection을 authority로 만들면 안 됩니다.
...
Decision Packet projection과 card는 구체적인 결정 유형을 보여줘야 합니다. 기준 schema field와 표시용 label은 분리해서 렌더링합니다. `decision_kind`는 lifecycle/gate route이고, `decision_profile`은 Decision Packet `profile_payload` branch와 profile별 필수 정보를 검증할 때 사용하는 schema/profile field이며, `judgment_domain`은 사용자 표시 판단 묶음을 위한 schema-owned enum입니다. Template은 enum 값을 자연스러운 label로 바꿔 보여줄 수 있지만, 그 label은 표시 전용입니다. 선택된 `decision_profile`에 따라 화면은 간단한 결정, 제품/UX 절충, 아키텍처 절충, 승인 형태 질문, 면제 판단, 작업 수락, 잔여 위험 수용, 조정, 복합 결정에 맞는 깊이와 맥락을 보여줍니다. 구체적인 결정 유형은 route, profile, 관련 owner ref에서 파생하고 domain은 표시 묶음을 제어합니다. 그래야 민감 동작 승인이 작업 수락처럼 보이지 않고, 작업 수락이 잔여 위험 수용처럼 보이지 않습니다. 영향을 받는 gate나 막힌 행동은 domain 또는 profile label이 아니라 packet의 `affected_gates`와 관련 owner record에서 옵니다. `decision_profile`은 `ProjectionKind`, gate, 닫기 집계 규칙, Approval 대체물, waiver 대체물, 작업 수락 대체물, 잔여 위험 수용 rule, 권한 경로가 아닙니다. `judgment_domain`은 enum 값으로 검증되지만 payload branch를 고르거나 gate를 다시 계산하거나 닫기 집계 규칙, Approval 대체물, waiver 대체물, 잔여 위험 수용 rule, 권한 경로, `decision_kind`를 대체하지 않습니다. 이 field에서 파생한 표시용 label은 schema field, 검증 입력, gate, owner record, authority path가 아닙니다.
...
- Decision Packet: `decision_kind`, `decision_profile`, `judgment_domain`을 포함한 `state.sqlite.decision_packets`와 관련 ref -> `TASK` Pending Decisions, status/next responses, judgment-context resources, decision-packet resources; standalone projection이 켜져 있을 때 optional `DEC` projection
