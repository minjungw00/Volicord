...
- `secret_omitted`와 `blocked` artifact는 ref와 redaction/omission/block note로 표시합니다. Projector는 생략된 secret/PII value를 재구성하거나, inline하거나, 요약하거나, export하면 안 됩니다.
- `blocked` artifact 표시는 raw capture를 사용할 수 없다는 뜻입니다. Evidence, QA, verification, Release Handoff, export display는 artifact를 검토할 수 있는 것처럼 보이지 않게 block 상태를 드러내야 합니다.
