...
Decision Packet projection과 card는 구체적인 결정 유형을 보여줘야 합니다. `judgment_domain`은 schema가 소유하는 사용자 표시 판단 묶음으로 렌더링하고, `decision_profile`은 표시 깊이와 선택 맥락을 해당 profile에 맞게 정합니다. Template은 enum 값을 자연스러운 label로 바꿔 보여줄 수 있지만, `decision_kind`는 lifecycle/gate route로, `judgment_domain`은 표시 묶음으로, `decision_profile`은 질문 깊이와 profile별 필수 정보를 정하는 기준으로 유지해야 합니다. 선택된 `decision_profile`에 따라 화면은 간단한 결정, 제품/UX 절충, 아키텍처 절충, 승인 형태 질문, 면제 판단, 작업 수락, 잔여 위험 수용, 조정, 복합 결정에 맞는 깊이와 맥락을 보여줍니다. 구체적인 결정 유형은 경로, profile, domain, 관련 owner ref에서 파생합니다. 그래야 민감 동작 승인이 작업 수락처럼 보이지 않고, 작업 수락이 잔여 위험 수용처럼 보이지 않습니다. 영향을 받는 gate나 막힌 행동은 domain 또는 profile label이 아니라 packet의 `affected_gates`와 관련 owner record에서 옵니다. `judgment_domain`과 `decision_profile`은 `ProjectionKind`, gate, 검증기 입력, 닫기 집계 규칙, Approval 대체물, waiver 대체물, 잔여 위험 수용 rule, 권한 경로가 아닙니다.
