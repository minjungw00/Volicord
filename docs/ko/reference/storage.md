...
이 문서는 향후 하네스 저장소를 위한 참조 문서입니다. 이 리포지터리에 하네스 서버,
Runtime Home, 데이터베이스, 아티팩트 저장소, 마이그레이션 실행기, 생성된 Projection,
런타임 상태, 구현 완료 DDL이 있다는 뜻이 아닙니다. 현재 리포지터리 상태는
...
계약을 넘어서는 보안 격리, 암호학적 증거 보장, 변조 방지 저장소 보장을 만들지 않습니다.
...
| `surfaces` | `state.sqlite` | `surface_id`, 기능 프로필, 로컬 접근 상태, 보장 표시를 해석하는 데 필요한 등록된 로컬/참조 surface 사실을 저장합니다. | `surface_id`, `project_id`, `surface_kind`, `capability_profile_json`, `local_access_posture`, `guarantee_level`, `status`, `created_at`, `updated_at`. |
...
`surfaces`는 connector marketplace나 넓은 connector ecosystem 테이블이 아닙니다.
`surface_id`, 기능, 로컬 접근 상태, 보장 표시를 해석하는 데 필요한
active 로컬/참조 surface 등록입니다.
...
- 아티팩트 저장소 아래 등록된 바이트 또는 안전한 메타데이터 notice,
...
`secret_omitted` 또는 `blocked` notice, 안전한 handle, 담당 문서가 허용한 안전한 표현을
저장합니다.
...
`tool_invocations`는 커밋된 non-dry-run 상태 변경 응답의 정확한 replay를
...
잠금은 동시 상태 쓰기를 보호합니다. OS 샌드박스, 아티팩트 무결성 보장,
변조 방지 저장소, 권한 격리, 도구 실행 전 차단을 제공하지 않습니다.

## 10. 마이그레이션 경계

이 리포지터리에는 마이그레이션 실행기가 없고 마이그레이션할 런타임 데이터도 없습니다.
...
- Runtime Home 메타데이터와 `project_state`, 또는 수락된 동등한 메커니즘에
  스키마/프로필 버전을 저장합니다.
...
Later/profile 저장소는 담당 문서가 범위, 대체 동작, 증명 기대치와 함께 좁은
동작을 승격하기 전까지 현재 활성 MVP 밖에 있습니다. 참조 스키마에 존재한다는
