...
- `task_events`에는 고유한 `event_id`가 필요하고, 영향을 받는 범위 안에서
  `event_seq`가 단조 증가하며 고유해야 합니다. Task 범위 event는
  `(project_id, task_id, event_seq)`, `task_id`가 null인 프로젝트 범위 event는
  `(project_id, event_seq)` 범위입니다.
- `tool_invocations`에는 `(project_id, tool_name, idempotency_key)`에 대한 고유 replay
  key가 필요합니다. `request_hash`는 그 행에 저장하는 충돌 판별자입니다. 같은
...
- 현재 MVP의 일반 Core 동작은 권한 행을 물리 삭제하지 않습니다. 상태 또는 생명주기
  필드를 바꾸고, event를 추가하며, replay와 아티팩트 메타데이터를 감사와 복구를 위해
  남깁니다.
...
- 향후 retention 또는 migration 경로는 아티팩트 hash, 담당 연결, event, replay 행을
...
| `tool_invocations.status` | `committed` | replay 행은 커밋된 non-dry-run 상태 변경 응답에만 존재합니다. Dry-run과 커밋 전 실패에는 replay 행이 없습니다. |
...
- 차단된 `prepare_write`, dry-run, 잘못된 요청, 커밋 전 실패는 소비 가능한
...
복구는 담당 경로를 통한 새 event와 현재 행 업데이트로 기록합니다. 멱등 replay, dry-run,
잘못된 요청, 커밋 전 실패는 event를 추가하지 않습니다.
...
차단 사유가 없다고 말하는 권한을 만들면 안 됩니다. 예를 들어 blocked `prepare_write`
응답은 소비 가능한 `write_authorizations`를 만들지 않습니다.
...
`harness.status`, dry-run 호출, 잘못된 요청, 커밋 전 검증 실패, 커밋 전 상태 충돌,
멱등 replay는 상태 시계를 올리지 않습니다. 커밋된 차단 응답은 메서드 담당 문서가
Core에 blocker 또는 다른 현재 행 상태 변경 저장을 허용할 때만 영향을 받은 시계를
올립니다.
...
`expected_state_version`이 필요해지려면 향후 담당 문서가 명시적 다중 범위 version
동작을 먼저 정의해야 합니다.
...
- 아티팩트 해시와 담당 연결을 보존하거나 영향을 받은 ref를 복구 대상으로 유효하지 않게
  표시합니다.
- 커밋된 `tool_invocations` replay 행을 보존해 migration 뒤 멱등성이 갈라지지 않게
  합니다.
