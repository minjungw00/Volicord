...
| `blockers` | `state.sqlite` | 다음 행동, 쓰기 호환성, 증거 공백, 닫기 준비, 복구를 위한 구조화된 차단 사유 상태를 저장합니다. `close_task` 계약이 커밋을 허용한 닫기 차단 사유 행렬 기록은 `CloseBlocker` 관련 기록입니다. `prepare_write`의 `write_decision_reasons`는 응답과 재실행 페이로드에 담기는 prepare_write 판단 사유이지 닫기 차단 사유 행이 아닙니다. | `blocker_id`, `task_id`, `blocked_action`, `blocker_kind`, `status`, `message`, `owner_ref_json`, `related_refs_json`, `required_next_action`, `created_at`, `resolved_at`. |
...
- 차단된 `prepare_write`, `dry_run`, 잘못된 요청, 커밋 전 실패는 소비 가능한
  ...
  이 사유는 `CloseBlocker` 기록이 아니며 닫기 상태 차단 사유 저장소도 아닙니다.
...
때만 저장합니다. `decision=blocked`, `decision=approval_required`,
`decision=decision_required`인 커밋된 `PrepareWriteResult` 응답은 현재 API 계약이
허용하는 대로 `tool_invocations.response_json`에 `write_decision_reasons`를 담을 수
있습니다. 이 `WriteDecisionReason` 값은 응답과 재실행 페이로드에 담기는 prepare_write 판단 사유,
즉 write decision 이유이지 `CloseBlocker` 기록이 아닙니다. `prepare_write`는
`close_state`를 바꾸지 않고, 닫기 차단 사유 행렬을 실행하지 않으며, 닫기 차단 사유
행렬 기록을 만들지 않습니다. `CloseTaskResult(close_state=blocked)`는 유효한 닫기 차단 사유 행렬
...
있습니다. 이 사유는 prepare_write 판단 사유 또는 write decision 이유이지
`CloseBlocker` 기록도 아니고 닫기 차단 사유 행렬 기록도 아닙니다. 이 분기는 소비 가능한
Write Authorization을 만들지 않고, `close_state`를 바꾸지 않고, 닫기 차단 사유 행렬을
실행하지 않고, `CloseBlocker`를 만들지 않고, 닫기 차단 사유 행렬 기록을 만들지 않으며,
증거를 갱신하거나, 아티팩트를 바꾸거나, 스테이징된 핸들을 소비하거나, `close_task`
효과를 수행하면 안 됩니다. 메서드 계약이 허용한 이벤트, 재실행 행, write decision 이유
페이로드, `project_state.state_version` 효과만 가질 수 있습니다.
...
없음, Write Authorization 소비 없음, `close_state` 변경 없음, `CloseBlocker` 없음,
닫기 차단 사유 행렬 기록 없음, 아티팩트 효과 없음, 증거 업데이트 없음, 스테이징된 핸들
소비 없음, `project_state.state_version` 증가 없음입니다.
...
저장소는 이 prepare_write 판단 사유를 `CloseBlocker`나 닫기 차단 사유 행렬 기록으로
바꾸면 안 됩니다. 커밋된 차단 `CloseTaskResult`는 `CloseBlocker`를 사용하며,
`close_task` 계약이 명시적으로 허용한 차단 사유, 이벤트, 재실행 행,
`project_state.state_version` 효과만 만들 수 있습니다. 어떤 커밋된 차단 결과도 차단
사유가 없거나 부족하다고 지적한 권한을 만들면 안 됩니다. 차단된 `prepare_write`는
소비 가능한 `write_authorizations`를 만들지 않습니다.
