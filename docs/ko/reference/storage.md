...
`evidence_summaries.completion_policy_json`은 Task 또는 Change Unit에서
`close_task intent=complete`에 쓰는 간결한 `CompletionPolicy`를 저장합니다. `intent=cancel`이나
`intent=supersede` 정책이 아닙니다. `coverage_items_json`은 명시적인 `claim`,
...
- Projection 작업, 영속 Projection 캐시, managed-output outbox, managed block 해시와 drift
