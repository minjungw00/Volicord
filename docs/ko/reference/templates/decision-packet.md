...
구현 계층: User-facing MVP summaries입니다. Decision Packet 사용자 판단 요청 display/card shape이며, standalone `DEC` `ProjectionKind`가 아닙니다. Standalone persisted `DEC` Markdown projection은 standalone Decision Packet projection 기능이 켜진 경우에만 사용하며, 필요한 prompt는 status/next, `TASK`, Decision Packet resource를 통해 나타날 수 있습니다.
