...
수동 QA가 필요할 때 기록, gate, 프로필, 대상, 확인 목록, 기록할 근거, 면제와 위험 표시를 사람이 확인하기 쉬운 간결한 안내 카드로 보여주기 위해 수동 QA 카드를 사용합니다.
...
구현 계층: 보증 프로필 보고서입니다. Manual QA 프로필이 명시적으로 활성화된 경우 사용하며 전체 Manual QA policy coverage는 이후 staged scope입니다.
...
- 사람 inspector 또는 역할과 요청되는 사람의 판단
- 대상 화면(screen) 또는 흐름(flow)
- 확인 목록 항목
- 예상 screenshot, walkthrough note, browser log, Browser QA artifact, 수동 제공 artifact 근거
- QA가 면제되거나 미뤄질 때 waiver reason, 필요한 경우 QA waiver 사용자 판단 참조, Residual Risk 참조
- 검증, 작업 수락, 닫기 영향 요약
- 수동 QA 기록, QA waiver 사용자 판단, 근거 목록(Evidence Manifest), Eval(분리 검증 결과), 작업 수락 맥락, 잔여 위험(Residual Risk), 아티팩트 참조, 가림 상태, 읽기용 보기 최신성(projection freshness)을 위한 간결한 참조
...
- Browser QA 아티팩트: {artifact_refs|none; 뒷받침 참조만}
...
수동 QA 결과를 기록하거나, 허용된 낮은 위험의 QA waiver 사유를 기록하거나, 사용자 소유 위험이 있으면 QA waiver 사용자 판단을 요청하시겠습니까?
````
