...
이 문서는 상태 카드, 판단 요청, 실행/증거 요약, 닫기 결과, 에이전트 맥락 패킷에 대한 현재 MVP 활성 렌더링 본문 기대치를 담당합니다. 참조 문서일 뿐이며 상태 보기 파일, 런타임 아티팩트, QA 기록, 수락 기록, 닫기 기록을 생성하지 않습니다.
