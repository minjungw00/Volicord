...
- 설계 품질 관찰 사항, 활성 `ValidatorResult.validator_id` 값, 지원 범위 밖 설계 정책 후보 사이의 경계
...
- 넓은 설계 정책 검증기, 설계 정책 면제, 심각도 기반 활성 차단 정책, 관리 책임 정책, 전체 검토 절차, 운영/보고 후보, 지원 범위 밖 적합성 목록.
...
| 지원 범위 밖 설계 관문, 정책 차단 사유, 넓은 검증기, 면제 후보, 정책 목록 | [범위 참조](scope.md) |
...
허용되지 않는 것:
- 지원 범위 밖 설계 정책 자료를 활성 관문, 닫기 차단 사유, 면제 경로, 증거 규칙, 닫기 권한으로 취급하지 않습니다.

<a id="design-quality-close-advisory-only-policy-phrase"></a>
### 조언에 그치는 정책 문구

조건:
- 발견 사항이 도메인 언어, 세로 조각 형태, TDD, 모듈/인터페이스 검토, 관리 책임, 수동 QA, 분리형 검증, 검토 단계, 지원 범위 밖 정책 후보를 언급합니다.
...
허용되지 않는 것:
- 지원 범위 밖 정책 후보를 기준 범위 요구사항처럼 제시하지 않습니다.
...
기준 범위에는 활성 설계 품질 면제나 설계 정책 면제 경로가 없습니다. 어떤 요구사항을 유예하거나, 위험으로 수락하거나, 사용자 판단으로 해결할 수 있는지는 해당 활성 담당 경로가 정합니다. 그 경로의 정확한 `judgment_kind`, 차단 사유 범주, 증거 동작을 사용해야 합니다.
...
| 지원 범위 밖 설계 품질 면제 후보 | [지원 범위 밖 면제 후보](#design-quality-route-future-waiver-candidates) 참조 |
...
허용되지 않는 것:
- 설계 정책 면제, 포괄 승인, QA 면제, 검증 위험 수락, 승격되지 않은 지원 범위 밖 후보로 취급하지 않습니다.

<a id="design-quality-route-future-waiver-candidates"></a>
### 지원 범위 밖 면제 후보

조건:
- 제안된 설계 품질 면제나 정책 면제가 기준 범위로 승격되지 않았습니다.
...
- 수동 QA 또는 검증 참조는 해당 지원 범위 밖 기능 담당 경로가 승격된 뒤에만 사용

<a id="design-quality-evidence-non-satisfying-references"></a>
### 증거를 자동 충족하지 않는 참조

허용되지 않는 것:
- 채팅 주장, 일반 요약, 렌더링된 상태 보기 문장, 등록되지 않은 파일, 담당 경로 없는 화면 캡처, 테스트 통과만 있는 상태, 지원 범위 밖 면제 후보, 최종 수락, 잔여 위험 수락을 필요한 증거로 자동 취급하지 않습니다.
...
지원 범위 밖 안정 검증기 ID 집합은 담당 문서가 좁은 활성 계약으로 승격하기 전까지 [정책과 적합성: `ValidatorResult` 안정 ID와 정책 계열](scope.md)의 후보로 남습니다.

## 8. 지원 범위 밖 정책 후보 경계

전체 설계 품질 정책 목록은 기준 범위가 아닙니다. 아래 아이디어는 이름 있는 담당 문서가 범위, 대체 동작, 정확한 계약, 증명 기대치와 함께 좁은 동작을 승격하기 전까지 지원 범위 밖입니다.
...
| 자세한 검토 표시와 보고 자료 | [자세한 검토 표시](#design-quality-later-detailed-review-displays) 참조 |
...
승격에 필요한 것:
- 승격하려면 범위 있는 정책 담당 문서, 독자에게 보이는 동작, 증명 기대치, 기준 범위/지원 범위 밖 전환 경로가 필요합니다.
...
승격에 필요한 것:
- 승격하려면 [범위 참조](scope.md)을 통한 승격, 승격 시점의 담당 문서 갱신, 기준 계약 정리가 필요합니다.

지원 범위 밖 기능은 이름만 유지할 수 있습니다. 이를 기준 범위 요구사항, 차단 사유, 면제 규칙, 증거 기대치, 검증기 매핑, 픽스처 요구사항, 운영 보고, 구현 작업처럼 제시하면 안 됩니다.
