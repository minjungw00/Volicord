...
| `CloseReadinessBlocker` | close 가능성 평가에서 닫기 가능성을 판단할 때 발견한 닫기 차단 사유의 데이터 구조입니다. 활성 사용 위치는 `StatusResult.close_blockers`와 `CloseTaskResult.blockers`입니다. 이 타입 자체는 저장 효과를 뜻하지 않으며, 지속 저장, `close_state` 변경, 다른 상태 효과를 뜻하지 않습니다. `prepare_write`, `DryRunSummary.would_blockers`, `ToolRejectedResponse.errors`에는 쓰지 않으며, `STATE_VERSION_CONFLICT`는 `CloseReadinessBlocker.code`가 될 수 없습니다. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [API Errors](api/errors.md), [Core Model](core-model.md), [Storage](storage.md) |
