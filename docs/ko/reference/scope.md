# 범위 참조

이 참조 문서는 Volicord의 지원되는 기준 범위 경계를 담당합니다. 제품 정체성은 [사용자 가이드 개요](../user-guide/overview.md)가 담당합니다. 이 문서는 기준 범위 안에 있는 기능 묶음, 기준 범위 밖에 남는 기능, 예약된 값과 프로필 조건부 값이 범위에 미치는 영향을 정의합니다.

이 페이지는 안정적인 참조 계약이며, 프로젝트 경과 서술이나 구현 계획이 아닙니다.

<a id="owns-does-not-own"></a>
## 담당하는 것 / 담당하지 않는 것

이 문서가 담당합니다.

- 지원되는 기준 범위 경계
- 포함되는 기능 묶음과 제외되는 기능 묶음
- 예약된 값과 프로필 조건부 값의 범위상 의미
- 다른 문서가 반복하지 말고 요약해 연결해야 하는 범위 수준 보장과 비주장 문구

이 문서가 담당하지 않습니다.

- 기준 범위 구현 읽기 경로. [구현 가이드](../architecture-guide/change-guide.md)를 봅니다.
- API 메서드 동작
- 스키마 필드
- 저장소 기록이나 저장 효과
- 런타임과 제품 저장소 경계의 세부사항
- 보안 보장 의미나 증명 요구사항
- 적합성 절차
- 템플릿 본문
- 커넥터 동작
- 지원 범위 밖 기능의 상세 명세

어떤 기능이 지원되는 기준 범위에 속하는지 판단할 때는 이 문서를 기준으로 삼습니다. 경로 문서, 아키텍처 가이드, README, 참조 문서는 상세 범위 목록을 반복하지 말고 이 문서로 연결합니다.

<a id="supported-baseline-scope"></a>
## 지원되는 기준 범위

기준 범위는 의도적으로 좁습니다. 이 문서가 기능 묶음을 포함하고 관련 담당 문서가 동작, 데이터 형태, 저장 또는 보안 결과를 정의할 때만 그 기능은 지원되는 기준 범위 안에 있습니다.

<a id="what-supported-means"></a>
### 지원된다는 뜻

이 문서 세트에서 지원된다는 것은 아래를 뜻합니다.

- 그 기능은 에이전트, 구현자, 유지보수자가 의존할 수 있는 기준 계약에 속합니다.
- 필요한 경우 동작, 스키마 형태, 저장 효과, 보안 결과, 용어를 정의하는 담당 문서가 있습니다.
- 다른 문서는 기능을 요약할 수 있지만, 세부 질문은 이 문서나 [참조 색인](README.md)에 나열된 담당 문서로 보내야 합니다.

지원된다는 것은 아래를 뜻하지 않습니다.

- 스키마나 예시에 등장하는 모든 값 이름이 기준 범위 동작으로 제공된다는 뜻이 아닙니다.
- 경로 문서, 예시, 스키마 메모, 값 집합이 지원 범위 밖 기능을 언급한다고 해서 그 기능이 지원되는 것은 아닙니다.
- 관련 담당 문서가 정의하지 않은 보안, 적합성, 저장소, 상태 보기 보장이 존재한다는 뜻이 아닙니다.

<a id="included-capabilities"></a>
### 포함되는 기능

| 기준 기능 | 지원되는 경계 | 담당 문서 |
|---|---|---|
| 자연어 접수와 `Task` 생성 | 지원되는 접수 경로로 사용자의 자연어 의도에서 로컬 `Task`를 시작할 수 있습니다. | [접수 메서드](api/method-intake.md), [Core 모델](core-model.md) |
| 범위 업데이트 | 지원되는 범위 업데이트 경로로 `Task`와 Change Unit 범위를 갱신할 수 있습니다. | [범위 업데이트 메서드](api/method-update-scope.md), [Core 모델](core-model.md) |
| 상태와 닫기 준비 상태 확인 | 지원되는 읽기 경로로 상태, 증거 충분성, 알려진 차단 사유, 닫기 준비 상태를 읽을 수 있습니다. | [상태 메서드](api/method-status.md), [닫기 메서드](api/method-close-task.md), [API 상태 스키마](api/schema-state.md), [Core 모델](core-model.md) |
| 관리되는 최종 출력 권한 고지 | 지원되는 관리 Codex 또는 Claude Code 최종 출력 이벤트에서 Volicord는 현재 읽기 전용 상태를 새로 읽고, 검증된 기준 `AuthorityReceipt` 전체 또는 크기가 제한된 Task별 `volicord status` 대체 경로를 호스트 소유 고정 UI 표면에 표시할 수 있습니다. 이 상태 보기는 Core 상태를 바꾸지 않으며 모델이 작성한 최종 산문과 Detective 닫기 관문 양쪽과 구분됩니다. | [관리 CLI](admin-cli.md#managed-final-output-authority-disclosure), [Agent Connection](agent-connection.md#managed-final-output-authority-disclosure), [상태 보기 권한 참조](projection-and-templates.md#managed-final-output-authority-disclosure) |
| 정확한 과거 동작 결과 조회 | `volicord.get_operation_result`는 조회할 수 있는 변경 불가능한 `operation_category=agent_workflow` Core 변경 응답을 크기가 제한된 UTF-8 페이지로 읽을 수 있습니다. 접근하려면 원래 호출을 수행한 Agent Connection이 현재 활성 상태이고 선택 프로젝트가 허용되어 있으며 현재 행위자가 저장된 행위자와 같아야 합니다. 결과는 현재 권한이 아닌 과거 기록이며 `operation_category=user_only` 사용자 행동 응답은 제외합니다. | [동작 결과 조회 메서드](api/method-get-operation-result.md), [보안](security.md#historical-operation-result-access), [저장소 버전 관리](storage-versioning.md#exact-operation-result-retrieval) |
| 쓰기 티켓 | `volicord.prepare_write`는 호환되는 제안 제품 파일 변경 하나에 쓰기 티켓을 발급할 수 있습니다. | [쓰기 준비 메서드](api/method-prepare-write.md), [저장 효과](storage-effects.md), [보안](security.md) |
| Agent Connection 맥락 | 등록된 Agent Connection은 범위 확인에 쓸 기록된 출처, 모드, 명시적인 프로젝트 허용 목록을 제공합니다. | [Agent Connection 참조](agent-connection.md), [보안](security.md) |
| 아티팩트 스테이징과 호환되는 아티팩트 연결 | 새 아티팩트 바이트는 지원되는 스테이징 경로로 기준 범위에 들어올 수 있고, 호환되는 영속 아티팩트 참조는 아티팩트 담당 문서가 허용할 때 연결할 수 있습니다. | [아티팩트 스테이징 담당 문서](#artifact-staging-owners)를 봅니다. |
| 실행 및 증거 기록 | 기준 범위 작업에 대해 실행 기록, 간결한 증거 요약, 출처를 가진 대상 단위 증거 관찰을 남길 수 있습니다. | [실행 기록 메서드](api/method-record-run.md), [저장 효과](storage-effects.md), [Core 모델](core-model.md) |
| 집중된 사용자 소유 행동 캡처 | 지원되는 공통 User Action 경로로 choice 판단과 Evidence 관찰을 요청하고 해결할 수 있습니다. 이 행동은 Core 소유 상태, Evidence, 닫기 준비 상태 규칙을 대체하지 않습니다. | [사용자 행동 담당 문서](#user-action-owners)를 봅니다. |
| 프로젝트 연속성 기록 | 오래 유지해야 하는 결정, 의무, 알려진 한계, 수락된 잔여 위험, 제약을 현재 Core 권한을 대체하지 않는 프로젝트 수준 맥락으로 보존할 수 있습니다. | [Core 모델](core-model.md), [API 상태 스키마](api/schema-state.md), [저장소 기록](storage-records.md), [저장 효과](storage-effects.md) |
| 권한 번들 내보내기 | `volicord export authority-bundle`로 원본 Runtime Home이나 프로젝트 상태를 변경하지 않고 등록된 프로젝트 하나의 로컬 기록과 현재 사용할 수 있는 영속 아티팩트 본문을 무결성 라벨이 붙은 복사본으로 내보낼 수 있습니다. 문서에 정의된 비보장이 그대로 적용됩니다. | [관리 CLI](admin-cli.md#authority-bundle-export), [저장소 기록](storage-records.md), [보안](security.md) |
| 닫기 시도 | `volicord.check_close`는 닫기 준비 상태를 읽기 전용으로 평가할 수 있고, `volicord.close_task`는 증거, 최종 수락, 잔여 위험, 대체 불가 경계를 유지하면서 지원되는 닫기 결과를 시도할 수 있습니다. | [닫기 메서드](api/method-close-task.md), [Core 모델](core-model.md), [API 차단 사유 처리 경로](api/blocker-routing.md) |
| 읽기 시점 상태 표시 | 읽기 전용 상태 또는 파생 표시는 상태 보기와 템플릿 담당 문서가 허용할 때 원천 상태를 요약할 수 있습니다. | [상태 보기 권한 참조](projection-and-templates.md), [템플릿 본문](template-bodies.md), [API 상태 스키마](api/schema-state.md) |

<a id="artifact-staging-owners"></a>
아티팩트 스테이징 담당 문서:

- 메서드 동작: [아티팩트 스테이징 메서드](api/method-stage-artifact.md).
- API 형태: [API 아티팩트 스키마](api/schema-artifacts.md).
- 생명주기와 저장 효과: [아티팩트 저장소](storage-artifacts.md)와 [저장 효과](storage-effects.md).

<a id="user-action-owners"></a>
사용자 행동 담당 문서:

- 메서드 동작: [사용자 행동 요청 메서드](api/method-request-user-action.md)와 [사용자 행동 해결 메서드](api/method-resolve-user-action.md).
- 제품 의미: [Core 모델](core-model.md).
- API 형태와 값: [API 사용자 행동 스키마](api/schema-user-action.md), [API 판단 스키마](api/schema-judgment.md), [API 값 집합](api/schema-value-sets.md).

<a id="excluded-from-baseline-scope"></a>
## 기준 범위에 포함되지 않는 것

아래 기능 묶음은 기준 범위 밖에 있어 지원되지 않습니다. 이 범위 참조가 그 기능 묶음을 포함하고 영향받는 기존 담당 문서가 그 기능 묶음의 동작을 정의할 때만 지원됩니다.

<a id="product-role-exclusions"></a>
### 제품 역할 제외

Volicord의 지원되는 기준 범위는 Volicord를 아래 제품 역할로 만들지 않습니다.

- 코딩 에이전트 또는 코드 생성 에이전트
- 전체 워크플로 엔진 또는 워크플로 오케스트레이터
- 테스트 러너 또는 독립적인 제품 정확성 검증기
- 기술 부채 분석기 또는 상환 시스템
- 보안 샌드박스, OS 수준 격리 계층, 전역 파일시스템 모니터, 외부 파일 변경 방지 시스템
- 전체 에이전트 하네스, 호스팅 에이전트 플랫폼, 대시보드, 보고서 생성기

Volicord는 AI 지원 작업 주변의 담당 문서가 정의한 상태를 기록하거나, 경로를 잡거나, 요약하거나, 표시할 수 있습니다. 이런 지원 동작이 Volicord를 제외된 제품 역할로 승격하지 않습니다.

제외된 기능:

- 호스트 애플리케이션의 자체 아티팩트 캡처
- 지속 저장되는 상태 보기 작업, 상태 보기 조정, 생성된 상태 보기 파일, 관리되는 상태 보기 복구
- 확장 또는 추가 증거 수집 흐름
- 수동 QA와 외부 검증 작업 흐름
- 명령, 네트워크, 비밀값 접근 관찰
- 명령, 네트워크, 비밀값의 도구 실행 전 차단
- 예방형 격리, 샌드박싱, 또는 더 강한 격리 보장 의미
- 호스팅 대시보드
- 커넥터 마켓플레이스
- 권한 번들 가져오기, 복원, 전달 패키지, 그리고 원본 상태를 변경하지 않는 지원 대상 권한 번들 내보내기 이외의 내보내기 패키지 계열
- 실행 가능한 픽스처 실행기
- 생성된 적합성 산출물
- 운영 프로필

범위 규칙:

- 여기에 나열된 기능은 기준 범위 밖에 있습니다.
- 제외된 기능은 기준 범위 요구사항이 아닙니다.
- 제외된 기능은 이 범위 참조와 영향받는 기존 담당 문서가 지원된다고 명시적으로 정의할 때만 지원 동작이 됩니다.
- 그 전에는 지원 동작이 아닙니다.
- 민감 동작 승인은 그 자체로 광범위한 보안 모니터링, 격리 보관, QA 게이트, 검증 게이트, 명령/네트워크/비밀값 관찰, 도구 실행 전 차단, 격리, 또는 그 밖의 지원 범위 밖 동작을 뜻하지 않습니다.

담당 문서 경로:

- API 메서드 동작과 스키마 형태: [API 메서드](api/methods.md), [API 코어 스키마](api/schema-core.md), [API 상태 스키마](api/schema-state.md), [API 아티팩트 스키마](api/schema-artifacts.md), [API 판단 스키마](api/schema-judgment.md).
- API 값의 담당 범위, enum 형태 값과 상태 값의 의미, 정확한 값 이름: [API 값 집합](api/schema-value-sets.md).
- 저장소 기록/효과와 아티팩트 생명주기: [저장소](storage.md), [저장소 기록](storage-records.md), [저장 효과](storage-effects.md), [아티팩트 저장소](storage-artifacts.md).
- 런타임과 제품 저장소 경계: [런타임 경계](runtime-boundaries.md).
- 보안 비주장, 보장 수준, 관찰, 차단, 격리, 민감 동작 승인 경계: [보안](security.md), [Core 모델](core-model.md).
- 적합성 절차와 점검 기준: [적합성](conformance.md).
- 완전한 구조화 용어 메타데이터와 한영 표현 통제: [`docs/terminology-map.yaml`](../../terminology-map.yaml).

<a id="reserved-and-profile-gated-values"></a>
## 예약된 값과 프로필 조건부 값

일부 값 이름은 예약된 값이나 프로필 조건부 값으로 존재할 수 있지만, 그것만으로 사용자에게 보이는 지원 기능이 되지는 않습니다.

예약된 값:

- 예약된 값은 어휘, 호환성 경계, 또는 값 집합 항목으로 나타날 수 있습니다.
- 예약된 값은 이름만으로 동작을 켜거나 기본값을 만들거나 닫기 준비 상태를 충족하거나 보장을 만들지 않습니다.

프로필 조건부 값:

- 프로필 조건부 값은 관련 프로필이나 게이트가 담당 문서에 정의되고 이 범위 참조가 그 결과 기능을 포함할 때만 사용할 수 있습니다.
- 프로필/게이트나 기능 담당 문서 중 하나라도 없으면 그 값은 지원되는 기준 범위 동작이 아닙니다.

사용 지점 규칙:

- 예약된 값과 프로필 조건부 값은 나타나는 자리에서 그렇게 표시합니다.
- 예약된 값이나 프로필 조건부 값은 이 문서와 의미 담당 문서가 모두 그 동작을 정의할 때만 기본값, 필수값, 지원됨, 강제됨, 수락됨, 검증됨, 닫기 준비 상태, 탐지형, 샌드박싱, 더 강한 격리 동작처럼 설명할 수 있습니다.
- 그렇지 않으면 그 값을 위 라벨로 설명하지 않습니다.

담당 문서 링크:

- API 값의 담당 범위, 정확한 값 이름, 값 집합 위치, enum 형태 값과 상태 값의 의미: [API 값 집합](api/schema-value-sets.md).
- 더 강한 격리 주장을 포함한 보장 의미와 비주장: [보안](security.md).
- 예약된 값과 프로필 조건부 값의 구조화 용어 메타데이터: [`docs/terminology-map.yaml`](../../terminology-map.yaml).

<a id="out-of-scope-capability-promotion"></a>
## 지원 범위 밖 기능의 승격

지원 범위 밖 기능은 이 범위 참조와 영향받는 담당 문서가 좁은 지원 계약을 정의할 때만 지원됩니다.

승격은 아래를 정의해야 합니다.

- 지원되는 동작과 대체 동작
- 책임지는 담당 문서
- 영향을 받는 경우 API 동작, 스키마, 저장 효과, 런타임 경계, 보안 보장, 적합성 점검, 용어 갱신
- 영어와 한국어 대응 문서

지원하려는 기능의 적용되는 담당 문서가 없다면 지원 기능으로 설명하기 전에 담당 문서를 만들거나 지정해야 합니다. 자리표시자를 기존 담당 문서처럼 독자에게 안내하지 않습니다.

승격 경계:

- 제외된 기능, 예약된 값, 프로필 조건부 기능이 예시, 경로 문구, 스키마 메모, 값 집합, 이 참조 문서에 언급된다는 사실만으로는 승격되지 않습니다.

<a id="supported-guarantee-boundary"></a>
## 지원되는 보장 경계

기준 범위는 [보안](security.md)이 정의하는 보장 수준을 사용합니다. 어떤 보장을 지원된다고 설명하려면 범위상 사용 가능성과 보안 의미가 서로 맞아야 합니다.

지원되는 경계:

- 이 문서와 [보안](security.md)이 다른 지원 보장을 정의하지 않는 한 기준 범위의 보장 경계는 `cooperative`입니다.
- `volicord.prepare_write`와 쓰기 티켓은 Volicord 안의 제품 파일 쓰기 권한 메커니즘이며 격리나 샌드박싱 보장이 아닙니다.

포함되지 않는 것:

- 기준 범위는 더 강한 격리 보장 의미를 제공하지 않습니다.
- 예약된 값이나 프로필 조건부 값인 보장 라벨은 기준 범위를 확장하지 않습니다.

담당 문서 링크:

- 보장 의미, 탐지형 표현, 보안 비주장: [보안](security.md).
- 보장 라벨 값 항목: [API 값 집합](api/schema-value-sets.md).
- 메서드 동작: [쓰기 준비 메서드](api/method-prepare-write.md), [API 메서드](api/methods.md)의 안내.
- Core 의미: [Core 모델](core-model.md).

<a id="owner-documents"></a>
## 담당 문서

지원되는 기준 범위 질문에는 이 문서를 사용합니다. 범위 경계 밖의 세부 계약 질문은 [참조 색인](README.md)에서 시작한 뒤 적용되는 집중 참조 문서를 고릅니다. API 메서드 동작은 [API 메서드](api/methods.md)에서 시작합니다. 문서 기여자는 기계 판독 담당 메타데이터에 [`docs/doc-index.yaml`](../../doc-index.yaml)을 사용할 수 있습니다.
