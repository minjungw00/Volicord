...
이 참조 문서는 Volicord의 지원되는 기준 범위 경계를 담당합니다. Volicord는 AI 지원 제품 작업에서 로컬 작업 권한을 다루는 제품이자 시스템입니다. 이 문서는 기준 범위 안에 있는 기능 묶음, 기준 범위 밖에 남는 기능, 예약된 값과 프로필 조건부 값이 범위에 미치는 영향을 정의합니다.
