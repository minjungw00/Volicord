...
이 문서는 기준 범위의 아티팩트 저장 생명주기를 정의합니다.
...
- 호환되는 스테이징 핸들을 영속 `ArtifactRef`로 승격하는 저장소 경계.
- 기존 영속 아티팩트를 새 담당 관계에 연결할 수 있는 조건.
...
뜻하고, 점선 화살표는 스테이징만으로는 증거가 되지 않는 제외 경로를 표시합니다.
이 그림은 아티팩트 본문 읽기 점검이나 테이블 배치를 보여 주지 않습니다. 생명주기의
정확성은 이 문서가 담당하고, API 형태와 테이블 개요는
[API 아티팩트 스키마](api/schema-artifacts.md)와 [저장소 기록](storage-records.md)이
담당합니다.
...
flowchart LR
  ...
  Evidence["증거 자격은 여기서 시작<br/>영속 연결된 담당 기록"]
...
호환되는 담당 메서드만 스테이징 입력을 승격으로 넘길 수 있습니다. 기존 아티팩트 재사용은 이미 영속하는 `ArtifactRef`에서 시작하며 새 스테이징이 아닙니다.
...
| `attached` | 보통 `volicord.record_run` 같은 커밋된 담당 메서드가 증거 관찰 또는 아티팩트 참조를 안정적인 수락 기준 또는 보충 주장 대상에 연결한 상태. | 증거가 기록된 `Run`과 대상에 첨부된 상태입니다. 그래도 닫기에 대해 부족하거나, 오래됐거나, 막혔거나, 가려졌거나, 사용할 수 없을 수 있습니다. |
...
- 이미 영속하는 아티팩트를 새 담당 관계에 연결하는 단계입니다.
...
- 이 예시를 영속 `ArtifactRef`로 취급하면 안 됩니다.
...
- 영속 등록 전에 임시 스테이징 객체를 버렸습니다.

소비할 수 있는 값은 `staged`뿐입니다. `consumed`, `expired`, `discarded`는 `staged`로 돌아갈 수 없습니다.

## 승격

호환되는 담당 메서드만 스테이징 핸들을 소비하고 영속 `ArtifactRef`로 승격할 수 있습니다.
...
- 영속 검증은 빈 `body_path`, 절대 경로, 상위 디렉터리 이동, 플랫폼별 경로 접두사, `artifacts/`로 시작하는 저장 값을 거절합니다.
- 영속 검증은 유효하지 않은 저장 경로를 다른 저장 표현으로 변환하면 안 됩니다.

## 기존 아티팩트 참조

`existing_artifact`는 기존 아티팩트가 새 사용과 호환될 때만 영속 아티팩트 행을 재사용합니다.
...
- 심볼릭 링크를 해석한 뒤에도 대상이 아티팩트 저장소 안에 남는 안전한 저장소 상대
  `body_path`.
