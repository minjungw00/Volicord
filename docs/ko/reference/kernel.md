...
- `judgment_category`는 사용자-facing category를 저장합니다. Payload branch를 고르거나 gate를 혼자 재계산하거나 route 의미를 대체하지 않습니다.
