...
| 사용자 판단 category와 route | [판단 경로 경계](#판단-경로-경계), [Decision Packet](#decision-packet), [Decision Gate](#decision-gate) | Public request field는 [`harness.request_user_decision`](mcp-api-and-schemas.md#harnessrequest_user_decision)이 담당합니다. |
...
4. 이 Task를 close할 수 있는가?
   ...
   `close_task`는 close intent를 open Run state, scope, required decision, 민감 동작 Approval, evidence, required verification, required Manual QA, residual-risk visibility, required residual-risk acceptance, required final acceptance, projection freshness, artifact availability와 비교합니다.
...
| 추적되는 작업 | `work` | 구조화된 작업, 여러 단계, 위험한 변경, 사용자-facing 변경, public interface, 보안/개인정보, architecture, 기타 non-trivial work에 사용합니다. 범위, 사용자 판단, 근거, close blocker, 작업 수락, 잔여 위험을 보이게 둡니다. 모든 future gate가 자동으로 required가 되는 것은 아닙니다. |
...
Harness는 사용자가 무엇을 판단하는지와 내부에서 어떤 route로 기록하는지를 분리합니다. 사용자-facing 문서는 세 개의 독립 축을 사용자가 이해해야 하는 것처럼 노출하면 안 됩니다. 사용자는 category를 보고, route는 맞는 동사를 사용하며, 표시 깊이는 위험도에 맞게 커집니다.

### 사용자-facing category

| Category | 사용자가 소유하는 판단 | 저장 domain mapping |
|---|---|---|
| 제품/UX | 제품 동작, 문구, interaction, taste, 사용자 가치, release-facing promise. | `judgment_domain=product_ux` |
| 기술 구조 | Public API, module boundary, dependency, migration, compatibility, cost, maintainability, 중요한 implementation direction. | `judgment_domain=technical_architecture` |
| 보안/개인정보 | Data exposure, secret, auth, permission, retention, logging, redaction, user notice, security/privacy trade-off. | `judgment_domain=security_privacy` |
| 범위/자율성 | Scope expansion, non-goal, agent latitude, Change Unit update, Autonomy Boundary update. | `judgment_domain=scope_autonomy` |
| QA/검증 | 이름 붙은 QA나 verification path를 요구, 수행, 미룸, 면제할지. | `judgment_domain=qa_acceptance` |
| 작업 수락 | Final acceptance가 required일 때 결과를 받아들일지. | `judgment_domain=qa_acceptance`와 accept-result route |
| 잔여 위험 | 보이는 close-relevant risk를 이번 close에서 받아들일지. | `judgment_domain=residual_risk` |
| 복합 | 나누면 의미가 사라지는 하나의 cross-cutting judgment. | `judgment_domain=mixed` |
...
Existing schema field는 이 단순 모델에 이렇게 mapping됩니다.

- `judgment_domain`은 사용자-facing category를 저장합니다. Payload branch를 고르거나 gate를 혼자 재계산하거나 route 의미를 대체하지 않습니다.
- `decision_kind`는 내부 route와 resolution branch를 저장합니다. API/storage 이름이 위 route label과 다르면 public docs는 raw enum mechanics보다 이 동사로 mapping해 설명해야 합니다.
- `decision_profile`은 선택한 route의 표시/검증 깊이를 저장합니다. Schema `profile_payload` branch를 고를 수 있지만, gate, Approval substitute, waiver, 작업 수락, residual-risk acceptance, close rule이 아닙니다.
...
| 잔여 위험 수용 | Verification, Manual QA, evidence sufficiency, no-risk close, final acceptance, Approval. |
...
| v0.2 User-Facing Harness MVP | Scope, pending user judgment, evidence summary, close readiness, required final acceptance, close-relevant risk가 있을 때 residual-risk visibility를 사용자에게 보여줍니다. v0.2가 detached verification이 항상 required라고 암시하면 안 됩니다. |
...
Lifecycle은 작습니다. 필요한 judgment를 draft 또는 detect하고, 필요할 때 사용자에게 묻고, 호환되는 답변을 기록하고, deferral, rejection, blocked, superseded outcome을 보존합니다. 정확한 public field는 [`harness.request_user_decision`](mcp-api-and-schemas.md#harnessrequest_user_decision)과 [`harness.record_user_decision`](mcp-api-and-schemas.md#harnessrecord_user_decision)이 담당합니다.
...
Approval은 범위가 정해진 민감 동작 허가입니다. Paths, tools, commands, network targets, secret scope, sensitive categories, baseline, expiry, 해당 민감 동작에 대한 user decision을 cover할 수 있습니다.
...
Active profile이 requiredness를 정합니다. v0.1은 좁은 authority loop를 증명합니다. v0.2는 사용자-facing judgment, evidence summary, close readiness, required final acceptance, relevant residual-risk visibility를 보여줍니다. Later assurance profile은 detached verification, Manual QA, stewardship, feedback-loop/TDD, operations, export/recover behavior를 required로 만들 수 있습니다.
...
`approval_gate`는 sensitive categories가 있을 때만 적용됩니다. `granted`는 compatible Approval이 sensitive scope를 cover한다는 뜻입니다. Write Authorization, product judgment, evidence, verification, QA, final acceptance, residual-risk acceptance가 아닙니다.
...
Residual-risk visibility는 별도입니다. Known close-relevant risk가 없으면 `ResidualRiskSummary.status=none`이 visibility를 만족합니다. Known close-relevant risk가 있으면 final acceptance 또는 successful close 전에 보여야 합니다. Risk-accepted close에는 `accept-risk` route를 통해 accepted Residual Risk refs가 추가로 필요합니다.
...
Verification waiver는 detached verification이 아닙니다. Waived verification gap이 close-relevant이면 close에는 visible and accepted Residual Risk refs가 필요하고 `completed_with_risk_accepted`를 사용합니다. QA waiver는 Manual QA pass, verification, final acceptance, assurance upgrade가 아닙니다. Decision deferral은 waiver가 아닙니다.
