...
접점 이름 자체는 역량이 아닙니다. 커넥터는 실제 호스트, 버전/설정, 작업공간 정책, MCP 연결 태세, 수동 아티팩트 첨부 경로에 맞춘 `capability_profile`을 사용해야 합니다. 향후 캡처 경로, 가드 경로, 분리 경계가 승격된다면 그 경계도 프로필 범위에 포함해야 합니다.
...
접점 버전, MCP 설정, 권한, 작업공간 정책, 생성 파일, 관리형 블록, 수동 아티팩트 첨부 경로, 가림 정책, 아티팩트 보존 방식, 로컬 접근 태세, 적합성 근거가 바뀌면 `capability_profile`을 갱신합니다. 후크, 접점 자체 캡처, 도구 실행 전 차단, 격리 지원은 승격 전까지 later/profile-gated로 남습니다.
...
| `detective` | 하네스가 행동 뒤에 지원되는 사실을 보고 상태를 `stale`, `partial`, `blocked`, `failed`로 표시할 수 있다고 말합니다. `reference-local-mcp`에서는 변경 경로 관찰로 제한됩니다. 명령, 네트워크, 비밀값 접근, 아티팩트 캡처, 차단, 격리 사실은 승격된 역량 프로필이 필요합니다. |
...
| 로컬 접근 불일치 | 호출자, 경로, 태세가 등록된 로컬 프로필 밖이거나 로컬 접근이 철회되었을 때. | 표시해도 안전한 진단과 함께 `LOCAL_ACCESS_MISMATCH`를 사용합니다. 접점별 `UNAUTHORIZED` 코드를 만들지 않습니다. |
