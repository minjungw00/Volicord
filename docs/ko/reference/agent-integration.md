...
- 위협 모델과 보장 라벨 의미. [보안 참조](security.md)를 봅니다.
...
보장 라벨 표시는 [보안 참조](security.md#정직한-guarantee-display)를 따릅니다. 정확한 스키마 값 집합은 [API Schema Core](api/schema-core.md#current-mvp-value-sets)가 담당합니다. 이 참조는 커넥터가 `capability_profile`을 사용자에게 보이는 표시로 어떻게 연결하는지 담당합니다.
...
에이전트는 사용자가 더 강한 안전성을 요청했거나, guard/freeze/careful mode를 요청했거나, 대화에서 더 강한 표현을 썼다는 이유만으로 `preventive` 또는 `isolated`를 고르면 안 됩니다. 활성 프로필이 더 강한 주장을 증명하지 못하면 커넥터는 보장 라벨을 낮추거나 `CAPABILITY_INSUFFICIENT`를 반환해야 합니다.
...
| 역량 부족 | 요청한 쓰기, 캡처, guard, 격리, 보장이 지원하지 않는 필드에 의존할 때. | `CAPABILITY_INSUFFICIENT` 또는 구조화된 차단 사유를 반환하고 보장 라벨을 낮춥니다. |
