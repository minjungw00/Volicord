...
접점 버전, MCP 설정, 권한, 작업공간 정책, 생성 파일, 관리형 블록, `stage_artifact` 경로, 가림 정책, 아티팩트 보존 방식, 로컬 접근 태세, 적합성 근거가 바뀌면 `capability_profile`을 갱신합니다. 후크, 접점 자체 캡처, 도구 실행 전 차단, 격리 지원은 승격 전까지 later/profile-gated로 남습니다.
