...
접점 버전, MCP 설정, 권한, 작업공간 정책, 생성 파일, 관리형 블록, `harness.stage_artifact` 경로, 수동 첨부 지원, 가림 정책, 아티팩트 보존 방식, 로컬 접근 태세, 적합성 근거가 바뀌면 `capability_profile`을 갱신합니다. 하지만 프로필 갱신만으로 `LocalSurfaceRegistration`이 새로 고쳐지지는 않습니다. 등록 변경은 현재 로컬 접점 맥락을 확인하는 서버 담당 경로가 필요합니다. 후크, 접점 자체 캡처, 도구 실행 전 차단, 격리 지원은 승격 전까지 later/profile-gated로 남습니다.
...
기준 로컬 MCP 프로필은 기본적으로 협력형입니다. 변경 경로 관찰이 지원되고 관련 역량 확인이 통과한 곳에서만 제한된 `detective` 동작을 표시할 수 있습니다. 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 접점 자체 아티팩트 캡처, 도구 실행 전 차단, 격리는 없습니다. `harness.stage_artifact`를 통한 수동 아티팩트 첨부는 접점을 아티팩트 캡처 프로필로 바꾸지 않습니다. `preventive`나 `isolated` 동작을 주장하면 안 됩니다.
...
- `harness.stage_artifact`가 같은 프로젝트/같은 Task에 묶인 임시 스테이징 핸들만 만들고 Core 상태를 만들지 않는지 확인
