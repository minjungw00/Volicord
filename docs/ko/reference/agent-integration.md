...
토큰을 아끼더라도 올바른 동작에 필요한 사용자 소유 판단, blocker, scope limit, safety boundary, close-relevant residual-risk 정보를 빼면 안 됩니다. 특히 사용자 판단 요청은 사용자가 충분히 판단할 수 있도록 정확한 질문, judgment category, route, display depth, 간결한 options 또는 chosen outcome, 영향을 받는 범위, 관련 refs, 답변이 확정하지 않는 것을 포함해야 합니다. High-risk 또는 close-affecting prompt는 추가로 recommendation, uncertainty, affected gates 또는 acceptance criteria, deferral consequence, route-specific context를 포함합니다.
...
| 사용자 판단 요청 | 정확한 사용자 소유 판단, judgment category, internal route, display depth, `display_depth`에 맞는 options 또는 chosen outcome, 영향을 받는 범위, 관련 참조 또는 명시적 absence, 답변이 확정하지 않는 것, 답변 뒤의 next action. High-risk 또는 close-affecting prompt는 trade-offs, 추천, 불확실성, affected gates/acceptance criteria, deferral consequence도 보여줍니다. | [에이전트 세션 흐름: 사용자 소유 판단으로 막힐 때](../use/agent-session-flow.md#사용자-소유-결정으로-막힐-때), [커널: Decision Packet](kernel.md#decision-packet), [Decision Gate](kernel.md#decision-gate), 정확한 API field가 필요할 때만 [`harness.request_user_judgment`](mcp-api-and-schemas.md#harnessrequest_user_judgment). | 포괄적 approval 표현, 관련 없는 judgment, 전체 근거 본문, 전체 log, 전체 schema reference, 전체 Template 세트. | 사용자가 충분히 판단할 수 있게 exact choice, options, 필요할 때 recommendation, scope, 참조, uncertainty, 답변이 확정하지 않는 것을 보여줍니다. | Current Decision Packet 또는 current state-derived judgment candidate에서 가져옵니다. Ref가 stale 또는 missing이면 묻기 전에 표시합니다. Replay id, routing metadata, internal enum detail은 경계를 설명할 때만 보조로 둡니다. |
