...
| `MVP-ACTIVE-record-run-promotes-staged-artifact-to-artifact-ref` | 호환되는 `run_recording`의 `harness.record_run`만 현재 확인된 `surface_instance_id`에서 만든, 만료되지 않은 같은 프로젝트/같은 Task의 `StagedArtifactHandle`을 소비해 지속 `ArtifactRef`로 승격할 수 있습니다. | `record_run`은 누락, 만료, 불일치, 이미 소비됨, 다른 프로젝트, 다른 Task, cross-surface, 잘못된 `created_by_surface_instance_id`, 잘못된 `sha256`, 잘못된 `size_bytes`, 잘못된 `redaction_state`, 그 밖의 무결성 불일치 handle을 `VALIDATION_FAILED`와 `ToolError.details.artifact_input_error`로 거부하며, Run, artifact, 증거 업데이트, 승인 소비, 재실행 행, 상태 버전 증가를 만들지 않습니다. 유효하게 소비된 handle은 staging된 `sha256`, `size_bytes`, `content_type`, `redaction_state`, project, Task, 서버가 기록한 생성 접점 출처 기록과 맞는 등록 artifact 메타데이터와 담당 연결을 만듭니다. 이 승격은 `artifact_registration`을 요구하지 않습니다. 그 접근 분류는 앞선 `harness.stage_artifact` 요청에서만 사용됩니다. 현재 MVP는 접점 간(cross-surface) staged artifact handoff를 지원하지 않으며, 스테이징된 핸들 불일치는 요청 수준 접점 검증이나 역량 확인 자체가 실패한 경우가 아니라면 `LOCAL_ACCESS_MISMATCH`나 `CAPABILITY_INSUFFICIENT`가 아닙니다. |
