...
| <a id="scenario-baseline-mcp-semantic-descriptor-conformance"></a>`BASELINE-mcp-semantic-descriptor-conformance` | 모든 프로덕션 도구는 결정적인 의미 descriptor 하나를 가지며, 광고된 typed example은 검증되고 정확한 request type으로 decode되며 선언된 중첩 tagged-union variant를 선택합니다. | [MCP 전송](mcp-transport.md#generated-semantic-schema-catalog); [테스트 전략](../architecture-guide/testing-strategy.md) |
