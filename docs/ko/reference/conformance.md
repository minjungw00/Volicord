...
| <a id="scenario-baseline-mcp-committed-mutation-recovery-preserves-authority"></a>`BASELINE-mcp-committed-mutation-recovery-preserves-authority` | 커밋된 MCP mutation 뒤의 제한된 복구는 현재 authority 의미를 보존하고 mutation을 다시 시도하지 않습니다. | [MCP 전송](mcp-transport.md#mutation-authority-receipt-projection); [API 코어 스키마](api/schema-core.md); [저장 효과](storage-effects.md) |
