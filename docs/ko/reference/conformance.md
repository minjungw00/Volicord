...
| `MVP-ACTIVE-close-task-complete-blocker-matrix` | `close_task intent=complete`는 Task 유효성부터 복구 제약까지 결정적 차단 순서를 적용한 뒤 완료를 커밋합니다. | 응답 차단 사유는 Core 행렬 순서를 따릅니다. 빠진 필수 증거는 최종 수락이나 잔여 위험 수락보다 먼저 막습니다. 최종 수락과 잔여 위험 수락은 필수 증거나 필수 아티팩트를 대신하지 않습니다. 차단 사유가 남아 있으면 Task는 열린 상태로 남습니다. |
