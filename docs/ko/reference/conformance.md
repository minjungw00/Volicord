...
| <a id="scenario-baseline-managed-final-output-refreshes-current-authority"></a>`BASELINE-managed-final-output-refreshes-current-authority` | 읽기 전용 바인딩 검증 뒤 진행하는 적격 내장 관리 어댑터의 최종 출력 전달은 정확한 재생과 기능 지원 상태가 `verified`가 아닌 최선형 전달까지 포함해, 변경 불가능한 과거 Stop 결정을 바꾸지 않고 새로 읽은 현재 상태 receipt를 매번 검증합니다. | [상태 보기와 템플릿](projection-and-templates.md#managed-final-output-authority-disclosure); [Agent Connection](agent-connection.md#managed-final-output-authority-disclosure); [관리 CLI](admin-cli.md#managed-final-output-authority-disclosure) |
| <a id="scenario-baseline-managed-final-output-uses-bounded-status-fallback"></a>`BASELINE-managed-final-output-uses-bounded-status-fallback` | receipt를 사용할 수 없거나, 잘못되었거나, 너무 크면 크기가 제한된 정확한 `volicord status` 경로로 바꾸며 receipt JSON 일부를 내보내지 않습니다. | [템플릿 본문](template-bodies.md#final-output-authority-disclosure-body); [상태 보기와 템플릿](projection-and-templates.md#managed-final-output-authority-disclosure); [보안](security.md#generated-displays-and-text) |
