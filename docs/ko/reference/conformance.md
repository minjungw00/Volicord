...
| `MVP-ACTIVE-prepare-write-committed-scoped-authorization` | 허용되어 committed 처리된 `prepare_write`는 범위가 정해진 1회용 Write Authorization을 기록합니다. | 응답의 승인 범위, Core 상태, `write_authorizations.attempt_scope_json`이 Task, Change Unit, 상태 버전, 접점, 의도한 제품 파일 경로, 민감 범주, 기준 참조, 관련 판단, 보장 표시 수준에서 같은 의미를 가집니다. |
