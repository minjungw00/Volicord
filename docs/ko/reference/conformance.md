...
이 적합성 예시는 네 응답/차단 사유 구조를 분리합니다. 허용되지 않은 `prepare_write` 결과는 `PrepareWriteResult.write_decision_reasons: WriteDecisionReason[]`로 prepare_write 판단 사유를 설명하며, `CloseBlocker`는 `prepare_write`에 쓰지 않습니다. 커밋된 `close_task` 닫기 차단 결과는 `CloseTaskResult(close_state=blocked).blockers: CloseBlocker[]`로 닫기 차단 사유 행렬을 설명합니다. 유효한 dry-run 미리보기는 `DryRunSummary.would_blockers: PlannedBlocker[]`로 dry-run 예상 차단 사유를 보여 주며 실제/현재 `CloseBlocker`나 `WriteDecisionReason`을 저장하지 않습니다. 커밋 전 실패는 `ToolRejectedResponse.errors`를 쓰는 거절 응답이고, `errors[0].code`에 관련 `ErrorCode`를 둡니다. 이 거절 응답은 상태 효과 없음이어야 합니다. `WriteDecisionReason.code`, `CloseBlocker.code`, `PlannedBlocker.code`는 `STATE_VERSION_CONFLICT`가 아니어야 합니다.
...
| `MVP-ACTIVE-prepare-write-requires-compatible-scope-and-approval` | `harness.prepare_write`는 의도한 제품 파일 쓰기가 활성 Task, Change Unit, 범위, 기준선, Autonomy Boundary, 필요한 사용자 판단, 필요한 별도 민감 동작 승인, 접점 역량, 현재 프로젝트 전체 상태 버전과 호환될 때만 `decision=allowed`를 반환할 수 있습니다. | 커밋된 허용되지 않은 결과의 예상 응답 분기는 `PrepareWriteResult`이고 `decision=blocked`, `decision=approval_required`, 또는 `decision=decision_required`를 씁니다. `write_decision_reasons: WriteDecisionReason[]`는 비어 있으면 안 되며, `WriteDecisionReason.code != STATE_VERSION_CONFLICT`입니다. `CloseBlocker`는 쓰지 않고, `close_state`를 바꾸지 않고, 닫기 차단 사유 행렬을 실행하지 않고, `close_task` 효과를 만들지 않습니다. 프로젝트 전체 버전 불일치는 `errors[0].code=STATE_VERSION_CONFLICT`인 `ToolRejectedResponse` 거절 응답이며 `PrepareWriteResult.decision` 값이 아닙니다. 허용되지 않은 결과는 소비 가능한 Write Authorization, Run, 아티팩트, 증거, 최종 수락, 잔여 위험, 닫기 상태를 만들지 않습니다. 유효한 `dry_run` 미리보기는 `DryRunSummary.would_blockers: PlannedBlocker[]`를 쓰며 저장된/현재 `WriteDecisionReason`이나 `CloseBlocker`를 만들지 않고, `PlannedBlocker.code != STATE_VERSION_CONFLICT`입니다. |
...
| `MVP-ACTIVE-close-task-complete-stale-state-version-rejected` | `harness.close_task intent=complete`에서 `ToolEnvelope.expected_state_version`이 현재 프로젝트 전체 `project_state.state_version`보다 오래됐으면, complete 닫기 차단 사유 행렬을 실행하기 전에 `close_task` 사전 확인에서 커밋 전 실패가 됩니다. | 예상 응답 분기는 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답이고 `errors[0].code=STATE_VERSION_CONFLICT`입니다. `CloseTaskResult`가 아닙니다. 거절된 시도는 `CloseBlocker` 없음, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 생성 없음, Write Authorization 소비 없음, 스테이징된 핸들 소비 없음, 아티팩트 승격 또는 연결 없음, 증거 업데이트 없음, 상태 버전 증가 없음(`project_state.state_version` 증가 없음)을 유지해야 합니다. |
| `MVP-ACTIVE-close-task-complete-stale-write-authorization-basis-rejected` | `harness.close_task intent=complete`의 사전 확인에서 닫기와 관련된 `WriteAuthorization.basis_state_version`이 현재 `project_state.state_version`보다 오래됐다고 확인되면, 닫기 차단 사유 행렬 커밋이나 Write Authorization 소비 전에 커밋 전 실패가 됩니다. | 예상 응답 분기는 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답이고 `errors[0].code=STATE_VERSION_CONFLICT`입니다. `CloseTaskResult`가 아닙니다. 거절된 시도는 `CloseBlocker` 없음, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 생성 없음, Write Authorization 소비 없음, 스테이징된 핸들 소비 없음, 아티팩트 승격 또는 연결 없음, 증거 업데이트 없음, 상태 버전 증가 없음(`project_state.state_version` 증가 없음)을 유지해야 합니다. |
| `MVP-ACTIVE-close-task-blocks-current-write-compatibility` | `harness.close_task intent=complete`에서 사전 확인이 현재 `ToolEnvelope.expected_state_version`과 닫기 관련 `WriteAuthorization.basis_state_version`의 최신성을 확정한 뒤에도, complete 닫기 차단 사유 행렬은 의미적 Write Authorization 또는 제품 쓰기 Run 비호환을 찾을 수 있습니다. | 예상 응답 분기는 커밋된 닫기 차단 결과인 `CloseTaskResult`입니다. `CloseTaskResult.close_state=blocked`와 `CloseTaskResult.blockers: CloseBlocker[]`를 사용합니다. 차단 사유는 `CloseBlocker.category=write_compatibility`와 `CloseBlocker.code != STATE_VERSION_CONFLICT`를 가져야 합니다. `CloseBlocker.code`는 `WRITE_AUTHORIZATION_REQUIRED`, `WRITE_AUTHORIZATION_INVALID`, `SCOPE_VIOLATION`, `BASELINE_STALE`, `VALIDATOR_FAILED` 같은 의미적 차단 사유 코드여야 하며 Task는 열린 상태로 남습니다. |
| `MVP-ACTIVE-close-task-blocks-evidence-insufficient` | `harness.close_task intent=complete`는 활성 `CompletionPolicy`에 따라 필수 증거가 없거나, 부분적이거나, 오래됐거나, 차단됐거나, 충분하지 않으면 차단됩니다. | 예상 응답 분기는 `CloseTaskResult`이고 `CloseTaskResult.close_state=blocked`와 `CloseTaskResult.blockers: CloseBlocker[]`를 사용합니다. 차단 사유는 `CloseBlocker.category=evidence`, `CloseBlocker.code != STATE_VERSION_CONFLICT`, 선택적 주 오류 `EVIDENCE_INSUFFICIENT`를 포함할 수 있습니다. Task는 열린 상태로 남습니다. 최종 수락과 잔여 위험 수락은 `EvidenceCoverageItem.required_for_close=true`인 필수 증거 공백을 충분한 증거로 바꾸지 못합니다. |
| `MVP-ACTIVE-close-task-blocks-required-artifact-unavailable` | `harness.close_task intent=complete`는 닫기에 필요한 `ArtifactRef`가 누락, 사용 불가, 무결성 실패, 허용된 안전 공지를 넘어선 차단, 또는 사용할 수 없는 상태이면 차단됩니다. | 예상 응답 분기는 `CloseTaskResult`이고 `CloseTaskResult.close_state=blocked`와 `CloseTaskResult.blockers: CloseBlocker[]`를 사용합니다. 차단 사유는 `CloseBlocker.category=artifact_availability`, `CloseBlocker.code != STATE_VERSION_CONFLICT`, 선택적 주 오류 `ARTIFACT_MISSING`을 포함할 수 있습니다. Task는 열린 상태로 남고 최종 닫기 상태는 커밋되지 않습니다. 증거 충분성과 아티팩트 가용성은 별개이며, 최종 수락이나 잔여 위험 수락은 필요한 아티팩트를 대신하지 않습니다. |
| `MVP-ACTIVE-close-task-blocks-final-acceptance-missing` | `harness.close_task intent=complete`는 증거와 닫기 관련 아티팩트가 통과한 뒤에도 필요한 `final_acceptance`가 없거나, 거절됐거나, 오래됐거나, 보이는 닫기 근거와 연결되지 않았으면 차단됩니다. | 예상 응답 분기는 `CloseTaskResult`이고 `CloseTaskResult.close_state=blocked`와 `CloseTaskResult.blockers: CloseBlocker[]`를 사용합니다. 차단 사유는 `CloseBlocker.category=final_acceptance`, `CloseBlocker.required_judgment_kind=final_acceptance`, `CloseBlocker.code != STATE_VERSION_CONFLICT`, 선택적 주 오류 `ACCEPTANCE_REQUIRED`를 포함할 수 있습니다. Task는 열린 상태로 남으며, 증거, 상태 문구, 채팅, 잔여 위험 수락에서 최종 수락을 추론하지 않습니다. |
| `MVP-ACTIVE-close-task-blocks-visible-unaccepted-residual-risk` | `harness.close_task intent=complete`는 닫기에 영향을 주는 잔여 위험이 보이지만 호환되는 `residual_risk_acceptance`가 없으면 차단됩니다. | 예상 응답 분기는 `CloseTaskResult`이고 `CloseTaskResult.close_state=blocked`와 `CloseTaskResult.blockers: CloseBlocker[]`를 사용합니다. 차단 사유는 `CloseBlocker.category=residual_risk_acceptance`, `CloseBlocker.required_judgment_kind=residual_risk_acceptance`, `CloseBlocker.code != STATE_VERSION_CONFLICT`, `DECISION_REQUIRED` 또는 `DECISION_UNRESOLVED`를 포함합니다. Task는 열린 상태로 남습니다. 이 경우는 사용자가 판단할 만큼 위험이 보이지 않을 때의 `RESIDUAL_RISK_NOT_VISIBLE`과 구분됩니다. |
...
| `MVP-ACTIVE-close-task-state-effecting-dry-run-preview` | `harness.close_task`는 혼합 intent 메서드입니다. 응답 분기 선택은 메서드 이름만이 아니라 선택된 intent의 상태 효과를 따릅니다. `intent=complete`, `intent=cancel`, `intent=supersede`와 `dry_run=true`가 함께 있고 그 외에는 유효하며 미리보기 가능하면 상태 효과가 있는 `dry_run` 분기를 선택합니다. | 예상 응답 분기는 그런 유효한 상태 효과 `dry_run` 미리보기에 대한 `effect_kind=no_effect`인 `ToolDryRunResponse`입니다. 미리보기는 `DryRunSummary.would_blockers: PlannedBlocker[]`로 예상 닫기 차단 사유를 설명합니다. 실제/현재 `CloseBlocker`나 `WriteDecisionReason`을 저장하거나 반환하지 않고, `PlannedBlocker.code != STATE_VERSION_CONFLICT`입니다. 닫기 상태 변경, `task_events`, 재실행 행, 아티팩트 생성·갱신·연결, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version` 증가를 만들지 않습니다. |
