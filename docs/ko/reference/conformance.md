...
|---|---|---|
| <a id="scenario-baseline-mcp-supported-profile-capability-conformance"></a>`BASELINE-mcp-supported-profile-capability-conformance` | 프로덕션 지원 MCP profile은 선언된 semantic capability를 통해 같은 적용 가능한 initialize, lifecycle, discovery, call-result, schema, batching 시나리오를 충족합니다. | [MCP 전송](mcp-transport.md#protocol-revision-negotiation); [Agent Connection](agent-connection.md) |
| <a id="scenario-baseline-mcp-semantic-descriptor-conformance"></a>`BASELINE-mcp-semantic-descriptor-conformance` | 모든 프로덕션 도구는 결정적인 의미 descriptor 하나를 가지며, 광고된 typed example은 검증되고 정확한 request type으로 decode되며 선언된 중첩 tagged-union variant를 선택합니다. 잘못됐거나 없는 discriminator는 추측한 branch 없이 해당 union의 discriminator issue만 만들며 descriptor/decode 불일치는 conformance를 실패시킵니다. | [MCP 전송](mcp-transport.md#public-argument-projection); [테스트 전략](../architecture-guide/testing-strategy.md) |
| <a id="scenario-baseline-workflow-model-liveness-recovery"></a>`BASELINE-workflow-model-liveness-recovery` | 결정적인 reachable-state 탐색은 고유한 catalog action key, required member 무결성, 정확한 transition 소유 Core admission, nonterminal liveness, 현재 typed recovery membership, 완전한 Agent transition/action-form parity, 게시된 모든 form이 정확한 메서드의 수락된 Core no-commit plan에 도달하는 성질을 보존합니다. | [Core 모델](core-model.md); [API 상태 스키마](api/schema-state.md); [MCP 전송](mcp-transport.md#public-argument-projection); [테스트 전략](../architecture-guide/testing-strategy.md) |
| <a id="scenario-baseline-mcp-unsupported-revision-rejected"></a>`BASELINE-mcp-unsupported-revision-rejected` | 알 수 없거나 추적만 하고 프로덕션에서 지원하지 않는 MCP revision은 다른 profile로 대체하지 않고 거절합니다. | [MCP 전송](mcp-transport.md#protocol-revision-negotiation); [Agent Connection](agent-connection.md) |
| <a id="scenario-baseline-mcp-mutation-finalization-preserves-effect-branches"></a>`BASELINE-mcp-mutation-finalization-preserves-effect-branches` | 효과 전 내부 실패는 일반 no-effect 오류로 유지됩니다. 효과 뒤 복구는 새 commit, staging, replay의 정확한 fact를 보존하고 mutation retry를 금지하며 현재 status를 요구합니다. 정상 no-effect 결과와 typed rejection은 서로 다른 branch로 유지됩니다. | [MCP 전송](mcp-transport.md#mutation-authority-receipt-projection); [API 오류 처리 경로](api/error-routing.md); [저장 효과](storage-effects.md) |
...
| <a id="scenario-baseline-shaping-readiness-gap-blocks-or-asks"></a>`BASELINE-shaping-readiness-gap-blocks-or-asks` | 구체화 공백은 별도 계획 아티팩트가 아니라 담당 문서가 정의한 차단 사유나 판단 후보로 남습니다. | [Core 모델](core-model.md); [API 상태 스키마](api/schema-state.md); [상태 메서드](api/method-status.md); [판단 요청](api/method-request-user-action.md); [판단 기록](api/method-resolve-user-action.md) |
...
| <a id="scenario-baseline-write-ticket-attempt-scope-bounded-intent"></a>`BASELINE-write-ticket-attempt-scope-bounded-intent` | `WriteTicketAttemptScope`는 경계가 정해진 제품 파일 쓰기 의도 하나 또는 유효 `sensitive` 통제 아래의 정확한 승인 결속 비제품 동작 하나를 다룹니다. | [Core 모델](core-model.md); [쓰기 준비 메서드](api/method-prepare-write.md); [API 판단 스키마](api/schema-judgment.md) |
