...
| `MVP1-projection-template-output-not-state` | Projection, template, 사용자용 작은 출력(`status-card`, `judgment-request`, `run-evidence-summary`, `close-result`), 에이전트용 `agent-context-packet`, Markdown output은 derived 상태로 남습니다. 이를 읽거나 편집해도 state 생성, gate 충족, write authorize, evidence attach, 최종 수락 기록, 잔여 위험 수락, Task close가 일어나지 않습니다. 사용자용 출력은 agent authority로 쓰면 안 되고, 에이전트 패킷은 사용자 상태 표시처럼 보여주면 안 됩니다. |
