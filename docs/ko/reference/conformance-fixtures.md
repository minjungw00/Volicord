...
| `MVP1-unresolved-user-judgment-blocks-write-or-close` | 관련 product, UX, architecture, sensitive-action, work-acceptance, residual-risk judgment가 missing, pending, rejected, blocked, stale, incompatible이면 affected write 또는 close가 structured Core/API result로 막히고, action이 필요한 judgment ref를 보여줍니다. |
...
| `MVP1-residual-risk-visible-before-acceptance-risk-close` | Known close-relevant residual risk는 successful work acceptance 또는 close 전에 보여야 합니다. Hidden 또는 stale risk는 `residual_risk_visibility`를 통해 해당 route를 막습니다. |
| `MVP1-accepted-risk-close-requires-explicit-risk-acceptance` | `completed_with_risk_accepted`는 risk가 visible이고 compatible `judgment_type=residual_risk_acceptance` user judgment가 관련 blocker/evidence ref와 함께 수용을 기록할 때만 성공합니다. Work acceptance나 broad "go ahead" text는 이를 충족하지 않습니다. |
