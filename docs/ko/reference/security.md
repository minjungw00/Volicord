...
간결한 사람용 출력은 해당 명령과 현재 branch에 적용되는 fact만 선택할 수 있습니다.
Verbose 사람용 출력은 모두 성공한 typed evidence를 요약하고 실패, 불완전, unavailable,
contradictory evidence를 펼칠 수 있으며 JSON은 완전한 typed disclosure로 유지됩니다.
사람용 요약에서 성공한 식별자나 path를 생략하거나 적용되지 않는 필드를 생략하거나
