...
Volicord 보안 표현은 문서화된 Volicord 경로 안의 권한·증거 기록과 정책 경계를
설명합니다. 운영체제 샌드박스, 파일시스템 권한 시스템, 변조 불가능한 감사 로그,
정확성 증명, 악성코드 검사기, 네트워크 격리 계층, 완전한 호스트 신뢰 강제 시스템,
일반 호스트 정책 엔진을 설명하지 않습니다.
...
- 쓰기 뒤 기록한 최종 수락이 필요한 쓰기 전 민감 동작 승인을 대신하거나 쓰기에
  소급해 권한을 부여한다는 주장.
...
Guard는 정책 결속이 없거나 일치하지 않는 티켓을 `current_write_ticket_ids`와
`active_write_tickets`에서 제외하고 `stale_write_ticket_ids`로 보고합니다. 이러한
오래된 티켓만 그 밖에는 결정적인 직접 제품 쓰기를 덮는다면 결정은
`write_ticket_backing.status=policy_authority_stale`, `ticket_backed=false`, 거부 사유
`write_ticket_policy_changed`를 사용합니다. 이 협력형 Guard 계층만 검사하는 것은
아닙니다. [`volicord.record_run`](api/method-record-run.md)은 Run을 만들기 전에 현재 정책
결속을 독립적으로 다시 읽고 검증하며 Store는 티켓 소비 안에서 다시 확인합니다. 따라서
Guard를 우회해도 오래된 티켓을 사용할 수 없으며, 정확한 거절 동작은 메서드 담당 문서가
정의합니다.
