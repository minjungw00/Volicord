...
기준 `reference-local-mcp` 프로필에서 Write Authorization과 제품 쓰기 Run 호환성은 경로 수준입니다. 이 프로필은 기본적으로 협력형이고, 관련 역량 확인이 통과한 뒤 관찰된 변경 경로에 대해서만 제한된 탐지형 지원을 가집니다. 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 접점 자체 아티팩트 캡처, 도구 실행 전 차단, 격리는 없습니다. `manual_artifact_attachment_supported=true`는 안전한 바이트 또는 안전한 알림을 위한 활성 `harness.stage_artifact` 스테이징 경로를 사용할 수 있다는 뜻입니다. 커넥터 아티팩트 캡처가 아니며 아티팩트가 어떻게 만들어졌는지 검증하지 않습니다.
