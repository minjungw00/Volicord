...
- `harness.stage_artifact`가 안전한 바이트 또는 안전한 알림에 대해 같은 프로젝트/같은 Task에 묶인 임시 스테이징 핸들만 만듭니다.
