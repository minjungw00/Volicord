...
저장하면 안 됩니다. 읽기 또는 점검 경계가 첫 watcher baseline을 만들면 baseline
메타데이터는 coverage 시작과 `method_boundary` coverage basis를 부분 coverage 경고와
함께 기록합니다. 그 경계 전의 Product Repository 변경은 watcher coverage 밖에 있습니다.
