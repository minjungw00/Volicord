...
Status는 read-only입니다. State를 만들거나, 제품 파일 쓰기를 compatible하게 만들거나, Write Authorization을 만들거나, gate를 충족하거나, work acceptance를 기록하거나, residual risk를 받아들이거나, projection repair를 enqueue하거나, Task를 close하면 안 됩니다.
