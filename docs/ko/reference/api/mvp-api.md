...
`CloseTaskResponse`의 응답 분기는 intent별로 정합니다. `CloseTaskResult`는 `intent=check` 읽기 전용 결과에 사용하며, `intent=check`와 `dry_run=true`가 함께 있어도 이 응답 분기를 유지합니다. 또한 non-dry-run으로 커밋된 `intent=complete`, `intent=cancel`, `intent=supersede` 결과와 커밋된 차단 닫기 결과에도 `CloseTaskResult`를 사용합니다. `ToolDryRunResponse`는 `intent=complete`, `intent=cancel`, `intent=supersede`의 유효한 dry-run 미리보기에서만 사용하고 `intent=check`에는 사용하지 않습니다. 커밋 전 실패는 intent와 `dry_run` 값과 무관하게 `ToolRejectedResponse`입니다.
