...
모든 `ToolError`에는 `details` 필드가 있습니다. 기계 판독용 세부사항 객체가
없으면 값은 `null`이고, 있으면 아래에서 설명하는 담당 원본 정의 진단 사실을 담은
객체입니다. 이를 감싸는 `ToolError` 객체는 닫혀 있습니다. 정확한 필수 여부,
null 허용 여부, 필드 형식은 [API 코어 스키마](schema-core.md#common-response)의
생성 구조에서 가져옵니다.
