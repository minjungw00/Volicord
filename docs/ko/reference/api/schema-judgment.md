...
레거시 지속 선택지 표현은 호환성 데이터이며 공개 요청 스키마가 아닙니다. 이 표현에서 요청 필드를 추론하면 안 됩니다.
...
`machine_action`과 `resolution_outcome`은 선택된 `UserJudgmentOption`에서 복사됩니다. 선택된 선택지의 저장 동작과 결과가 기준입니다. 새 `status=resolved` 판단은 `null`이 아닌 `machine_action`과 `null`이 아닌 `resolution_outcome`을 가집니다. `machine_action`의 null 허용은 레거시 감사 상태 보기만 보존합니다. `answer` 안의 결과, 결정, 수락 필드는 선택된 선택지와 일치해야 합니다. 자유 형식 답변 텍스트는 권한을 부여할 수 없습니다.
