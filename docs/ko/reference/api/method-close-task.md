...
3. [`volicord.status`](method-status.md)에서 증거나 닫기 상태 보기를 선택했을 때와 같은 방식으로 현재 닫기 준비 상태와 기준 `EvidenceGateSummary`를 계산하고 `CloseTaskResult`를 반환합니다. 최상위 gate, `state.evidence_gate`, `summary_card.evidence`는 이 하나의 결과를 재사용합니다.
...
<!-- BEGIN GENERATED: contract-structures api.method.check_close.response[response_variants] api.method.check_close.response[result_body] api.method.check_close.response[rejection] api.method.check_close.response[dry_run] api.method.close_task.response[response_variants] api.method.close_task.response[result_body] api.method.close_task.response[rejection] api.method.close_task.response[dry_run] -->
<!-- Generated by `cargo run -p xtask -- docs-sync`; do not edit this region. -->
