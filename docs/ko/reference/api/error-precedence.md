...
데이터를 `Unavailable`로 다시 분류하지 않고 `UnsupportedContract`를 `Rejected` 요청
검증으로 다시 분류하지 않습니다. 핵심 동작을 계속하는 `Degraded` 진단은 이 표에
