...
- `ToolResultBase`에는 결과 메타데이터만 둡니다. `response_kind`는 정확히
  ...
  `staging_created`, `no_effect` 중 하나입니다. `dry_run=true`인 결과는
  `read_only` 결과이며, 그 밖의 결과 효과에는 `dry_run=false`가 필요합니다.
- `ToolRejectedResponse`는 `base: ToolRejectedBase`와 `errors`를 담습니다.
  `ToolRejectedBase.response_kind`는 정확히 `rejected`이고
  `ToolRejectedBase.effect_kind`는 정확히 `no_effect`입니다. `dry_run`은 요청
  모드를 기록하므로 `true` 또는 `false`일 수 있습니다.
