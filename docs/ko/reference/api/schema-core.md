...
- `ToolEnvelope`
- 공통 메서드 결과 분기 모델
- `read_only`, `core_committed`, `staging_created`, `no_effect`의 정확한 결과
  메타데이터 구조
- `ToolRejectedBase`
- `ToolDryRunBase`
...
- 모든 구체 메서드 결과는 메서드별로 정확한 자체 `base` 형식과 해당
  메서드 결과 필드만 담습니다. 기준 메서드 선언이 도달 가능한 메타데이터
  variant를 선택하므로 한 메서드 스키마에는 다른 메서드의 효과가 들어가지
  않습니다.
- 모든 결과 메타데이터 variant의 `response_kind`는 `result`입니다.
  `read_only` variant는 일반 결과 메서드에서만 요청된 dry-run 의도를
  보존하며, 그 밖의 성공 결과 메타데이터는 `dry_run=false`를 요구합니다.
  `read_only`, `staging_created`, `no_effect`의 `events` 배열은 비어 있어야
  하고(`maxItems: 0`), `core_committed`는 권한 이벤트 참조를 하나 이상
  요구합니다(`minItems: 1`). 성공 결과 메타데이터의 `state_version`은
  null일 수 없습니다.
...
- 아래의 정확한 공유 메타데이터 구조는 공개 catch-all 결과 base를 이루지
  않습니다. 각 메서드별 base에는 선언된 variant만 포함됩니다.
...
### `NotRequestedReadOnlyResultBase` 필드
