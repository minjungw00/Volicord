...
쓰기 준비에는 `direct/implementation` 또는 `work/implementation`, 현재 active Change Unit, 그리고 `scope_update`, `advance_task`, `prepare_write`에 필요한 pending, 거부, 보류, 만료, 불일치 상태의 현재 shaping 결정이 없어야 합니다. shaping에서 만든 Change Unit은 단계 gate를 만족하지 않습니다.
...
현재 읽기 경로는 원시 UserAction 권한 fact를 정규 Write Ticket 승인 담당자에게
직접 전달합니다. 이 담당자만 typed 현재 민감 승인 집합을 비공개로 구성하고, typed
발급 근거 또는 Store가 검증한 영속 근거에 대한 단일 의미 평가를 반환합니다. 원시
승인 fact는 공유 현재 유효성 fact에 보관되지 않으며 티켓 선택이나 재사용에
제공되지 않습니다. 재사용은 현재 근거나 승인이 필요하지 않은 근거를 허용하고 변경된
근거를 거절합니다. 민감 티켓을 재사용하려면 정규화된
...
원시 UserAction 권한을 받거나 승인 참조나 resolution ID를 독립적으로 비교하지
않고 이 typed 평가 또는 파생된 평가 상태를 사용합니다.
