...
| `inbox_item` | 같은 대기 판단을 사용자에게 보여 주는 `JudgmentInboxItem`입니다. 판단 식별자, 질문, 선택지 또는 답변 제약, 필수 여부, User Channel 답변 경로의 사용 가능 상태, 사용할 수 있을 때의 선호 답변 경로, 로컬 동의 URL이나 `volicord inbox answer <judgment-id> --choice <choice>` 같은 대체 경로를 포함합니다. 형태는 [API 판단 스키마](schema-judgment.md#judgmentinboxitem)가 담당합니다. |
