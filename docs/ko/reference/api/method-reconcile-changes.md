...
- 미기록 변경이 해결되거나 사용자 행동이 생성되면 `base.effect_kind=core_committed`
...
| `summary_card` | 선택된 조정 결과에 대한 `SummaryCard`입니다. 기록, 변경, 대기 사용자 행동, 닫기 상태, 전송, 선택된 다음 행동 하나, 보장 줄을 요약하며 구조화된 결과 필드 너머의 권한을 추가하지 않습니다. 형태는 [API 상태 스키마](schema-state.md#current-position-display-shapes)가 담당합니다. |
