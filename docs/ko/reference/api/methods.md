...
이 문서는 공개 Volicord API 메서드 동작을 담당하는 집중 참조 문서를 찾기 위한 메서드 경로 안내입니다. 정확한 메서드 계약은 연결된 메서드 담당 문서에 있습니다.
...
| 표면 | 안정성 | 비고 |
|---|---|---|
| 아래 표의 지원되는 공개 메서드 이름 | `stable` | 이 Core 권한 메서드들이 공개 API 메서드 집합입니다. |
