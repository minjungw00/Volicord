...
<!-- BEGIN GENERATED: contract-structures api.method.prepare_evidence_capture.response[response_variants] api.method.prepare_evidence_capture.response[result_body] api.method.prepare_evidence_capture.response[result_metadata] api.method.prepare_evidence_capture.response[rejection] api.method.prepare_evidence_capture.response[dry_run] -->
<!-- This region is generated from maintained sources; do not edit it directly. -->

### `PrepareEvidenceCaptureResult` 성공 필드

| 필드 | 필수 | Null 허용 | 형식 |
|---|---|---|---|
| `base` | 예 | 아니요 | `PrepareEvidenceCaptureResultBase` |
| `capture_intent` | 예 | 아니요 | `EvidenceCaptureIntent` |
| `capture_intent_ref` | 예 | 아니요 | `StateRecordRef` |
...
- `volicord.prepare_evidence_capture`: `dry_run=true`가 `ToolDryRunResponse` 미리보기 분기를 선택하며, 이 분기의 `base.dry_run`은 `true`입니다. `dry_run=false`이거나 `dry_run`이 생략되면 미리보기 분기를 선택하지 않습니다.
...
<!-- END GENERATED: contract-structures api.method.prepare_evidence_capture.response[response_variants] api.method.prepare_evidence_capture.response[result_body] api.method.prepare_evidence_capture.response[result_metadata] api.method.prepare_evidence_capture.response[rejection] api.method.prepare_evidence_capture.response[dry_run] -->
