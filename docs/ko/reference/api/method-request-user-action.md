...
`volicord.record_shaping`은 사용자 소유 shaping gap에 이 메서드 계열의 typed UserAction draft와 judgment model을 재사용합니다. checkpoint transaction은 정확한 요청을 생성하고 연결하지만 별도 decision schema를 만들거나 요청을 해결하지 않습니다.
