...
- null이 아닌 `BaselineRef`는 UTF-8 문자열이지만 허용되는 값은 다음 byte-level 정규 scalar 계약과 정확히 일치합니다.
  - byte 길이: `1..=64`
  - 허용 ASCII byte 전체: `-0123456789_abcdefghijklmnopqrstuvwxyz`
  - JSON Schema pattern: `^[\-0123456789_abcdefghijklmnopqrstuvwxyz]{1,64}$`
  - 금지되는 완전한 값: `null`
  - 정규 예시: `baseline_example_001`, `baseline-example-001`, `0123456789abcdef0123456789abcdef01234567`, `aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa`
- `BaselineRef | null`로 명시된 위치만 실제 JSON `null`을 허용합니다. String `null`은 null sentinel이 아니며 유효하지 않습니다.
