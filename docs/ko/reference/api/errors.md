...
Unresolved user judgment blocker는 missing judgment type이 분명하지 않으면 `user_judgment`를 사용하고, 분명하면 더 구체적인 `sensitive_action_approval`, `work_acceptance`, `residual_risk_acceptance`를 사용합니다. Work acceptance는 residual-risk acceptance를 해소하지 않고, residual-risk acceptance는 work acceptance를 해소하지 않습니다.
...
Projection freshness blocker는 requested display/action에 필요한 readable context가 너무 오래됐거나 failed일 때만 `projection_freshness`를 사용합니다. 그 자체로 Core state, evidence summary status, 작업 수락, 잔여 위험 상태, close result를 바꾸지 않습니다.
