...
| 패키지 관리자 설치 | **지원 범위 밖입니다.** | 이 저장소는 Homebrew tap, Homebrew formula, Linux 패키지 관리자 패키지, 외부 패키지 레지스트리를 제공한다고 주장하지 않습니다. | 소스 빌드, 로컬 Docker 빌드, 기존 `volicord` 실행 파일, 또는 검증된 게시 자산 세트가 뒷받침하는 릴리스 설치 스크립트를 사용합니다. |
...
- 선택한 환경이 이 저장소가 문서화하지 않는 패키지 관리자, Homebrew tap, 게시된 Docker 레지스트리 이미지, 원격 호스트, 네트워크 리스너, 호스트 버전 약속에 의존합니다.
