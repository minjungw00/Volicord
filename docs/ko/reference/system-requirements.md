...
| 운영체제 계열 | 이 체크아웃은 어떤 이름 붙은 운영체제 계열도 일반 지원 대상으로 선언하지 않습니다. POSIX 스타일 명령 예시와 Unix 조건부 테스트는 저장소 증거로 검증되어 있지만, 모든 POSIX 시스템에 대한 약속은 아닙니다. | 유지되는 예시는 Cargo 명령, `./target/debug/volicord` 같은 상대 실행 파일 경로, 현재 세션 `PATH` 갱신, `~/.local/bin` 같은 홈 기준 경로, 슬래시 구분 경로, `PATH` 명령 찾기를 담은 `sh` 코드 블록을 사용합니다. CLI 통합 테스트는 `#[cfg(unix)]` 아래에서 `#!/bin/sh` 가짜 실행 파일을 만들고 `std::os::unix::fs::PermissionsExt`로 실행 비트를 설정합니다. 이 체크아웃에는 체크인된 CI 워크플로 행렬이 없습니다. | 유지되는 명령 예시는 POSIX 스타일 셸에서 실행합니다. 특정 운영체제, 컨테이너, WSL, 원격 셸, Windows `cmd.exe`, PowerShell은 향후 담당 문서가 증거를 추가하기 전까지 미검증으로 취급합니다. |
| 셸 문법 | 유지되는 POSIX 스타일 예시에 대해 지원됩니다. 다른 셸은 이 예시에 대해 미검증입니다. | 설치 예시는 `cargo build --workspace --bins`, `export PATH="$PWD/target/debug:$PATH"`, `volicord setup`, 선택적인 `volicord setup --link-bin ~/.local/bin`, setup 뒤의 일반 `volicord connect ...` 명령을 사용합니다. | 셸이 이 문법을 실행하거나 경로를 확장할 수 없다면 직접 명령을 옮기고, 옮긴 각 명령을 확인한 뒤 계속합니다. |
...
- `export PATH="$PWD/target/debug:$PATH"` 같은 현재 세션 `PATH` 갱신
