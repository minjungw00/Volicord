...
| 호스트 역량 릴리스 증거 manifest | 현재 어댑터의 운영 local-web 자격에서는 **미검증이며 사용할 수 없습니다.** | 신뢰되는 결과는 최종 실행 파일이 생긴 뒤 실제 호스트 증거가 생성되므로 바이너리 밖에 있어야 합니다. 별도로 검증된 정확한 최종 아티팩트 릴리스 증거 manifest 또는 receipt가 역량, 호스트·클라이언트, 어댑터, 빌드, source, target, 최종 실행 파일 다이제스트, 예상 `evidence_artifact_sha256`에 결속되어야 합니다. 체크인된 빌드 설명자는 일치 대조 좌표일 뿐 최종 실행 파일 다이제스트나 예상 증거 다이제스트를 담지 않습니다. 현재 어댑터에는 이 manifest를 신뢰해 획득하는 경로가 없습니다. | `build_id`, 영속 검증 행, 복사한 다이제스트, fixture로 credential을 포함한 local-web 가용성을 주장하지 않습니다. 집중 담당 문서가 신뢰된 manifest 스키마와 획득 경로를 정의하고 어댑터가 구현하기 전까지 CLI inbox를 사용합니다. |
...
| Codex 관리 최종 출력 루트 해석 | Linux와 macOS의 로컬 Git 작업 트리에서 **지원되고 검증되었습니다.** Git 저장소가 아닌 Codex `record` 설치에서는 관리 고정 UI 고지를 사용할 수 없습니다. | 생성된 Codex 최종 출력 명령은 Volicord 관리 래퍼를 호출하기 전에 `git rev-parse --show-toplevel`로 Git 작업 트리 루트를 찾습니다. Git 저장소가 아닌 Codex `record` 초기화는 해당 처리기를 생성하지 않고 성공하며 고지 기능을 사용할 수 없다고 기록합니다. | Codex 관리 최종 출력 고지가 필요하면 로컬 Git 작업 트리를 사용합니다. Git 저장소가 아닌 Codex `record` 저장소에서는 `volicord status --task TASK_ID --json`으로 현재 권한을 확인하고 현재 Task가 없으면 `volicord status --json`을 사용합니다. Claude Code 처리기에는 Git 루트가 필요하지 않습니다. |
