...
Projection은 현재 Core 소유 기록과 지속 `ArtifactRef` 메타데이터에서 읽는 시점에 만드는 파생 보기입니다. 사람이 작업, 출처 참조, 차단 사유, 증거 공백, 닫기를 막는 이유, 최신성, 다음 안전한 행동을 읽게 돕습니다. 두 번째 상태 저장소가 아닙니다.
...
표시된 `ArtifactRef`는 지속 아티팩트 기록을 가리키는 포인터입니다. 그 자체가 증거 충분성, 검증, QA, 최종 수락, 잔여 위험 수락, 닫기 준비 상태는 아닙니다. 주장에 필요한 담당 관계, 무결성 메타데이터, `redaction_state`, 가용성이 빠졌으면 공백을 보여줍니다.

임시로 스테이징된 아티팩트 자료와 지속 `ArtifactRef`는 다릅니다. 사용자용 렌더링은 첨부가 스테이징 중인지, 만료 예정인지, 기록을 기다리는지 말할 수 있습니다. 하지만 호환되는 `harness.record_run`이 스테이징 핸들을 소비하고 지속 아티팩트 참조를 주장에 연결하기 전까지는 지속 증거라고 부르면 안 됩니다. 원시 파일 경로, 복사한 로컬 경로, 원시 로그, "화면 캡처가 이 경로에 있다"는 문장은 현재 MVP에서 아티팩트 권한이 아닙니다.
...
사용자용 출력은 Discovery, Change Unit, `EvidenceSummary`, `CloseReadinessBlocker` 같은 하네스 어휘를 알아야 이해되는 형태가 되면 안 됩니다. 먼저 평범한 결과를 보여줍니다. 무엇이 알려졌는지, 사용자가 무엇을 결정해야 하는지, 이 행동 하나에 대해 무엇이 승인됐는지, 어떤 증거가 있거나 빠졌는지, 어떤 위험이 보이는지, 무엇이 닫기를 막는지를 먼저 말합니다. 정확한 스키마 이름은 참조 모델을 설명하거나 에이전트용 패킷을 렌더링할 때만 씁니다.
...
- 증거 참조, 스테이징된 아티팩트 상태, 지속 아티팩트 참조, 가림 처리, 가용성 메모
...
지속 아티팩트 참조: {artifact_ref_summary|none}
...
- 스테이징된 첨부는 임시 입력 스테이징일 뿐입니다. 지속 아티팩트 참조와 따로 보여주고, 실행 기록이 이를 소비해 지속 아티팩트 참조를 주장에 연결하기 전까지 지속 증거로 세지 않습니다.
...
- 관련 있을 때의 스테이징된 아티팩트 핸들 상태와 지속 `ArtifactRef` 상태
...
- 전체 스키마, 전체 참조 문서, 전체 이벤트 로그, 지속 아티팩트 파일 본문, 전체 보고서 본문, 전체 템플릿, 관련 없는 템플릿, 전체 설계 품질 목록, 향후 목록 자료를 기본으로 넣지 않습니다.
