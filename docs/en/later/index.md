...
| `captured_artifact` and captured artifact handles | value names only | Promoted capture/API/schema/storage owners define exact handle source, capability profile gate, validation, storage, redaction, fallback behavior, and proof expectations. Active MVP uses `stage_artifact` with `source_kind=staged_file` instead. | none until promoted |
