...
- The previously open items for simplified judgment naming, `request_user_judgment`, moving MVP-1 next-action output to `harness.status.next_actions` while leaving separate `harness.next` as later/compatibility material, MVP-1 storage minimum, local access error taxonomy, compact status card scope, small direct change evidence requirement, and acceptance/residual-risk minimal records are resolved in the documentation baseline but not yet accepted for coding.
