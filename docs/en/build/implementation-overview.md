...
| First user work loop | MVP-1 User Work Loop. Adds ordinary-language intake, status with `harness.status.next_actions`, focused user judgment request/record, Core-owned evidence summary, close result/blockers, work-acceptance and residual-risk visibility, accepted-risk close when explicitly accepted, and compact Core-derived views. | Full Evidence Manifest, detached Eval, full Manual QA matrix, Assurance hardening, operations/export/recover, dashboards, hosted UI, broad connectors, automation, or detailed reports. |
...
| MVP-1 scope | MVP-1 includes user-visible scope, judgment, Core-owned evidence summary, close blockers, next action, work acceptance when required, residual-risk visibility, and accepted-risk close only through explicit residual-risk acceptance, while excluding later profiles. | [MVP-1 User Work Loop](mvp-user-work-loop.md#mvp-1-included). |
