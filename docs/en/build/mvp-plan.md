...
- `harness.record_run` with an invalid staged handle returns `ToolRejectedResponse` with no `run_summary`, no artifact promotion, no staged handle consumption, and no Write Authorization consumption
