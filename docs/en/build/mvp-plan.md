...
- `harness.record_run` consuming one valid staged handle and promoting it to a persistent `ArtifactRef` after project, Task, server-recorded `created_by_surface_id` / `created_by_surface_instance_id`, expiration, consumed status, checksum, and size validation, linking the resulting artifact/evidence refs through the owner path, and rejecting expired, mismatched, already-consumed, or cross-task staging handles without mutation
...
- `close_task intent=check` as read-only: it may compute close readiness and blockers for the response but must not store blockers, append events, create replay rows, change close state, consume authorization, promote or link artifacts, or increment `project_state.state_version`
