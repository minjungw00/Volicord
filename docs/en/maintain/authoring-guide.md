...
- Blocker-like fields must declare type ownership. Current MVP ownership is `ToolRejectedResponse.errors: ToolError[]` for pre-commit failures; `PrepareWriteResult.write_decision_reasons: WriteDecisionReason[]` for `prepare_write` decision reasons; `CloseTaskResult.blockers: CloseBlocker[]` for `close_task` close-matrix blockers; and `DryRunSummary.would_blockers: PlannedBlocker[]` for dry-run expected blockers. New blocker-like fields must declare the owning method or owning response branch before they appear in examples. Do not reuse `CloseBlocker` outside `close_task` unless the `close_task`-only contract is explicitly changed everywhere it is owned or referenced.
...
- [ ] Blocker-like fields preserve current type ownership: pre-commit failures use `ToolRejectedResponse.errors`, `prepare_write` decision reasons use `WriteDecisionReason`, close-matrix blockers use `CloseBlocker`, and dry-run expected blockers use `PlannedBlocker`.
