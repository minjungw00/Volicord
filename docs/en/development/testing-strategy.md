...
| Binary tests for administrative CLI | [`crates/volicord-cli/tests/binary_admin.rs`](../../../crates/volicord-cli/tests/binary_admin.rs), target `binary_admin`, package `volicord-cli`. | The `volicord` binary, Runtime Home setup commands, project detection, `volicord connect`, `volicord connections`, `volicord connection status/verify/mode/remove`, `volicord export mcp-config`, `volicord inbox ...`, zero-write dry runs, host-state verification, connected-project membership lifecycle, residual-effect reporting, host config writes, preflight failure handling, and command-line error paths. | Public API method behavior. |
