...
| Administrative CLI, host setup, managed host configuration, registration, or `volicord inbox` commands | `crates/volicord-cli/src/` and `crates/volicord-cli/tests/binary_admin.rs`. | CLI colocated unit tests and `binary_admin`. | Store tests when bootstrap, registry, inspection, migration, Agent Connection, or project membership behavior changes; docs-check when CLI docs change. |
