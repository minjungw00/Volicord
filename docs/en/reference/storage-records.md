...
| `state.sqlite` | `expected_writes` | Expected Product Repository write | Project-scoped expected-write correlation record created by an allowed observe pre-tool write, with connection/session identity, optional host invocation identity, exact path policy, active task/change-unit/write-check basis, timestamps, and matched post-tool metadata. |
