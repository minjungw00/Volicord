...
`manual_cli`, `cli_preflight`, or `integration_probe`. Only successful atomic
launch-lease consumption creates `managed_host`; every other source can be
inspected but can never satisfy a managed-host operational-evidence lookup or
authorize a managed call.
