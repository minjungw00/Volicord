...
| `MVP1-unresolved-user-judgment-blocks-write-or-close` | When a relevant product, UX, architecture, sensitive-action, work-acceptance, or residual-risk judgment is missing, pending, rejected, blocked, stale, or incompatible, affected write or close is blocked through structured Core/API results with the judgment refs that need action. |
...
| `MVP1-residual-risk-visible-before-acceptance-risk-close` | Known close-relevant residual risk is visible before successful work acceptance or close; hidden or stale risk blocks the relevant route through `residual_risk_visibility`. |
| `MVP1-accepted-risk-close-requires-explicit-risk-acceptance` | `completed_with_risk_accepted` succeeds only when the risk is visible and a compatible `judgment_type=residual_risk_acceptance` user judgment records acceptance with related blocker/evidence refs; work acceptance or broad "go ahead" text does not satisfy it. |
