...
| `ToolRejectedResponse` | Response branch for pre-commit failures. It has `response_kind=rejected`, `effect_kind=no_effect`, no state effect, and no method-specific result fields. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md) |
| `ToolDryRunResponse` | Response branch for valid dry-run previews. It has `response_kind=dry_run`, `effect_kind=no_effect`, no state effect, and no generated real refs for records that do not exist. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md), [MVP API](api/mvp-api.md) |
