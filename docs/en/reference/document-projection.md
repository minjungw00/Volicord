...
- `secret_omitted` and `blocked` artifacts are displayed as refs plus redaction, omission, or block notes. The projector must not reconstruct, inline, summarize, or export omitted secret/PII values.
- A `blocked` artifact display means the raw capture is unavailable. Evidence, QA, verification, Release Handoff, and export displays must surface that block instead of implying the artifact can be inspected.
