...
Decision Packet projections and cards must show the concrete decision type. Render schema-owned `judgment_domain` as the user-facing judgment grouping and schema-owned `decision_profile` as the source of display depth and profile-specific requiredness. Templates may translate enum values into friendly labels, but they must keep `decision_kind` as the lifecycle/gate route, `judgment_domain` as display grouping, and `decision_profile` as the source of profile-appropriate display depth and choice context. The selected profile determines whether the display is a concise decision, product/UX trade-off, architecture trade-off, approval-shaped prompt, waiver, final acceptance, residual-risk acceptance, reconcile, or mixed decision. The concrete decision type is derived from the route, profile, domain, and related owner refs, so sensitive-action approval does not look like final acceptance and final acceptance does not look like residual-risk acceptance. Affected gates or blocked actions come from the packet's `affected_gates` and related owner records, not from the domain or profile label. Neither `judgment_domain` nor `decision_profile` is a `ProjectionKind`, gate, validator input, close aggregation rule, approval substitute, waiver substitute, residual-risk acceptance rule, or authority path.
