...
| Decision Packet | `state.sqlite.decision_packets` including `decision_kind`, `decision_profile`, and `judgment_domain`, related `decision_gate` state, decision events, related approval or reconcile records, artifact refs, and linked `state.sqlite.residual_risks` when applicable | `TASK` Pending Decisions, Journey Card decision line, status/next responses, judgment-context resources, and decision-packet resources; optional `DEC` when standalone projection is enabled | `request_user_decision` / `record_user_decision`, then projector |
...
Decision Packet projections and cards must show the concrete decision type. Render canonical schema fields separately from friendly labels: `decision_kind` is the lifecycle/gate route, `decision_profile` is the schema/profile validation input for the Decision Packet `profile_payload` branch and profile-specific requiredness, and `judgment_domain` is the schema-owned enum for user-facing judgment grouping. Templates may translate enum values into friendly labels, but those labels remain display-only. The selected profile determines whether the display is a concise decision, product/UX trade-off, architecture trade-off, approval-shaped prompt, waiver, final acceptance, residual-risk acceptance, reconcile, or mixed decision. The concrete decision type is derived from the route, profile, and related owner refs while the domain controls grouping, so sensitive-action approval does not look like final acceptance and final acceptance does not look like residual-risk acceptance. Affected gates or blocked actions come from the packet's `affected_gates` and related owner records, not from the domain or profile label. `decision_profile` is not a `ProjectionKind`, gate, close aggregation rule, approval substitute, waiver substitute, final-acceptance substitute, residual-risk acceptance rule, or authority path. `judgment_domain` is schema-validated as an enum but is not a payload branch selector, gate recompute input, close aggregation rule, approval substitute, waiver substitute, residual-risk acceptance rule, authority path, or replacement for `decision_kind`. Friendly rendered labels derived from these fields are not schema fields, validator inputs, gates, owner records, or authority paths.
...
- Decision Packet: `state.sqlite.decision_packets` including `decision_kind`, `decision_profile`, `judgment_domain`, and related refs -> `TASK` Pending Decisions, status/next responses, judgment-context resources, and decision-packet resources; optional `DEC` projection when standalone projection is enabled
