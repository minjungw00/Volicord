...
- Judgment Inbox CLI behavior belongs in [Administrative CLI](admin-cli.md); User Channel versus Agent Connection boundaries belong in [Agent Connection Reference](agent-connection.md); inbox item shape belongs in [Judgment Schemas](api/schema-judgment.md); elicitation and local web consent transport behavior belongs in [MCP Transport](mcp-transport.md).
