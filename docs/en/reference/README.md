...
- User-action inbox CLI behavior belongs in [Administrative CLI](admin-cli.md); User Channel versus Agent Connection boundaries belong in [Agent Connection Reference](agent-connection.md); inbox item shape belongs in [User Action Schemas](api/schema-user-action.md); host prompt and local consent URL transport behavior belongs in [MCP Transport](mcp-transport.md).
