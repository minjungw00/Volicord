...
- Invocation-scoped Product Repository snapshots, deterministic deltas, and exact expected-write matching: [Repository Observation](repository-observation.md).
...
- User-owned action and judgment meaning belongs in [Core Model](core-model.md); request and resolution method behavior belongs in [Request-user-action method](api/method-request-user-action.md) and [Resolve-user-action method](api/method-resolve-user-action.md); shared request, resolution, status, and adapter-neutral resolution-form shapes belong in [User Action Schemas](api/schema-user-action.md); exact CLI inbox schemas belong in [Administrative CLI](admin-cli.md#user-channel-commands); and nested choice-judgment payloads belong in [Judgment Schemas](api/schema-judgment.md).
...
- The exact `.volicord/policy.json` object, policy commands, defaults, and project/Connection binding checks belong in [Administrative CLI](admin-cli.md#project-workflow-policy-commands); Core policy authority belongs in [Core Model](core-model.md), and persistence belongs in [Storage Records](storage-records.md) and [Storage Effects](storage-effects.md).
- Evidence-capture authorization belongs in [Prepare-evidence-capture method](api/method-prepare-evidence-capture.md); command/tool fulfillment belongs in [Administrative CLI](admin-cli.md#evidence-capture-fulfillment); evidence and Run authority meaning remains with [Core Model](core-model.md) and [Record-run method](api/method-record-run.md).
- The `doctor --privacy-footprint` field set and count projection belong in [Administrative CLI](admin-cli.md#doctor-diagnostic-states); stored-record meaning belongs in [Storage Records](storage-records.md), and assurance limits belong in [Security](security.md).
...
- Repository-observation states, exact invocation correlation, unavailable outcomes, and unmatched-delta creation belong in [Repository Observation](repository-observation.md).
