...
The Assurance Profile and Operations Profile ValidatorResult ID set is API-owned and listed in [API Schema Later](../later/index.md#later-schema-candidates). This runtime reference owns where those validators sit relative to Core and adapters, not a second copy of the ID registry.
...
Core preconditions and mechanical checks such as state/envelope validation, active Task, active Change Unit, changed paths, baseline freshness, approval scope, evidence sufficiency, artifact integrity, verification independence, same-session verification guard, evaluator bundle freshness, and projection freshness may run before or beside these validators. They are not alternate validator IDs unless this section, [API Schema Core](api/schema-core.md#validatorresult), [API Schema Later](../later/index.md#later-schema-candidates), or [Storage](storage.md) explicitly promotes them into the stable ValidatorResult-emitting set. Surface capability is intentionally modeled as the `surface_capability_check` capability validator when emitted as a `ValidatorResult`.
