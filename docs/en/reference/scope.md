...
This reference owns the Volicord supported baseline scope boundary. Product identity belongs to [User Guide Overview](../user-guide/overview.md). This page defines which capability families are inside the baseline, which remain outside it, and how reserved or profile-gated values affect scope.
...
- baseline implementation reading paths; see the [Implementation Guide](../architecture-guide/change-guide.md)
- API method behavior
- schema fields
...
| Managed final-output authority disclosure | On a supported managed Codex or Claude Code final-output event, Volicord can refresh the current read-only status and show either the complete validated canonical `AuthorityReceipt` or a bounded, task-specific `volicord status` fallback in a host-owned fixed UI surface. This projection does not change Core state and is distinct from model-authored final prose and Detective close gating. | [Administrative CLI](admin-cli.md#managed-final-output-authority-disclosure), [Agent Connection](agent-connection.md#managed-final-output-authority-disclosure), [Projection Authority Reference](projection-and-templates.md#managed-final-output-authority-disclosure) |
