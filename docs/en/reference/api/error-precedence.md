...
| 3 | A typed Volicord request reaches Core, but an infrastructure dependency required to produce any method result is operationally unavailable. | Inside Core, during preflight, planning, Store access, or commit. | Typed neutral Core operational error. No public `ErrorCode`, `ToolRejectedResponse`, or other method-response branch exists. | The calling adapter maps the neutral operation, resource, and retryability facts to its own failure contract. CLI uses its runtime diagnostic channel; MCP uses the capability-selected MCP result carrier. Public error precedence does not apply. |
| 4 | A typed Volicord request reaches Core and request validation, exact-contract selection, persisted owner-data validation, Core preflight, invocation compatibility, task lookup, freshness, or idempotency establishes a deterministic method rejection before the method-owned result branch. | Inside Core, before committed method execution. | `ToolRejectedResponse.errors[]` with required `FailureCategory` and public `ErrorCode` values. | Use [API error routing](error-routing.md) for the rejected branch and this document's precedence table for `errors[0]`. No committed operation proceeds. Known corruption keeps its distinct category and code. |
