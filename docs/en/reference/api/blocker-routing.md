...
Method-specific close behavior belongs to [`volicord.check_close` and `volicord.close_task`](method-close-task.md). Route read-only close-readiness validation, intent handling, terminal mutation, state-version behavior, and committed blocked outcomes to that method owner.
