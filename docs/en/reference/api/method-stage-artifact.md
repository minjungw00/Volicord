...
`StageArtifactResult` is the method-specific result branch for a successful staging operation. It carries `base: StageArtifactResultBase`, whose only result effect is `staging_created`, and these method-owned top-level fields:
...
| Field | Required | Nullable | Type |
|---|---|---|---|
| `base` | yes | no | `StageArtifactResultBase` |
...
### `Result Metadata: staging_created` fields
...
- `volicord.stage_artifact`: `dry_run=true` selects the `ToolDryRunResponse` preview branch, whose `base.dry_run` is `true`. `dry_run=false` or an omitted `dry_run` does not select a preview branch.
