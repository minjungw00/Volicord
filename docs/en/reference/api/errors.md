...
| `close_blocked` | `CloseTaskResult.close_state=blocked` plus the primary `ErrorCode` | Return structured blockers and next actions after a valid close matrix evaluation. Preflight failures return `ToolRejectedResponse`; do not mark the Task terminal. |
...
`CloseTaskResult.blockers` must use structured `CloseBlocker` objects from [API Schema Core](schema-core.md#current-position-display-schemas). Prose-only status text, report text, rendered views, or agent summaries are not close-blocker results.
