...
`volicord.record_shaping_checkpoint` reuses this method family's typed UserAction draft and judgment model when a shaping gap is user-owned. The checkpoint transaction atomically creates and links the exact request before the actionable choice is presented; callers must not issue a separate `volicord.request_user_action` for that gap. It does not create a second decision schema or resolve the request. Only the User Channel can resolve it, and chat text never supplies that authority.
...
<!-- BEGIN GENERATED: contract-structures api.method.request_user_action.response[response_variants] api.method.request_user_action.response[result_body] api.method.request_user_action.response[result_metadata] api.method.request_user_action.response[rejection] api.method.request_user_action.response[dry_run] -->
<!-- This region is generated from maintained sources; do not edit it directly. -->

### `RequestUserActionResult` success fields

| Field | Required | Nullable | Type |
|---|---|---|---|
| `base` | yes | no | `RequestUserActionResultBase` |
| `blocker_refs` | yes | no | `StateRecordRef[]` |
| `state` | yes | no | `StateSummary` |
| `user_action_request_summary` | yes | no | `AgentSafeUserActionRequestSummary` |
...
- `volicord.request_user_action`: `dry_run=true` selects the `ToolDryRunResponse` preview branch, whose `base.dry_run` is `true`. `dry_run=false` or an omitted `dry_run` does not select a preview branch.
...
<!-- END GENERATED: contract-structures api.method.request_user_action.response[response_variants] api.method.request_user_action.response[result_body] api.method.request_user_action.response[result_metadata] api.method.request_user_action.response[rejection] api.method.request_user_action.response[dry_run] -->
...
A read-only Agent Connection cannot use either branch. The CLI presentation may
