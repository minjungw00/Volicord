...
- `ToolResultBase` carries only result metadata. Its `response_kind` is exactly
  ...
  `staging_created`, or `no_effect`. A result with `dry_run=true` is a
...
- `ToolRejectedResponse` carries `base: ToolRejectedBase` and `errors`.
  ...
  `ToolRejectedBase.effect_kind` is exactly `no_effect`. Its `dry_run` records
  the request mode and may be `true` or `false`.
