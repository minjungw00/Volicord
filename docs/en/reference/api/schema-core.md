...
- `ToolResultBase` carries only result metadata. Its `response_kind` is exactly
  `result`. Its `effect_kind` is one of `read_only`, `core_committed`,
  `staging_created`, or `no_effect`. A method whose contract accepts requested
  dry-run intent through its regular result branch returns
  `base.dry_run=true`; the current result schema permits that value only for a
  `read_only` result. The other result effects require `dry_run=false`.
...
<!-- BEGIN GENERATED: contract-structures api.schema.core[schema_object.ToolResultBase] api.schema.core[schema_object.ToolRejectedBase] api.schema.core[schema_object.ToolDryRunBase] api.schema.core[schema_object.ToolRejectedResponse] api.schema.core[schema_object.ToolDryRunResponse] -->
...
<!-- END GENERATED: contract-structures api.schema.core[schema_object.ToolResultBase] api.schema.core[schema_object.ToolRejectedBase] api.schema.core[schema_object.ToolDryRunBase] api.schema.core[schema_object.ToolRejectedResponse] api.schema.core[schema_object.ToolDryRunResponse] -->
