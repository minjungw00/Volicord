...
| `MVP-ACTIVE-close-task-complete-blocker-matrix` | `close_task intent=complete` applies the deterministic blocker order from Task validity through recovery constraints before committing completion. | Response blockers follow the Core matrix order; missing required evidence blocks before final acceptance or residual-risk acceptance; final acceptance and residual-risk acceptance never replace required evidence or required artifacts; Task remains open when blockers remain. |
