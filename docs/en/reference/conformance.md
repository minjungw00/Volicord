...
| <a id="scenario-baseline-mcp-semantic-descriptor-conformance"></a>`BASELINE-mcp-semantic-descriptor-conformance` | Every production tool has one deterministic semantic descriptor; its advertised typed examples validate, decode to the exact request type, and select the declared nested tagged-union variants. Invalid or missing discriminators produce only the discriminator issue for that union, never a guessed branch, and descriptor/decode disagreement fails conformance. | [MCP Transport](mcp-transport.md#public-argument-projection); [Testing Strategy](../architecture-guide/testing-strategy.md) |
| <a id="scenario-baseline-workflow-model-liveness-recovery"></a>`BASELINE-workflow-model-liveness-recovery` | Deterministic reachable-state exploration preserves unique catalog action keys, required-member integrity, exact transition-owned Core admission, nonterminal liveness, current typed recovery membership, complete Agent-transition/action-form parity, and an accepted exact-method Core no-commit plan for every advertised form. | [Core Model](core-model.md); [API State Schemas](api/schema-state.md); [MCP Transport](mcp-transport.md#public-argument-projection); [Testing Strategy](../architecture-guide/testing-strategy.md) |
...
| <a id="scenario-baseline-mcp-mutation-finalization-preserves-effect-branches"></a>`BASELINE-mcp-mutation-finalization-preserves-effect-branches` | Pre-effect internal failure remains an ordinary no-effect error; post-effect recovery preserves the exact new-commit, staging, or replay facts, prohibits mutation retry, and requires current status; normal no-effect results and typed rejections remain distinct. | [MCP Transport](mcp-transport.md#mutation-authority-receipt-projection); [API error routing](api/error-routing.md); [Storage Effects](storage-effects.md) |
