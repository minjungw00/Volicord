...
| <a id="scenario-baseline-managed-final-output-refreshes-current-authority"></a>`BASELINE-managed-final-output-refreshes-current-authority` | Every eligible built-in managed-adapter final-output delivery that proceeds after read-only binding verification, including an exact replay and a best-effort delivery whose typed feature state is not `verified`, validates a newly read current status receipt without changing the immutable historical Stop decision. | [Projection and Templates](projection-and-templates.md#managed-final-output-authority-disclosure); [Agent Connection](agent-connection.md#managed-final-output-authority-disclosure); [Administrative CLI](admin-cli.md#managed-final-output-authority-disclosure) |
| <a id="scenario-baseline-managed-final-output-uses-bounded-status-fallback"></a>`BASELINE-managed-final-output-uses-bounded-status-fallback` | A receipt that is unavailable, invalid, or too large is replaced by a bounded exact `volicord status` route; partial receipt JSON is never emitted. | [Template Bodies](template-bodies.md#final-output-authority-disclosure-body); [Projection and Templates](projection-and-templates.md#managed-final-output-authority-disclosure); [Security](security.md#generated-displays-and-text) |
