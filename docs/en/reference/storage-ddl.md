...
Every non-null scalar `BaselineRef` column uses the predicate generated from
the type-owned canonical scalar specification. The generated predicate checks
the exact byte-length interval, requires character length to equal byte length,
rejects every byte outside the allowed ASCII alphabet, and rejects every
forbidden complete value. Nullable scalar positions wrap that predicate with
SQL `NULL`; readiness positions require a present value and apply the same
predicate. The generated regions in the canonical SQL below are deterministic,
contribute to the current manifest identity, and feed both maintained
canonical-SQL projections. This DDL is defense in depth: Store still
checked-decodes every selected scalar or typed JSON baseline before the value
crosses the storage boundary and reports typed corruption when the physical
constraint has been bypassed.
...
- `repository_observations` has one row per exact project, Connection, host session, host turn, tool-use ID, and tool-name coordinate. Its checks keep `open`, `complete`, and `unavailable` baseline, outcome, delta, reason, event, completion, and terminal-result columns in their exact state combinations.
- `expected_writes.status` is constrained to `pending` or `matched`, and `path_policy` is constrained to `exact_paths`. Every row references one exact repository observation. Matched rows carry non-empty matched paths JSON and `matched_at`; pending rows carry neither.
- `unrecorded_changes.status` is constrained to `unresolved` or `resolved`. Every row references one exact repository observation, stores a non-empty path set and canonical unmatched-delta digest, and is unique for that observation and digest. Resolved rows carry resolution JSON, `resolved_at`, and `resolved_by_actor_source`; unresolved rows carry none of those resolution fields.
