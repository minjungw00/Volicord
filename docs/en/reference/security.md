...
| Artifact store | Artifact bytes are untrusted until persisted, related to the owner record, and validated for required integrity/redaction metadata. Staged bytes remain in temporary staging until an eligible `record_run` promotion creates a persistent `ArtifactRef`. |
...
| Artifact or evidence tampering | Bytes, paths, hashes, staging handles, or metadata are swapped, stale, expired, consumed, missing, redacted, blocked, cross-task, cross-surface, or unrelated to the owner record. | Treat evidence as insufficient or blocked until staging-handle project, task, `created_by_surface_id` / `created_by_surface_instance_id` against current verified `surface_id` / `surface_instance_id`, expiration, consumed-status, checksum, size, persistence, integrity, redaction, and owner relation checks pass where applicable. Raw paths, raw logs, arbitrary local path strings, and native capture claims are not artifact authority. |
...
- artifact `sha256`, `size_bytes`, `content_type`, ownership, availability, redaction, omission, or blocked-payload checks for owner-persisted artifact refs where the owner path requires them; these checks are not native artifact capture
