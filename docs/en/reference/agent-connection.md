...
| Connection Projects membership | `connection_projects` records under the same Runtime Home. | `volicord init`, `volicord connection add`, `volicord project use`, and connection removal flows can add, validate, or remove membership according to the selected repository root. | Membership controls the Agent Connection project allowlist. It does not register every Runtime Home project and does not delete project registration, project state, or Core records. |
...
| Verification state | `last_verification_status` in the Agent Connection registry record, plus command output owned by [Administrative CLI](admin-cli.md#agent-connection-result-states). | `volicord init`, `volicord connection add`, and `volicord connection verify` run observable host-configuration, hook path safety, MCP startup, MCP initialization, and `tools/list` checks where available. | Verification can inspect both Volicord-side state and host/MCP readiness. Hook path safety and MCP startup validation alone are not a `complete` Agent Connection and do not prove the host loaded, trusted, or approved the integration. |
