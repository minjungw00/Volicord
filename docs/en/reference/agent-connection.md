...
| Verification state | `last_verification_status` in the Agent Connection registry record, plus command output owned by [Administrative CLI](admin-cli.md#agent-connection-result-states). | `volicord init`, `volicord connection add`, and `volicord connection verify` run observable host-configuration, hook path safety, MCP startup, MCP initialization, and `tools/list` checks where available. | Verification can inspect both Volicord-side state and host/MCP readiness. Hook path safety and MCP startup validation alone are not a `complete` Agent Connection and do not prove the host loaded or trusted the integration. |
