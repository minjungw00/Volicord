...
schema vocabulary. One semantic descriptor entry per `AgentToolId` owns the
input and output nodes, explicit tagged-union discriminator paths and values,
required-nullable fields, semantic type names, field descriptions, validation
metadata, and typed canonical examples. The same entry generates discovery
schemas, semantic validation, the generated catalog below, and conformance
cases. This includes `McpOperationalErrorCode`,
...
| `volicord.get_integration_verification` | `IntegrationVerificationIdArguments` | none | none | `McpToolStructuredContent_for_GetIntegrationVerificationResult` | `/result_type`: `response`, `adapter_error` |
...
The canonical tool registry keys each definition by `AgentToolId` and consumes
its description, input schema, descriptor-generated compact output projection,
typed examples, validation descriptor, annotations, and any populated optional
presentation or metadata values from that identity's one semantic contract
entry. `tools/list` emits the
...
object. The MCP structured result adds the required `result_type` discriminator
owned by its output descriptor; nested public unions retain their own required
discriminators. The selected profile then chooses its MCP carrier without
changing the public method-result fields or their Core meaning:
