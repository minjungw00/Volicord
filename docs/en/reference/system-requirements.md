...
| Release version alignment | **Supported and verified** for the checked-in tag-release workflow. | Before any release build job, `.github/workflows/release.yml` runs `cargo run --locked -p xtask -- release-version-check`. For a tag it also passes `--tag` and requires the tag to equal `v` plus the root `[workspace.package].version`; the checker requires every workspace member package to use `version.workspace=true`. Existing CLI and MCP transport tests verify that `volicord --version` and MCP initialize `serverInfo.version` expose the inherited Cargo package version. The workflow reuses that SemVer identity without adding commit-SHA or build-metadata fields. | Before publishing a tag, complete the batch-level SemVer assessment, update the root workspace package version and lockfile as needed, and use the exact matching `vX.Y.Z` tag. |
