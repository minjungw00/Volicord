...
| Native Windows x86_64 Record profile | **Supported and verified** for the `record` profile and release target `x86_64-pc-windows-msvc`. `volicord evidence capture-command` is an explicit exception and is unavailable on native Windows. | The release workflow builds on `windows-2022`, smoke-tests `target/x86_64-pc-windows-msvc/release/volicord.exe`, packages `volicord-x86_64-pc-windows-msvc.zip`, generates `.sha256`, and runs a native Windows `cargo test --workspace --all-targets --all-features` job. The downloaded `install.ps1` asset installs the matching binary under a user-local directory by default. Bounded command process-group termination is implemented only for Linux and macOS. | Use PowerShell on native Windows x86_64. Use `volicord init --host HOST --repo PATH --profile record`. Run command evidence capture in WSL2, Linux, or macOS. |
