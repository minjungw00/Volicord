...
| Host release evidence gate | The external contract is **defined**, its `tests/release-validation` implementation is **unverified until present and passing**, and production local-web manifest acquisition is **unavailable**. | [Host Release Evidence](host-release-evidence.md) owns the four versioned schemas, exact external candidate binding, fixed twelve cells, canonical evaluator, half-open freshness rule, create-new manifest, and separate-process audit. These artifacts are produced after the final executable and remain outside the binary and Runtime Home. The current production adapter has no trusted acquisition path and external release artifacts are not runtime trust inputs. | Run the owner-defined package commands only when `tests/release-validation` exists. Do not claim credential-bearing local-web availability from a manifest, `build_id`, persisted row, copied digest, or fixture. Use CLI inbox for production fallback. |
