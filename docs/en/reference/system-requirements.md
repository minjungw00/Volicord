...
| Host-capability release evidence manifest | **Unverified and unavailable** for production local-web eligibility in the current adapter. | A trusted result must be external to the binary because live-host evidence is produced after the final executable exists. It must be a separately verified exact-final-artifact release evidence manifest or receipt that binds the capability, host/client, adapter, build, source, target, final executable digest, and expected `evidence_artifact_sha256`. The checked-in build descriptor is only a matching coordinate and carries neither the final executable digest nor the expected evidence digest. The current adapter has no trusted acquisition path for this manifest. | Do not claim credential-bearing local-web availability from `build_id`, a persisted verification row, a copied digest, or a fixture. Use CLI inbox until a focused owner defines and the adapter implements a trusted manifest schema and acquisition path. |
...
| Native Windows x86_64 Record profile | **Supported and verified** for the `record` profile and release target `x86_64-pc-windows-msvc`. `volicord evidence capture-command` and the managed per-final-output fixed-UI authority disclosure are explicit exceptions and are unavailable on native Windows. The canonical `volicord status` fallback remains available. | The release workflow builds on `windows-2022`, smoke-tests `target/x86_64-pc-windows-msvc/release/volicord.exe`, packages `volicord-x86_64-pc-windows-msvc.zip`, generates `.sha256`, and runs a native Windows `cargo test --workspace --all-targets --all-features` job. The downloaded `install.ps1` asset installs the matching binary under a user-local directory by default. Bounded command process-group termination and the currently verified generated final-output wrappers are implemented only for Linux and macOS. | Use PowerShell on native Windows x86_64. Use `volicord init --host HOST --repo PATH --profile record`. Run command evidence capture or managed fixed-UI final-output disclosure in WSL2, Linux, or macOS; on native Windows inspect current authority with `volicord status --task TASK_ID --json` or `volicord status --json` when there is no active Task. |
...
| Codex managed final-output root resolution | **Supported and verified** for local Git work trees on Linux and macOS. The managed fixed-UI disclosure is unavailable for a non-Git Codex `record` installation. | Generated Codex final-output commands resolve the Git work-tree root with `git rev-parse --show-toplevel` before dispatching to the Volicord-managed wrapper. A non-Git Codex `record` initialization succeeds without generating that handler and records the disclosure capability as unavailable. | Use a local Git work tree when the Codex managed final-output disclosure is required. In a non-Git Codex `record` repository, inspect current authority with `volicord status --task TASK_ID --json`, or `volicord status --json` when there is no active Task. Claude Code does not require a Git root for this handler. |
