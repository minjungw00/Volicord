...
Recovery may regenerate derived display, rescan artifact refs or route artifact repair through an owner path, mark dependent evidence or views stale or blocked, interrupt stale work records where the owner contract allows it, or route a needed user judgment or Core action. It must not create a second state model.
