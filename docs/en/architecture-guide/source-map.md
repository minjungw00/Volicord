...
| `crates/volicord-store/src/core_pipeline/shaping.rs` | Atomic shaping-checkpoint, gap, link, shaping-owned UserAction, and exact stale-authority mutations; strict current-aggregate and immutable reauthorization-history reads; persisted-invariant validation. |
...
| `crates/volicord-core/src/methods/` | Public-method entry points and request-specific orchestration. `record_shaping.rs` plans explicit checkpoint succession, prevents live-decision replacement, and finalizes advisor-owned decisions and results; `update_scope.rs` applies only exact scope-owned shaping decisions; `advance_task.rs` validates Task-wide authority and atomically applies advance-owned decisions with the explicit phase transition. Production modules import shared responsibilities from their explicit owners; `methods/mod.rs` provides only module wiring, while method-specific plan wrappers stay in their owning method modules. |
