...
| `crates/volicord-core/src/write_ticket/` | Canonical Write Ticket facts, validity and attempt-scope policy, prepare-write semantic planning, Record Run admission, and adapter-neutral ticket projection. `planning.rs` owns the validated unpersisted `PlannedWriteTicket` from which new-issuance projection and typed Store insertion derive. `projection.rs` explicitly projects planned, stored, and post-consumption ticket facts. `admission.rs` accepts typed operation facts and returns a typed attempt scope or semantic admission error; public response composition remains in the calling method. |
