...
| `crates/volicord-types/src/managed_guidance.rs` | Required semantic facts that generated managed-host guidance must preserve, including explicit shaping, UserAction, advance, implementation, rejection, and close-review boundaries. |
...
| `crates/volicord-store/src/core_pipeline/shaping.rs` | Atomic shaping-checkpoint, gap, link, and shaping-owned UserAction mutations; strict current-aggregate reads and persisted-invariant validation. |
...
| `crates/volicord-core/src/workflow_projection.rs` | Closed tagged workflow authority projection, including required and allowed actions, blocking reasons, current record references, and state-version binding. |
...
| `crates/volicord-mcp/src/constants.rs` | MCP initialize instructions for the user-level verification request and current managed workflow: explicit shaping, UserAction presentation and resolution, decision application, explicit advance, implementation admission, rejection surfacing, close review, stop rules, unavailable boundary, and optional active diagnostics. |
