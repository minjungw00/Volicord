...
| `crates/volicord-types/src/managed_guidance.rs` | Closed semantic facts and distinct canonical digests for generated managed-host guidance, the Core workflow and exact no-commit-planning contract, state-specific transition submission contracts, MCP action forms, and the MCP semantic schema. The facts include current transition and variant admission, closed accepted plan branches, typed rejection as planning failure, effect-free planning, authority-fixed and Task-mode-specific Agent-authored inputs, exact method-and-variant forms, no-commit publication gating, type-owned schema semantics, Store-derived current authority, non-actionable immutable history, exact stale retirement or fresh-identity reauthorization, implementation-preserving rejection, current checkpoint/UserAction preservation, explicit compatible-application carry-forward, decision ownership, User Channel resolution, typed rejection/recovery surfacing, and close-review boundaries. All five digests participate in the project integration revision. |
...
| `crates/volicord-types/src/integration_revision.rs` | Typed Connection/project integration revision bases and derivation, including distinct Store-manifest, managed-guidance, workflow-contract, submission-contract, action-form-contract, MCP-semantic-schema, and scalar-contract digest inputs. |
