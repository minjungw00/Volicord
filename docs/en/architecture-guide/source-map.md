...
| `crates/volicord-store/src/workflow_records.rs` | Private workflow-policy row projection, strict schema, closed-value, canonical-byte, fingerprint, source, and timestamp decoding into typed policy records; typed mutation input, application, and effects. |
...
| `crates/volicord-store/src/core_pipeline/write_tickets.rs` | Typed Write Ticket mutation inputs, storage validation and SQL application; private physical rows, strict status, validity-basis, attempt-scope, timestamp, and product-path decoding into typed records; facade reads and focused tests. |
...
| `crates/volicord-store/src/core_pipeline/reconciliation.rs` | Strictly decoded typed expected-write and unrecorded-change observation candidate projections, including Product Repository paths, plus current-handle unresolved-change reads used by close-readiness fact acquisition. |
...
| `crates/volicord-core/src/methods/` | Public-method entry points and request-specific orchestration. Production modules import shared responsibilities from their explicit owners; `methods/mod.rs` provides only module wiring and the small method-generic plan carrier. |
| `crates/volicord-core/src/recording/context.rs`, `model.rs` | Record Run request normalization, Store-aware typed fact acquisition, and the closed fact, authority, observation, artifact, mutation-plan, and planned-result models shared only within the recording package. |
...
| `crates/volicord-core/src/recording/plan.rs` | Record Run policy-service coordination and typed `RecordRunMutationPlan` assembly; domain mutations remain distinct until final Store-plan conversion. |
| `crates/volicord-core/src/recording/state.rs`, `projection.rs` | Store-aware post-operation state-fact acquisition followed by Store-free, policy-free `RecordRunResultFields` projection. |
| `crates/volicord-core/src/recording/tests/` | Responsibility-local Record Run context, capture-authority, evidence, artifact, and projection coverage. |
...
| `crates/volicord-core/src/methods/record_run.rs` | Public Record Run entry-point orchestration, semantic error-to-response mapping, Core plan submission, and method-specific metric recording. Detailed recording policy and planning stay with responsibility owners. |
