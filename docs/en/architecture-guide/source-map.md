...
| `crates/volicord-core/src/write_ticket/` | Canonical Write Ticket ownership. `planning.rs` owns validated unpersisted issuance plans. `semantic.rs` supplies the immutable meaning shared by planned and stored forms while preserving distinct identities. `read_model.rs` acquires typed ticket, Task, workflow-policy, current UserAction-resolution, and evidence facts. `selection.rs` owns pure candidate precedence and tie-breaking. `current_validity.rs` owns pure effective-status, invalidation, authority, and approval evaluation. `summary.rs` purely maps an evaluated ticket and supplied evidence to the adapter-neutral summary. `service.rs` narrowly coordinates persisted-summary loading. `policy.rs` owns prepare-write and attempt-scope decisions, and `admission.rs` returns a typed admitted scope or semantic admission error. Public response composition remains in the calling method. |
