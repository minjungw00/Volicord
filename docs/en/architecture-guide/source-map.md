...
| `crates/volicord-platform-fs/src/repository_observation/model.rs` | Closed repository snapshot, path-state, transition, delta, unavailable-reason, invocation-path, canonical serialization, and semantic observer-contract digest types. |
...
| `crates/volicord-platform-fs/src/repository_observation/snapshot.rs` | Typed invocation-path union, bounded stable double observation, surrounding coordinate rechecks, and repository snapshot construction. |
| `crates/volicord-platform-fs/src/repository_observation/delta.rs` | Dirty, untracked, invocation, and changed-tree candidate union plus deterministic before/after net path-transition calculation. |
