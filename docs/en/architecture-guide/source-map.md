...
| `crates/volicord-store/src/core_pipeline/write_tickets.rs` | Sole owner of the physical Write Ticket table and columns; typed mutations and SQL; one private row projection, canonical normal/transaction decoder, closed-value and structured-field validation, typed cross-field invariant corruption, focused authority views, facade reads, and aggregate tests. |
