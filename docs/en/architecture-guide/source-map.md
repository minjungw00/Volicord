...
| Path | Responsibility |
|---|---|
| `crates/volicord-platform-fs/src/lib.rs` | Current process target and platform observation, native Linux/WSL2 kernel classification, WSL2 `/etc/os-release` distribution validation, path-filesystem observation, effect-aware exact directory-tree removal and parent-entry durability, platform-native namespace operations, safe setup-lease exports, and canonical read-only Git layout discovery. |
| `crates/volicord-platform-fs/src/setup_lease.rs` | Canonical Runtime Home identity, full-digest external coordination-file derivation, immediate typed acquisition, and exclusive Unix/macOS or native Windows OS file-lock ownership and release. |
| `crates/volicord-platform-fs/tests/setup_lease_process.rs` | Cross-process setup-lease contention and process-termination release regression. |
...
| `crates/volicord-cli/src/connection_command/service.rs` | Locked setup service boundary that acquires before planning and retains the canonical Runtime Home lease through dry-run reporting, commit, cleanup, or rollback for init and Connection add. |
...
| `crates/volicord-cli/tests/init_record_regression.rs` | Init plan/read-only, replay, exact owner record, post-rename and all-stage setup fault injection, deterministic setup-lease success/rollback contention in both invocation orders, busy and dry-run non-mutation, unexpected external publication abort, concurrent file modification, full rollback, and partial-rollback reporting regressions. |
