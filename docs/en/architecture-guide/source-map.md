...
| `crates/volicord-types/src/managed_guidance.rs` | Closed semantic facts and distinct canonical digests for generic managed-host recovery guidance, mutation finalization, post-effect response recovery, the Core workflow and exact no-commit-planning contract, state-specific transition submission contracts, MCP action forms, and the MCP semantic schema. Managed guidance distinguishes method failure from response projection, prohibits retry after an applied effect, requires current status and exact-result retrieval when available, distinguishes commit, staging, and replay, withholds unchanged-state and completion claims, and preserves current workflow authority. All seven digests participate in the project integration revision. |
...
| `crates/volicord-types/src/integration_revision.rs` | Typed Connection/project integration revision bases and derivation, including distinct Store-manifest, managed-guidance, mutation-finalization, post-effect-response, workflow-contract, submission-contract, action-form-contract, MCP-semantic-schema, and scalar-contract digest inputs. |
...
| Path | Responsibility |
|---|---|
| `crates/volicord-core/src/pipeline.rs` | Separate read-only path and admitted-context `CoreService` construction, typed Core/Store Runtime Home authorization, common preflight, replay, plan selection, response, commit orchestration, and the exact typed no-commit transition-submission mode. That mode reuses method admission, normalization, policy, semantic validation, mutation planning, effect checks, and actual-result checks against a read-only current Store, then stops before transactions and every durable or repository effect. The module also owns Store-error detail projection and neutral typed operational projection of platform-owned Product Repository observation failures. |
...
| `crates/volicord-core/src/methods/tests/workflow_state_space.rs` | Deterministic pure reachable-state exploration that checks catalog uniqueness, required-member integrity, transition-owned admission identity, nonterminal liveness, and exact typed recovery membership. Store-backed MCP journey tests own actual form-to-Core no-commit executability coverage. |
...
| `crates/volicord-cli/src/guard_integration/plan.rs` and `hosts/codex.rs` | Source templates for the generic managed `AGENTS.md` mutation-recovery rules and the separate Codex rule guidance for integration verification and host hooks. |
...
| Path | Responsibility |
|---|---|
| `crates/volicord-mcp-protocol/src/lib.rs` | Closed typed MCP revision parsing, exact production profile lookup, the single revision-to-semantic-capability map including mutation-finalization recovery, deterministic supported-revision iteration, tracked pre-release classification, and explicit unknown or unsupported rejection. |
| `crates/volicord-mcp-protocol/tests/protocol_registry.rs` | Pinned manifest parity, complete semantic/schema and mutation-finalization-recovery capability parity, registry uniqueness, deterministic iteration, exact parsing and selection, preferred-revision membership, and pre-release exclusion. |

## MCP Wire

| Path | Responsibility |
|---|---|
| `crates/volicord-mcp-wire/src/methods.rs` | Exact MCP argument, structured result, workflow action form, authoritative argument context, retry contract, bounded workflow-contract failure action/stage diagnostics, operational error, compact mutation, UserAction projection, serialization, and generated request/result schema ownership. |
...
| `crates/volicord-mcp/src/mutation_projection.rs` | Mutation detail selection, effect anchoring, compact method-result projection, fresh-authority composition, capability-driven normal result-budget enforcement, and workflow diagnostics constructed with the observed effect facts. |
...
| `crates/volicord-mcp/src/action_form.rs` | Descriptor-bound projection from neutral Agent `TransitionDescriptor` values and their exact submission contracts to deterministic method-and-variant MCP forms, fixed binders, complete contract-owned validation witnesses, variant-aware authority-only retry contracts, and exact catalog-totality checks. It sequences semantic, decode, binding, adapter, and Core validation through an injected validator; it does not hydrate forms from method examples or recalculate transition availability. |
| `crates/volicord-mcp/src/mutation_finalization_recovery.rs` | Capability-selected, authority-first bounded mutation-finalization recovery for pre-effect failure, committed and staging effects, replay, and no-effect method branches. It validates diagnostics against observed facts and never retries or rewrites the mutation effect. |
...
| `crates/volicord-mcp/src/adapter.rs` | Retained pre-operation routing identity, bounded read-only invalid-argument authority bootstrap, exact method, semantic-variant, and action-form-ref admission before mutation Core entry, live mutation-context correlation, and context-bound Core invocation APIs. For catalog publication it projects every complete witness through the exact method adapter, derives a verified invocation from read-only current authority, and invokes the matching Core no-commit planner. It also owns managed in-chat begin/probe/get integration-verification orchestration outside Core. |
