...
| `crates/volicord-core/src/write_ticket/` | Canonical Write Ticket ownership. `planning.rs` evaluates focused `PrepareWriteInput` facts and returns typed decision reasons, semantic record identities, candidate mutations, and an identity-free issuance draft; it also validates method-supplied identity and basis facts when materializing `PlannedWriteTicket`. `semantic.rs` supplies the immutable meaning shared by planned and stored forms while preserving distinct identities. `read_model.rs` acquires typed ticket, Task, workflow-policy, current UserAction-resolution, and evidence facts. `selection.rs` owns pure candidate precedence and tie-breaking. `current_validity.rs` owns pure effective-status, invalidation, authority, and approval evaluation. `summary.rs` purely maps an evaluated ticket and supplied evidence to the adapter-neutral summary. `service.rs` narrowly coordinates persisted-summary loading. `policy.rs` owns typed prepare-write and attempt-scope decisions, and `admission.rs` accepts exact semantic facts and returns a typed admitted scope or admission error. Public request metadata and response composition remain in the calling method. |
