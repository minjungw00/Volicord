...
| `crates/volicord-cli` | `crates/volicord-cli/src/main.rs`, `crates/volicord-cli/src/setup_command.rs`, `crates/volicord-cli/src/doctor_command.rs`, `crates/volicord-cli/src/project_context.rs`, `crates/volicord-cli/src/connection_command.rs`, `crates/volicord-cli/src/guard_integration/`, `crates/volicord-cli/src/export_command.rs`, `crates/volicord-cli/src/user_command.rs`, `crates/volicord-cli/src/host_integration/`, `crates/volicord-cli/src/registration.rs` | `main.rs` dispatches administrative commands, `volicord mcp` command modes, and binary exit behavior. `setup_command.rs` and `doctor_command.rs` handle installation profile readiness. `project_context.rs` detects Git repository roots and orchestrates project commands. `connection_command.rs` parses and orchestrates `volicord init`, `volicord connection add`, `volicord connection list`, and `volicord connection ...` commands. `guard_integration/` owns factual guard file, generated host-hook file, and hook command audit helpers consumed by connection status. `export_command.rs` renders authority bundle exports. `user_command.rs` parses and orchestrates local User Channel status and judgment commands. `host_integration/` owns Codex, Claude Code, and managed host configuration planning. `registration.rs` builds Agent Connection, Connection Projects, and User Channel metadata. |
...
| CLI architecture | This page's administrative setup flow, `crates/volicord-cli/src/main.rs`, `setup_command.rs`, `doctor_command.rs`, `connection_command.rs`, `guard_integration/`, `project_context.rs`, `user_command.rs`, and `host_integration/`. | [Administrative CLI](../reference/admin-cli.md), [Runtime Boundaries](../reference/runtime-boundaries.md), and [Agent Connection](../reference/agent-connection.md). |
...
| Detective and session-watch design | `crates/volicord-cli/src/host_integration/`, `crates/volicord-cli/src/guard_integration/`, `crates/volicord-cli/src/guard_command.rs`, session-watch storage helpers, and close-readiness policy tests. | [Administrative CLI](../reference/admin-cli.md#guard-hook-commands), [Storage Records](../reference/storage-records.md), [MCP Transport](../reference/mcp-transport.md), and [Security](../reference/security.md). |
...
`volicord init`, `volicord connection add`, and `volicord connection ...` are implemented as local administrative orchestration, not as public Core methods. The implementation lives under `crates/volicord-cli/src/connection_command.rs`, factual guard integration audit helpers in `crates/volicord-cli/src/guard_integration/`, and the host adapters in `crates/volicord-cli/src/host_integration/`; exact command, Agent Connection, MCP transport, host-hook integration, and runtime-boundary contracts stay with [Administrative CLI](../reference/admin-cli.md), [MCP Transport](../reference/mcp-transport.md), [Runtime Boundaries](../reference/runtime-boundaries.md), and [Security](../reference/security.md).
