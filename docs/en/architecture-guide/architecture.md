...
| `crates/volicord-mcp` | MCP adapter library for the canonical managed-launch contract, startup validation, consumption of the canonical tool model supplied by Volicord tool owners, revision-specific `tools/list` and `tools/call` projection, managed stdio lifecycle and framing, Core invocation, and consumption of typed protocol profiles. |
...
| MCP protocol-profile boundary | `volicord-mcp-protocol` owns the closed typed revision set, reviewed production profiles, message/tool/schema feature declarations, explicit iteration order, and preferred server revision. It owns protocol feature facts, not Volicord tool definitions or host fixtures. MCP adapters consume this boundary; host compatibility observations do not own server preference. | [Source Map](source-map.md), [Testing Strategy](testing-strategy.md), and [MCP Transport](../reference/mcp-transport.md). |
