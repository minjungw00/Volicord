...
| `crates/volicord-store` | Canonical SQLite storage, Runtime Home, bootstrap, project Store, Agent Connection runtime/project sessions, structured finding and cause-graph persistence, artifact storage, inspection, export snapshots, and storage-error implementation. |
...
| `crates/volicord-cli` | Local `volicord` administrative binary and reusable command modules for setup, project registration, CLI inbox commands, Codex Agent Connection install/verify/repair/uninstall, dependency-aware verification policy, common diagnostic-report presentation, and managed stdio MCP supervision policy, deadlines, framing, progress, and diagnostics. |
...
| `crates/volicord-mcp` | MCP adapter library for the canonical managed-launch contract, startup validation, the one repository-owned offline conformance revision declaration, consumption of the canonical tool model supplied by Volicord tool owners, revision-specific `tools/list` and `tools/call` projection, managed stdio lifecycle and framing, Core invocation, and consumption of typed protocol profiles. |
...
| `xtask` | Repository maintenance tooling for documentation validation and pinned MCP specification synchronization and offline integrity. Its MCP check consumes the compiled production-profile and adapter-owned conformance declarations for exact manifest parity; it remains outside Volicord runtime architecture. |
...
- `xtask` remains repository maintenance tooling outside product runtime. Its
  MCP specification checker depends on `volicord-mcp-protocol` and
  `volicord-mcp` only to compare the compiled production-profile and
  conformance revision declarations with the pinned manifest. The ordinary
  check remains offline; only the explicitly invoked sync command uses the
  network.
...
| CLI operational diagnostics | `volicord-cli` separates immutable operational definitions, closed typed subjects and facts, typed action selection, finding projection, and owner-scoped current-condition persistence. Connection verification supplies typed observations; Store remains the lifecycle and query implementation owner. | [Source Map](source-map.md), [Failure Model](../reference/failure-model.md), [Administrative CLI](../reference/admin-cli.md), and [Agent Connection](../reference/agent-connection.md). |
...
| MCP protocol-profile and conformance boundary | `volicord-mcp-protocol` owns the closed typed revision set, reviewed production profiles, message/tool/schema feature declarations, explicit iteration order, and preferred server revision. `volicord-mcp` separately owns the one deterministic offline conformance revision declaration consumed by adapter tests, the CLI matrix, and the specification checker. Exact parity with manifest production support is required. Neither declaration is a host fixture or external certification; host compatibility observations do not own server preference or revision conformance. | [Source Map](source-map.md), [Testing Strategy](testing-strategy.md), and [MCP Transport](../reference/mcp-transport.md). |
| MCP adapter boundary | `volicord mcp --stdio` is the public transport entry path. Volicord tool owners provide one canonical tool model; `volicord-mcp` holds its MCP representation and projects definitions and results through the selected profile instead of forking tool ownership by revision. The adapter also resolves Runtime Home and Agent Connection context, validates startup/session facts, exposes tools by connection mode, selects permitted projects, decodes `tools/call`, derives adapter-managed local invocation facts, calls Core, and wraps Core JSON as MCP content. | [Request Lifecycle](request-lifecycle.md), [Source Map](source-map.md), [MCP Transport](../reference/mcp-transport.md), and [Agent Connection](../reference/agent-connection.md). |
| Administrative CLI and Codex adapter | The CLI owns Codex configuration discovery, managed-entry installation and validation, dependency-aware verification policy, deterministic diagnostic root selection, concise/verbose/JSON presentation of one report, finding and runtime-session lookup, repair, uninstall, and stdio MCP launch. The Codex adapter converts the canonical managed launch contract to and from Codex TOML while preserving only the allowed tool-approval overlay. It does not classify Linux or WSL2 and does not issue runtime authorization. | [CLI Workflows](cli-workflows.md), [Source Map](source-map.md), [Administrative CLI](../reference/admin-cli.md), [Agent Connection](../reference/agent-connection.md), and [Security](../reference/security.md). |
...
| Tests and validation | Implementation tests verify owner-defined facts at the appropriate layer. MCP production support requires a pinned released schema, a production profile, and inclusion in the adapter-owned offline conformance matrix; the checker enforces exact set parity. Tracked pre-release schemas remain outside production support, and local coverage is not external certification. Tests, fixtures, generated snapshots, and documentation checks do not become product contract owners. | [Testing Strategy](testing-strategy.md) and [Validation](../maintain/validation.md). |
