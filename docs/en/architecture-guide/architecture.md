...
| Runtime Home bootstrap | `volicord-store` inspects an existing Registry read-only against the exact current manifest and physical schema. An absent final path is prepared in same-parent staging with its singleton and initial installation profile, validated and synchronized, then published by an atomic no-replace rename. Incompatible state and every pre-publication failure remain outside mutation of the final path. | [Runtime Boundaries](../reference/runtime-boundaries.md), [Storage Versioning](../reference/storage-versioning.md), [Storage Records](../reference/storage-records.md), and [Storage DDL](../reference/storage-ddl.md). |
| Init setup transaction | `volicord-cli` builds one read-only typed plan for Runtime Home, Store, Codex configuration, repository managed files, and activation. Prepare validates snapshots and stages same-directory files while `volicord-store` prepares and checkpoints recovery entries. Commit applies Store state, replaces repository files deterministically, replaces Codex configuration last, records the integration revision, and only then exposes activation. Failures use bounded reverse rollback and preserve later external changes. This is not global cross-filesystem atomicity. | [Administrative CLI](../reference/admin-cli.md), [Runtime Boundaries](../reference/runtime-boundaries.md), [Agent Connection](../reference/agent-connection.md), and [Failure Model](../reference/failure-model.md). |
