...
| `crates/volicord-cli` | `crates/volicord-cli/src/main.rs`, `crates/volicord-cli/src/setup_command.rs`, `crates/volicord-cli/src/setup_command/`, `crates/volicord-cli/src/connection_command.rs`, `crates/volicord-cli/src/connection_command/`, `crates/volicord-cli/src/guard_command.rs`, `crates/volicord-cli/src/guard_command/`, `crates/volicord-cli/src/guard_integration/`, `crates/volicord-cli/src/doctor_command.rs`, `crates/volicord-cli/src/project_context.rs`, `crates/volicord-cli/src/export_command.rs`, `crates/volicord-cli/src/user_command.rs`, `crates/volicord-cli/src/host_integration/`, `crates/volicord-cli/src/registration.rs` | `main.rs` dispatches administrative commands, `volicord mcp` command modes, and binary exit behavior. `setup_command.rs` and `setup_command/` handle installation profile readiness, setup workflow, discovery, linking, shell startup, prompts, and setup output. `connection_command.rs` parses and dispatches `volicord init`, `volicord connection add`, `volicord connection list`, and `volicord connection ...` commands; `connection_command/` carries argument parsing, connection provisioning orchestration, selection, verification, MCP process checks, and rendering support for those commands. `guard_command.rs` and `guard_command/` dispatch and implement guard hook command phases, event normalization, observation extraction, mutation classification, prompt capture, write-ticket checks, and hook output rendering. `guard_integration/` owns guard integration planning, generated guard file application, capability metadata, policy helpers, host-specific guard hook planning, and factual audit helpers over generated files, recorded metadata, and observed state consumed by connection status and doctor diagnostics. `doctor_command.rs` reads installation, connection, and guard facts for diagnostic reporting. `project_context.rs` detects Git repository roots and orchestrates project commands. `export_command.rs` renders authority bundle exports. `user_command.rs` parses and orchestrates local User Channel status and judgment commands. `host_integration/` owns Codex, Claude Code, and managed host configuration planning. `registration.rs` builds Agent Connection, Connection Projects, and User Channel metadata. |
