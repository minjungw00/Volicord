...
| CLI architecture | This page's administrative setup flow, `crates/volicord-cli/src/main.rs`, `setup_command.rs`, `doctor_command.rs`, `connection_command.rs`, `connection_command/`, `guard_integration/`, `project_context.rs`, `user_command.rs`, and `host_integration/`. | [Administrative CLI](../reference/admin-cli.md), [Runtime Boundaries](../reference/runtime-boundaries.md), and [Agent Connection](../reference/agent-connection.md). |
