...
| Project Store facade and reads | `CoreProjectStore` remains the project-database facade. Its handle and open entry points are infrastructure-owned, while aggregate modules own their column projections, read SQL, row and JSON decoding, inherent facade methods, and focused tests. Project state, replay, Tasks and acceptance data, Change Units, Write Tickets, Runs, evidence, artifacts, User Actions, continuity, reconciliation observations, blockers, events, Agent Sessions, and clock data therefore have explicit current owners under `core_pipeline/`; workflow-policy records remain in `workflow_records.rs`. | [Storage and Transactions](storage-and-transactions.md), [Source Map](source-map.md), [Storage](../reference/storage.md), and [Storage Records](../reference/storage-records.md). |
