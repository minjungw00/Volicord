...
| Runtime Home mutation admission | `volicord-platform-fs` owns one shared/exclusive OS-backed coordination region per canonical Runtime Home. `SharedWriter` admits concurrent ordinary writers, while `ExclusiveSetup` conflicts with every shared or exclusive holder. A borrowed permit carries the exact canonical target and held mode to higher layers without exposing the OS handle. Setup is the current exclusive consumer; the current Store mutation APIs do not yet require the permit from every writer. The coordination file is outside the Runtime Home, and the lock is coordination rather than actor identity, a credential, or a security boundary. | [Source Map](source-map.md) and [Runtime Boundaries](../reference/runtime-boundaries.md). |
