...
| Codex release-validation cells | `tests/release-validation`, Cargo package `volicord-release-validation-tests`. | Strict parsing and exact lookup for the [checked-in manifest](../reference/host-release-evidence.md#canonical-checked-in-manifest), explicit test-only descriptor rejection, shared required scenarios, and independent `linux`, `macos`, `native_windows`, and `wsl2` cells. | Treating a fixture, repository test, one platform result, or rebuilt executable as finalized-artifact evidence, production runtime trust, Core evidence, host attestation, or support for another cell. |
...
The release-validation package follows the current
[target layout](../reference/host-release-evidence.md#release-validation-target-layout)
without making private helper structure a contract. Contract tests reject
unknown or duplicate fields, malformed digests, unsorted or duplicate
capabilities, non-`record` profiles, invalid platform values, mismatched
evidence bindings, and test-only descriptors presented as release cells. Exact
support-lookup tests return `unsupported_host_artifact` for an unregistered or
platform-mismatched digest and prove that support never propagates across
artifacts, capabilities, or platform cells.

Fixtures exercise parsing, routing, and negative cases only. Shared scenario
modules define one canonical domain setup and outcome, while platform modules
supply platform-specific filesystem, process, and projection assertions. Only
an exact finalized-artifact run in the matching
[independent platform cell](../reference/host-release-evidence.md#independent-platform-cells)
can produce `validation_evidence.status=passed`; a fixture or repository test
cannot register support in the canonical manifest.
...
UI receipt is required or credited. A bounded JSON summary is printed. This
standalone result is diagnostic test evidence, not a
[`CodexReleaseCell`](../reference/host-release-evidence.md#codexreleasecell) and
not a second support list. It can contribute to `validation_evidence` only when
the current release runner executes the exact finalized artifact in the
matching platform cell and binds the artifact, platform, capability set, and
`record` profile. Only a reviewed `passed` cell in the
[canonical checked-in manifest](../reference/host-release-evidence.md#canonical-checked-in-manifest)
registers support.
