...
| [Task control levels and integration profiles are separate axes](task-control-levels-vs-integration-profiles.md) | Why Record and Detective remain host-integration choices while Core records a separate project-policy-constrained control level for each Task. |
| [Write Ticket validity is bound to relevant work state](state-bound-write-ticket-validity.md) | Why ticket compatibility follows the Task, Change Unit, scope, baseline, workspace, and approval basis instead of unrelated project-wide mutations or a fixed default lifetime. |
