...
`volicord-host-release-manifest-v1` file without overwriting. After the gate
...
`volicord-host-release-audit-v1` file without overwriting. The audit may not
...
This decision adds a test-only external release contract and tightens internal
managed-host correlation; it does not change a public Core API schema, public
MCP method, SQLite DDL, or storage-profile version. There is no import or
migration from ad hoc historical live-host JSON into the v1 schemas. Such
results remain historical diagnostics and cannot satisfy this gate.
...
deployment surface; its externally stored v1 artifacts are new opt-in release
validation output.
