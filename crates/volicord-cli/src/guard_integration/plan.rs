...
fn agents_guidance_block() -> String {
    format!(
        concat!(
            ...
            "- Use the exact method-and-semantic-variant action form and `action_form_ref` for the called action. Variants in one required-method group are alternatives, not simultaneous requirements. Copy every fixed authority argument exactly; do not omit or alter it, and never repurpose another method or variant's form. Use the type-owned MCP tool schema and returned retry contract; ordinary CLI `--help` and binary strings are not MCP request schemas.\n",
            "- For `volicord.update_scope`, select `keep_current_change_unit`, `create_current_change_unit`, or `replace_current_change_unit` from the current catalog. Use `replace_current` for an explicit Change Unit retarget or rebaseline only when that variant-specific form is current; never retry replacement with an incompatible keep-current form.\n",
            "- Preserve JSON primitive types: never convert null, booleans, or numbers into strings, and never infer another tagged-union branch after validation rejects the selected branch. At nullable `BaselineRef` positions, use JSON null for absence; never use the string `\"null\"` as a baseline or null alias.\n",
            ...
            "- When Volicord reports persisted baseline corruption, treat it as a stored-state failure: stop the mutation, surface the persisted-data failure accurately, and do not continue or substitute an action-form retry.\n",
...
mod tests {
    ...
    fn generated_host_guidance_preserves_the_canonical_verification_boundary(
        ...
        for required in [
            ...
            "call only a currently allowed task-bound method",
            "exact method-and-semantic-variant action form",
            ...
            "MCP argument error does not diagnose Task-state corruption",
            ...
            "pre-Core admission rejection accurately",
