...
fn agents_guidance_block() -> String {
    format!(
        concat!(
            ...
            "- Use the exact method-and-semantic-variant action form and `action_form_ref` for the called Agent transition. Every form projects one transition descriptor. Copy every fixed authority argument exactly; do not omit or alter it, and never repurpose another method or variant's form. Use the type-owned MCP tool schema and returned retry contract; ordinary CLI `--help` and binary strings are not MCP request schemas.\n",
