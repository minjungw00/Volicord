...
fn agents_guidance_block() -> String {
    format!(
        concat!(
            ...
            "- Follow the exact Core transition and its current action form. Use only the exact recovery action and form returned from Core, never infer recovery from error text or field names, and never reuse a form for another method or semantic variant.\n",
            ...
            "- A transition compatibility error does not diagnose persisted corruption. Report pre-Core rejection, Core rejection, and persisted-data corruption as distinct outcomes with their exact commit and side-effect facts.\n",
...
mod tests {
    ...
    fn generated_host_guidance_preserves_the_canonical_verification_boundary(
        ...
        for required in [
            ...
            "never infer recovery from error text or field names",
