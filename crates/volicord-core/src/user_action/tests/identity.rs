...
fn reconciliation_identity_preserves_the_deduplication_origin() {
    ...
    let metadata: serde_json::Value =
        serde_json::from_str(&identity.metadata_json).expect("metadata must be valid JSON");
    assert_eq!(metadata["created_by"], "volicord.reconcile_changes");
    assert_eq!(metadata["unrecorded_change_id"], "change-observed");
}
