...
pub(crate) fn server_instructions() -> String {
    format!(
        "Volicord records task scope, write tickets, evidence, runs, user-action requests, evidence attachments, and Close Status for explicitly registered Product Repositories. Preserve the user's requested outcome when choosing Task scope: when analysis or shaping is a step toward an implementation outcome, keep one work Task and record that step as a shaping_update; use advisor only when the requested outcome itself is read-only advice. If the broader outcome is unclear, keep the known boundary in shaping state or ask the user instead of expanding it. If project selection is unclear, call {} and use one listed project_selector; do not guess from folders, roots, labels, or memory. For the canonical request `Run the Volicord integration verification.`, call {} and then {}. Call the returned {} only when begin reports `next_action=call_guard_probe`; otherwise do not probe an already acknowledged or passed run. Then call {} to read current completion. Only that first-party state-directed workflow proves current managed MCP and Guard correlation. If Volicord tools are not exposed, report the managed MCP connection as unavailable; do not substitute raw stdio, hand-author Codex `_meta`, or treat resources/list or resource templates as proof of tool availability. Read-only connection status and CLI MCP preflight are diagnostic only and are not managed-host evidence. Hook trust remains user/host owned. Mutation tools default to a fresh compact authority receipt plus the method outcome needed for the next step; request detail=workflow for current next actions or detail=full only when the bounded full method result is needed. When a mutation returns a non-null operation_result_ref, use {} for omitted exact historical bytes and {} separately for current authority; never retry an applied mutation. Volicord state management is separate from product-file edit authority: product-file edits still require the host/user path and any required write ticket. A write ticket records intended product-file changes; it is not OS permission, review bypass, access control, or a promise of automatic tool use.",
...
mod tests {
    ...
    fn server_instructions_define_the_only_managed_integration_proof_sequence() {
        ...
        assert!(instructions.contains("next_action=call_guard_probe"));
        assert!(instructions.contains("do not probe an already acknowledged or passed run"));
