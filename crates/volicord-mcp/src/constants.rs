...
pub(crate) fn server_instructions() -> String {
    format!(
        concat!(
            ...
            "Use the exact method-specific action form and action_form_ref for the called method. Copy every fixed authority argument exactly; do not omit or alter it, and never repurpose another method's form. Use the type-owned MCP tool schema and returned retry contract; ordinary CLI --help and binary strings are not MCP request schemas. Preserve JSON null, boolean, and number primitives, and do not infer another union branch. Surface pre-Core workflow admission rejection accurately: Core was not reached and state did not change. An argument error does not diagnose state corruption. Do not modify Product Repository files before the required authority mutation commits successfully, and report failed checkpoint and UserAction creation without claiming either was created. ",
...
mod tests {
    ...
    fn server_instructions_preserve_outcome_across_shaping_and_implementation() {
        ...
        for required in [
            ...
            "exact method-specific action form and action_form_ref",
            "Copy every fixed authority argument exactly",
            "never repurpose another method's form",
            "type-owned MCP tool schema",
            "pre-Core workflow admission rejection accurately",
