...
pub(crate) fn server_instructions() -> String {
    format!(
        concat!(
            ...
            "Follow the tagged workflow's required_action; do not call workflow tools speculatively or select progression from top-level array order. The tagged workflow action catalog is mutation admission authority: call only a currently allowed task-bound method, and never speculate with a different shaping or implementation method. Read-only status grants no mutation authority. Never replace the current checkpoint to remove a pending or accepted-but-unapplied decision. When revising a checkpoint, carry every current compatible applied decision explicitly through carry_forward_application_refs; never replace a checkpoint to discard applied authority. ",
            "Use the exact method-and-semantic-variant action form and action_form_ref for the called action. Variants in one required-method group are alternatives, not simultaneous requirements. Copy every fixed authority argument exactly; do not omit or alter it, and never repurpose another method or variant's form. For volicord.update_scope, select keep_current_change_unit, create_current_change_unit, or replace_current_change_unit from the current catalog. Use replace_current for an explicit Change Unit retarget or rebaseline only when that variant-specific form is current; never retry replacement with an incompatible keep-current form. Use the type-owned MCP tool schema and returned retry contract; ordinary CLI --help and binary strings are not MCP request schemas. Preserve JSON null, boolean, and number primitives, and do not infer another union branch. At nullable BaselineRef positions, use JSON null for absence; never use the string \"null\" as a baseline or null alias. Surface pre-Core workflow admission or variant rejection accurately: Core was not reached and state did not change. An argument error does not diagnose state corruption. When Volicord reports persisted baseline corruption, treat it as a stored-state failure: stop the mutation, surface the persisted-data failure accurately, and do not continue or substitute an action-form retry. Do not modify Product Repository files before the required authority mutation commits successfully, and report failed checkpoint and UserAction creation without claiming either was created. ",
...
mod tests {
    ...
    fn server_instructions_preserve_outcome_across_shaping_and_implementation() {
        ...
        for required in [
            ...
            "tagged workflow action catalog is mutation admission authority",
            "only a currently allowed task-bound method",
            "never speculate with a different shaping or implementation method",
            "Read-only status grants no mutation authority",
            "exact method-and-semantic-variant action form and action_form_ref",
            "Copy every fixed authority argument exactly",
            "method-and-semantic-variant action form",
            "never repurpose another method or variant's form",
            "select keep_current_change_unit, create_current_change_unit, or replace_current_change_unit",
            "explicit Change Unit retarget or rebaseline",
            "never retry replacement with an incompatible keep-current form",
            "type-owned MCP tool schema",
            "pre-Core workflow admission or variant rejection accurately",
            "Core was not reached and state did not change",
            "returned retry contract",
            "CLI --help and binary strings are not MCP request schemas",
            "Preserve JSON null, boolean, and number primitives",
            "At nullable BaselineRef positions, use JSON null for absence",
            "never use the string \"null\" as a baseline or null alias",
            "do not infer another union branch",
            "argument error does not diagnose state corruption",
            "persisted baseline corruption",
            "stop the mutation",
            "surface the persisted-data failure accurately",
            "do not continue or substitute an action-form retry",
