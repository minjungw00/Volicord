...
if [ "$uninstall" -eq 1 ]; then
    if command -v codex >/dev/null 2>&1; then
        codex mcp remove volicord >/dev/null 2>&1 || true
    fi
...
if [ "$setup_codex" -eq 1 ]; then
    if ! command -v codex >/dev/null 2>&1; then
        echo "Codex CLI is unavailable; executables are installed but MCP registration was skipped" >&2
        exit 3
    fi
    codex mcp remove volicord >/dev/null 2>&1 || true
    codex mcp add volicord --env "VOLICORD_RUNTIME_DIR=$runtime_dir" -- "$bin_dir/volicord-mcp"
fi
