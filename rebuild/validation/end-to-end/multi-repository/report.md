...
production/test candidate HEAD `ba4df7e4afcf6b44c30bf938aac41e2acc121c5d`.
All 54 steps passed, no blocking classification was reported, and
`phase_8_ready = true` (2026-09-09). This candidate includes repaired
Dogfood evidence evaluation and Codex authority-continuation guidance, and is
ready for a wholly fresh Phase 8 campaign. No fresh
...
| --- | --- | --- |
| Volicord reconstruction repository | `validated_candidate_head = ba4df7e4afcf6b44c30bf938aac41e2acc121c5d` | authenticated Codex target `volicord`: status `passed`, classification `passed` |
...
unchanged at `ba4df7e4afcf6b44c30bf938aac41e2acc121c5d`.
...
| `final_command.cargo_metadata.outcome`; `final_command.cargo_metadata.exit_code`; `final_command.cargo_metadata.termination`; `final_command.cargo_metadata.spawn_error`; `final_command.cargo_metadata.duration_ms` | `succeeded`; `0`; `null`; `false`; `11.547` |
| `final_command.cargo_fmt` | `cargo fmt --manifest-path rebuild/Cargo.toml --all -- --check` |
| `final_command.cargo_fmt.outcome`; `final_command.cargo_fmt.exit_code`; `final_command.cargo_fmt.termination`; `final_command.cargo_fmt.spawn_error`; `final_command.cargo_fmt.duration_ms` | `succeeded`; `0`; `null`; `false`; `886.297` |
| `final_command.cargo_clippy` | `cargo clippy --manifest-path rebuild/Cargo.toml --workspace --all-targets --all-features -- -D warnings` |
| `final_command.cargo_clippy.outcome`; `final_command.cargo_clippy.exit_code`; `final_command.cargo_clippy.termination`; `final_command.cargo_clippy.spawn_error`; `final_command.cargo_clippy.duration_ms` | `succeeded`; `0`; `null`; `false`; `975.309` |
| `final_command.cargo_test` | `cargo test --manifest-path rebuild/Cargo.toml --workspace --all-targets --all-features` |
| `final_command.cargo_test.outcome`; `final_command.cargo_test.exit_code`; `final_command.cargo_test.termination`; `final_command.cargo_test.spawn_error`; `final_command.cargo_test.duration_ms` | `succeeded`; `0`; `null`; `false`; `58857.428` |
...
`final_summary_sha256 = e44b1d2a0ecd4546d092991a9f75f5dd3d2c0d47a548e740826a60cee313cef4`.
...
`official_v11.result_sha256 = 91df639df56710c306ec3ae58c13b577ad6913663d17d1b2aba24f602cfe524c`.
...
`live_provider_qualification.evidence_sha256 = e947bf441632a854829349a8bc1001bc0a92a18b15ad4b910d774e86dbca4d4b`.
...
snapshot `ba96af7c4abc07eca5dc89664599bc85a8ed8a1e734b2c9a547239c1cba07144`
and analysis snapshot `3240d7707131fc33aa87dc1af847e54be92d775186f53e011939b62fb817b269`.
...
coupled regression, and created the changed candidate independently
re-admitted and sealed above. The failed predecessor capsule remains diagnostic.
...
The capsule records exact-final command durations of 11.547 ms, 886.297 ms,
975.309 ms, and 58857.428 ms. Official V11 reports
...
| --- | ---: | ---: |
| V11 duration (ms) | 648094.855 | 900000 |
| MCP process high-water RSS (bytes) | 2767609856 | 4294967296 |
| Largest Analysis Snapshot (bytes) | 993119401 | 2147483648 |
| Longest MCP call (ms) | 15001.241 | 90000 |

The capsule records 461 MCP calls, 5511 resource samples, total MCP call time
236724.229 ms, and zero sampling errors. These are bounded technical-journey
...
`evidence_archive.candidate_head = ba4df7e4afcf6b44c30bf938aac41e2acc121c5d`,
`evidence_archive.filename = validation-evidence-ba4df7e4afcf.tar.gz`,
`evidence_archive.sha256 = 61a88e3ede2c35b7f0731fc2c8d1c68e782c0e0d32de92e048e31ce2718d7efb`,
`evidence_archive.size_bytes = 14770`, and
...
`ba4df7e4afcf6b44c30bf938aac41e2acc121c5d`. The later documentation-only
...
- The successful capsule and verified archive were copied byte-for-byte to an
  explicit external handoff directory before this conclusion was written.
  The conversation handoff records that directory and the later documentation
  HEAD separately from the sealed candidate.
- Exact-final summary SHA-256:
  `e44b1d2a0ecd4546d092991a9f75f5dd3d2c0d47a548e740826a60cee313cef4`.
- Official V11 result SHA-256:
  `91df639df56710c306ec3ae58c13b577ad6913663d17d1b2aba24f602cfe524c`.
- Live production-provider evidence SHA-256:
  `e947bf441632a854829349a8bc1001bc0a92a18b15ad4b910d774e86dbca4d4b`.
