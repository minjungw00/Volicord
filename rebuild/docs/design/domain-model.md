...
active-agent assertion을 보존한다. 각 conclusion은 concrete reviewed outcomes와 해당 category의 real discovered choice identity를
연결하거나 bounded Source와 typed no-independent-fork basis/rationale를 명시한다. Alternative별
residual-fork closure는 fixed outcome, credible implementation comparison, remaining outcome 부재와
current Source identity를 보존하며 residual outcome이 있으면 subordinate choice로 표현한다. 이는 materiality나
