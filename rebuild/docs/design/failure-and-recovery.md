...
| required Question/Decision transition rejected or unavailable | Inquiry/Host의 affected transition `failed`; work authority는 `question_required`/blocking 유지 | current Materiality Review, Candidate/Question lifecycle, canonical records와 unrelated reads/work | ordinary prose/chat, Context-only preference, recommendation, Guarded confirmation 또는 attempted work를 Question response/Decision으로 추론 | exact failed action과 blocker를 보고하고 `Question Candidate → Question → current Frontier receipt → current-host response → Decision`의 실패 지점부터 retry; 성공 뒤 Materiality Review applicability 재평가 |
| Checkpoint work authority discovery/review missing/stale/late/unresolved | Inquiry/Local Operations `failed`; no Checkpoint publication | Engineering Choice Discovery와 Materiality Review Candidate, canonical Goal/Question/Decision, Analysis Snapshot과 ordinary repository work; deterministically observed late work-authority/readiness revision은 dimension과 affected changed path를 보존 | pause/completion success 추론, broad Goal authority, post-work discovery/review backfill, recommendation/convention을 authority로 승격, later fact/agent/delegation/Decision/research/learning-ready meaning으로 earlier affected work를 certify | missing discovery/review는 새 bounded work의 pre-work boundary에서만 기록; every-choice mapping, exploratory evidence나 existing Question/Decision lifecycle을 완료한다. Late first review나 deterministically late work-authority/readiness revision은 이미 수행한 work를 retroactively repair하지 못하며 새 bounded pre-work boundary가 필요함 |
| executable work scope or coupled-artifact review missing/stale/late-expanded | Inquiry/Local Operations `failed`; `ready_for_work`/Checkpoint publication 차단 | descriptive Materiality Review, exact typed path/component/work-context binding, complete coupled-artifact category review와 baseline/current fingerprints; 한 Checkpoint의 uncovered values와 current binding/basis/next action을 한 diagnostic으로 보존 | descriptive affected scope를 executable scope로 추론, artifact class를 자동 허용, first mismatch 뒤 나머지를 숨김, already-changed path를 late expansion으로 authorize | existing Materiality inspection transition으로 first write 전에 bounded scope와 included artifact를 bind; newly discovered artifact는 first write 전 prospective expansion하고 late path는 새 pre-work boundary에서 처리 |
| required Learning Deliberation missing/pending/reconsidered | Inquiry/Local Operations `failed`; affected work는 `learning_deliberation_pending`, no Checkpoint publication | authority result, exact discovery/review basis, current Candidate rounds와 explicit participation Source; unsupported routine downgrade는 prior deliberation-worthy state를 유지 | inactive로 추론, user-owned blocker 대체, pre-response recommendation 삽입, restart를 terminal로 취급, agent preference/implementation selection으로 routine downgrade | exact dimension Candidate를 begin/continue; select 뒤 feedback과 completion, delegate/skip terminal 또는 bounded research/prototype 후 readiness 재평가. Routine downgrade는 current Source-backed research/prototype evidence 또는 exact current-host user withdrawal/narrowing basis로만 prior Review를 revise한다. Equivalent successor는 이 effective state와 terminal non-Decision satisfaction을 이어받으며 pending/reconsidered branch는 계속 차단한다 |
...
| dogfood SessionStart evidence interpretation failure | validation evidence `evidence_failed`; malformed/conflicting identity, binding mismatch와 indeterminate timing은 `evidence`, validator state/boolean 모순은 `validation_internal` | raw evidence, exact repository/session binding, candidate와 frozen-task identity; product behavior 판단은 activation 검증과 분리 | transport `task_started`를 actual task visibility로 간주, unfamiliar ordering을 operator setup fault로 귀속, later MCP로 missing activation을 복구, prior failed campaign을 qualification evidence로 재사용 | validation owner가 supported transport/fixture를 검증; proven absent/late만 operator environment failure이며 실제 setup correction은 user-controlled trust 경계를 유지 |
| dogfood capture lifecycle/derived intake failure | validation `evidence_failed`; behavior, activation/environment와 validator-internal attribution은 독립 유지 | staged all-cycle diagnostics, exact raw hashes, intermediate interrupted turns와 terminal completion basis | incomplete/ambiguous turn을 success로 추정, unrelated later completion으로 recovery, partial campaign publication, rejected campaign의 rollout 교체 | validation owner가 staged evaluation과 derived savepoint를 완료한 뒤 하나의 terminal summary/state/inventory를 게시. Publication 실패는 original bytes rollback; journal이 남으면 staging/backup을 보존하고 `repair_required` read/mutation barrier로 차단. Rejected campaign은 historical evidence이며 새 campaign identity가 필요함 |
| dogfood candidate executable binding mismatch | validation evidence `evidence_failed`; candidate binding integrity rejection | prepared campaign, exact candidate HEAD와 bound executable names/private paths/SHA-256; existing immutable historical evidence remains read-only | current path existence나 newly observed hash를 prepared identity로 간주, mismatch를 qualitative uncertainty로 downgrade, replacement bytes로 activation/export/document/Viewer/CLI evidence publish | validation owner가 each candidate executable use 전후 hash를 확인하고 publication을 차단. Prepared campaign은 고치거나 rebind하지 않고 correct clean candidate로 새 campaign identity를 준비 |
