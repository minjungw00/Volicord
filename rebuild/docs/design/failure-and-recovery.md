...
| Checkpoint work authority missing/stale/late/unresolved | Inquiry/Local Operations `failed`; no Checkpoint publication | Materiality Review Candidate, canonical Goal/Question/Decision, Analysis Snapshot과 ordinary repository work | pause/completion success 추론, post-work review backfill, recommendation/convention을 authority로 승격 | missing review는 새 bounded work의 pre-work boundary에서만 기록; exploratory evidence나 existing Question/Decision lifecycle을 완료하고 same Goal/baseline review를 revise한 뒤 retry; late first review는 이미 수행한 work를 retroactively repair하지 못함 |
