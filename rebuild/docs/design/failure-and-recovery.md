...
| executable work scope missing/stale/late-expanded | Inquiry/Local Operations `failed`; `ready_for_work`/Checkpoint publication 차단 | descriptive Materiality Review, exact typed path/component/work-context binding과 baseline/current fingerprints; 한 Checkpoint의 uncovered values와 current binding/basis/next action을 한 diagnostic으로 보존 | descriptive affected scope를 executable scope로 추론, first mismatch 뒤 나머지를 숨김, already-changed path를 late expansion으로 authorize | existing Materiality inspection transition으로 first write 전에 bounded scope를 bind; unchanged prospective scope만 mechanically correct하고 late path는 새 pre-work boundary에서 처리 |
