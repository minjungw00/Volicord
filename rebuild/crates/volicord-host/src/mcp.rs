...
fn tool_contract(name: &str) -> Option<ToolContract> {
    let (description, input_schema, behavior) = match name {
        ...
        "checkpoint_record" => (
            "Record a grounded Checkpoint from a canonical Goal, the exact pre-work Analysis Snapshot retained for this bounded session, current analysis, applicable Decisions, and truthful verification evidence. The baseline_analysis_snapshot_id must identify an analysis captured after initialization or successful Recall and before the first ordinary repository write; a snapshot first captured after the bounded work is conceptually invalid even when snapshot provenance cannot prove edit ordering. Every executed verification requires the exact transient command_invocation separately from the presentation-only command_label so Volicord can derive a durable fingerprint without retaining raw arguments. A passed or failed verification also requires the numeric exit status from that same actually observed execution; output-only text is insufficient. Verified completion or pause after repository changes requires rerunning relevant bounded verification after the final meaningful mutation; a later mutation invalidates earlier verification for that work. Pre-mutation success, inspection, a prior Checkpoint, or prose success cannot certify later changes. Use a standalone bounded verifier as the unambiguous terminal evidence path. Compound commands remain useful for ordinary diagnostics, but validation mixed with unrelated or semantically ambiguous operations must not be the only terminal evidence. Long-running or polled commands keep the same execution identity through terminal completion; record the exact invocation and its actual numeric exit/termination outcome together. Read-only inspection, explanation-only requests, and no-write exploratory continuation have no artificial post-mutation validation requirement. Incidental inspection commands need not be Checkpoint verification facts. For no-write research/prototype conclusions based on execution, first complete the bounded scratch experiment actually supporting the conclusion. Ad-hoc diagnostics may precede that terminal experiment. Afterward normally use only read-only inspection/reporting that does not supersede or obscure its execution basis before the concluding Checkpoint. If later substantive executable diagnostics occur, establish a later bounded exploratory execution with observed numeric completion before recording that Checkpoint; do not reuse an earlier superseded experiment. Truthfully record no repository changed paths and include the terminal bounded experiment in verification facts, with its exact transient command_invocation and actual numeric exit/termination from the same execution. The durable Source retains the fingerprint and outcome, not raw arguments. Never claim success from prose or output alone; preserve failed or indeterminate experiments as such, including known limits. No-write exploratory evidence, repository inspection, repository validation, and post-mutation terminal verification are distinct: never promote arbitrary successful commands to repository validation or use exploration to certify repository mutations. Do not run meaningless extra repository validation merely to create a Checkpoint. Explicitly classify verification_basis: behavior-preserving/refactor completion and Compatibility effects require a source-grounded surface review linked to focused passed commands; inspect affected override hooks and default propagation when repository evidence makes them relevant.",
            object_schema(
                vec![
                    ("project_id", identity_schema("Project identity")),
...
fn learning_participation_schema() -> Value {
    json!({"description":"Learning participation answers whether the user explicitly requested learning/explanation participation within this bounded Goal; learning value assesses one agent-owned/delegated dimension. Preserve active participation with its exact current-host user-turn Source and verbatim statement even when every current dimension is routine. Routine value remains non-interrupting: active participation alone requires no Learning Deliberation, Question, or canonical Decision. Without explicit participation, inactive is the default; generic coding, agent-generated explanation, or keywords without current-host user provenance do not activate it.","oneOf":[
        object_schema(
...
fn checkpoint_verification_schema() -> Value {
    json!({
        "type": "array",
        "description": "Independent verification observations; verified completion/pause after changes requires verification after the final meaningful mutation. Use a standalone bounded verifier with exact transient invocation and numeric terminal outcome from the same execution; earlier verification or prose success cannot certify later changes",
        "minItems": 1,
        "items": {
            "oneOf": [
                object_schema(
...
fn materiality_draft_json(
    ...
    json!({
        ...
        "learning_participation":{"description":learning_participation_schema()["description"],"input_alternatives":schema_alternatives(learning_participation_schema())},
...
fn authority_learning_routing_json() -> Value {
    json!({
        ...
        "participation_rule":learning_participation_schema()["description"],
